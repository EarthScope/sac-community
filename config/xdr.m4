...
AC_DEFUN([LIBRPC_ON],
         [ AC_DEFINE([HAVE_LIBRPC], [1], [Compile with XDR file format]) librpc="xdr" ])
AC_DEFUN([LIBRPC_OFF],
         [ AC_DEFINE([HAVE_LIBRPC_DISABLED], [1], [Compile without XDR file format]) librpc=" "])

AC_DEFUN([CHECK_XDR], [
     AC_ARG_ENABLE(xdr, AS_HELP_STRING([--enable-xdr],[Enable XDR File Format]),
                     [ AS_IF( [ test x$enableval != xyes ], [ LIBRPC_OFF ], [ LIBRPC_ON ] ) ],
                     [ LIBRPC_OFF ])
     AS_IF( [ test x${librpc} = xxdr ], [ CHECK_LIBRPC ] )
])
