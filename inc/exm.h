...
struct t_kmexm {
    ...
    char kprmt[128];    /** String containing the prompt, terminated with '$' */
    ...
    char kmcdir[MMCDIR][MCPFN + 1];
                                  /** macro, Macro directories */
...
void setprompt(char *prompt, int prompt_s);
void settextwait(char *mode);
void tracereport(int *nerr);
void tracevariable(int activate, int blackboard, char *variable, int *nerr);
...
void xeval(int *nerr);
void xexmc(int index, int *nerr);
void xfg(int *nerr);
void xgetbb(int *nerr);
void xhelp(int lprint, int *nerr);
