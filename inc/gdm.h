...
typedef void (*event_i_t) (int *number, int *nerr);
...
typedef char *(*fill_t) (unsigned int height, unsigned int width, float data[],
                         float dmin, float range, int nspeudocolors,
                         int nsaccolors, int ndefcolors, int *nerr);
typedef void (*put_t) (char *data, unsigned int xloc, unsigned int yloc,
...
typedef void (*set_pseudo_color_table_t) (int *num, unsigned int nentry,
                                          ...
                                          float blue[]);
typedef void (*set_text_size_t) (float width, float height);
typedef void (*get_window_status_t) (int number, int *exists);
typedef void (*set_text_angle_t) (float angle);
typedef void (*adjust_geometry_t) (unsigned int *width, unsigned int *height);
typedef void (*stroke_t) ();
typedef void (*calc_loc_t) (float *x, float *y, float *cx, float *cy,
                            unsigned int w, unsigned int h, unsigned int iw,
                            unsigned int ih);
typedef void (*show_image_t) (float *data, unsigned int iw, unsigned int ih,
                              float xmin, float xmax, float ymin, float ymax,
                              float x, float y, float w, float h,
                              int npseudocolors, int nsacolors, int ndefcolors,
...
void begindevice(char *device, int device_s, int *nerr);
void begindevices(char *devices, int devices_s, int ndevices, int *nerr);
void beginframe(int lprint, int *nerr);
void beginwindow(int number, int *nerr);
void calstatus(void);
void calvspace(void);
...
void cursor0(float *xloc, float *yloc, char *kchar);
void cursoroff(void);
void cursoron(void);
void cursortext(float *xloc, float *yloc, char *ktext, int ktext_s);
void draw(float xloc, float yloc);
void enddevice(char *device, int device_s, int *nerr);
void endframe(int ldelay, int *nerr);
void endgraphics(int *nerr);
void erase(int *nerr);
char **fill_colorbar(int npseudocolors, int width, int npricolors,
                     int ndefcolors, int *nerr);
char **fill_image(unsigned int height, unsigned int width, float data[],
                  float dmin, float range, int npseudocolors, int nsaccolors,
                  int ndefcolors, int *nerr);
void flushbuffer(int *nerr);
void get_geometry(unsigned int *width_return, unsigned int *height_return,
                  int *nerr);
void getalphainfo(int *nlines, char *erase, int erase_s);
void getdeviceinfo1(char *kdname, int kdname_s, int *idtype);
void getdevicename(int number, char *name, int name_s);
...
void getstatus(char *kstat, int *ltrue);
void getstringsize(char *ktext, int ntext, float *width);
float gettextangle();
void gettextjust(char *khorz, int khorz_s, char *kvert, int kvert_s);
...
void initctable(char *name, int name_s, int *nentry, int *nerr);
void loadctable(char *name, char *directory, int *nentry, int *nerr);
void move(float xloc, float yloc);
void put_image(char **data, unsigned int xloc, unsigned int yloc,
               unsigned int width, unsigned int height, int *nerr);
...
void settextjust(char *khorz, char *kvert);
void settextsize(float width, float height);
void settexttype(char *kqual);
void setvspaceclip(int lclip);
void setvspacetype(int lfull, double ratio);
void softwaretext(display_t * out, char *ktext, int ntext);
void set_position(float x, float y);
void stroke();
void text(char *ktext, int ktext_s, int nctext);
...
void show_image(float *data,    /* Image Data */
                ...
                unsigned int ih, float xmin, float xmax, float ymin, float ymax, float x,       /* In View space Coordinates [0,1] */
                float y, float w, float h, int nspeudocolors, int nsacolors,
                int ndefcolors, int lbinary, int *nerr);
...
void text_box_line(display_t * out, float x, float y, int width, int style,
                   float text_width, float text_height);
void
  text_box_symbol(display_t * out, float x, float y, int symbol, int with_line,
                  float text_width, float text_height);
