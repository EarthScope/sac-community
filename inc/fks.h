...
void inifks(void);
void xbbfk(int *nerr);
void covmat(int nch, int nsamples, int ltofilter, complexf * scm, int *nerr);
void fkevalp(complexf * scm, float *x, float *y, int nch, int naz, double wvnum,
             int iwvs, float *fks, float fkmax, float fkmax_az, float fkmax_wav,
             int maxflag);
void fkevalr(complexf * scm, float *x, float *y, int nch, int nsamps,
             double wvnum, float *fks);
void eigenanal(int nm, int n, complexf * a, char *hires, int m, complexf * s,
               int *nerr);
void xbeam(int *nerr);
void beamadd(float *s, float *b, int n, int iadv);
void xfksc(int index, int *nerr);
int eventfile_count_lines(char *file, int *nerr);
float lon180(float lon);
float lon360(float lon);
void clamp_lat_lon(int meridian, float *minlon, float *maxlon, float *minlat,
                   float *maxlat);
void plot_legend_box(FILE * fp, float x, float y, char *output);
void plot_legend_symbols(FILE * fp, char *symbol, float x, float y, float min,
                         float mean, float max, char *output);
void plot_legend_text(FILE * fp, float x, float y, float min, float mean,
                      float max, char *output);
