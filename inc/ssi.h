...
void DBheaderToSac(struct SACheader *header, int lall, sac * s);
void DBwfToSac(sac * s, struct trace *seis, int *nerr);
int OnOrOff(char *inString);
void SacHeaderToDB(struct SACheader *header, int whichHeaders, int idfl);
int SeisMgrCode(char *kcommand, int *nerr);
void echoAndShave(char *kcommand, char **kcommandPtr, int commandLength);
void SeisMgrToSac(DBlist tree, int lname, int *nerr, int Verbose, int lcutnow,
                  int takeEvid);
void alignFiles(int *nerr);
int deleteAllSacFiles(int *nerr, int lname);
void inissi(void);
void prefPicksToHeader(struct SACheader *header, int idfl, struct wfdisc *wf,
...
void xreaddb(char *kinput, int *nerr);
void xrecall(int *nerr);
void xrollback(int *nerr);
void xtablname(char *kinput, int *nerr);
