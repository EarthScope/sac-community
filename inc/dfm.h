...
struct t_kmdfm {
    ...
    char krdcssdir[MCPFN + 1];
                           /** Current directory to read CSS files from */
    ...
    char kcfile[2][MCPFN + 1];
                           /** Current Convert In/Out Filename */
...
void cnvfre(char *kcard, int kcard_s, int mentry, int *nentry, float *fentry,
            int *ientry, char *kalpha, int kalpha_s, int lstrict, int *nerr);
void decont(char *kcont, int kcont_s, int maxch, int *numch, int *numxch,
            int *numych, int *iopch, int *ltoend, int *nerr);
void defcut(char kcut[2][9], double ocut[2], int idfl, int *nerr);
void defmem(int idfl, int lcutnow, int *nerr);
void detnum(char *kfmt, int kfmt_s, int *nentry);
void gennames(char *headerfield, int lenheaderfield, string_list * list,
              int nfiles, int *nerr);
void getatw(char *krtw, int krtw_s, double *ortw, double *tmin, double *tmax,
            int *nofmin, int *nlnwin, int *nerr);
void getprefs(int lauth, int lphase);
string_list *getwfdiscs(string_list * list, int *nerr);
void inidfm();
void iztypeMessage(const int item, int iztype);
int lfilesok(string_list * files, char *kdir, int kdir_s, int lmore,
             int lheader, int lxdr, int *nerr);
void makeuniq(char *filelist, int lenfilelist, int nfiles, int *nerr);
void rdci(int idfl, char *kname, int kname_s, int *nlen, int *ndx1, int *ndx2,
          int *nerr);
void readcfl(int lmore, char *kdirin, int kdirin_s, string_list * list,
             int Verbose, int isASCII, float MaxMem, int *nerr);
void readfl(int ldata, int lmore, int lsdd, char *kdirin, int kdirin_s,
            string_list * list, int *nerr);
void readgse(int lmore, char *kdirin, int kdirin_s, string_list * list,
             int Verbose, int isASCII, float MaxMem, int *nerr);

void readsuds(int lmore, char *kdirin, int kdirin_s, string_list * list,
              int Verbose, int isASCII, float MaxMem, int *nerr);
...
void vfmax(int *maxf, int *nerr);
void vfmaxn(int maxn, int *maxf, int *nerr);
void vfrng(double rngmin, double rngmax, int *nerr);
void vfspec(int *nerr);
void vftime(int *nerr);
void wrci(int idfl, char *kname, int kname_s, char *kfmt, int *nerr);
void xHeaderWindow(int *nerr, int argc, char **argv);
void xch(int *nerr);
void xconv(int *nerr);
