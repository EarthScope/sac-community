...
void calc_loc3(unsigned int *xloc, unsigned int *yloc,
...
void cbar_window3(unsigned int xloc, unsigned int yloc, unsigned int height,
                  unsigned int w_height, unsigned int w_width,
                  double vspaceratio, double ypmax, int *nerr);
void changectable3(int nentry, int icolortable);
void createwindow3(int *win_num, float *xmin_vp, float *xmax_vp, float *ymin_vp,
                   float *ymax_vp, int *nerr);
void cursor3(float *xloc_vp, float *yloc_vp, char cchar[], int cchar_length);
void cursortext3(float *xloc_vp, float *yloc_vp, char ktext[],
                 int ktext_length);
...
void expose3(void);
char *fill_clrbar3(int npseudocolors, int width, int npricolors, int ndefcolors,
                   int *nerr);
char *fill_image3(unsigned int height, unsigned int width, float data[],
                  float dmin, float range, int npseudocolors, int nsaccolors,
                  int ndefcolors, int *nerr);
void flushbuffer3(int *nerr);
void get_geometry3(int window, unsigned int *width_return,
                   unsigned int *height_return, int *nerr);
void getalphainfo3(int *num_lines, char erase[], int erase_length);
void getdeviceinfo3(char dev_name[], int dev_name_length, int *dev_type);
...
void setpsctable3(int *win_num, unsigned int nentry, float red[], float green[],
                  float blue[]);
void settextsize3(float width, float height);
...
void get_window_size_viewport3(float *xmin, float *xmax, float *ymin,
                               float *ymax);

Font load_font(Display * display, char *name);
void calculate_location3(float *x, float *y, float *cx, float *cy,
                         unsigned int w, unsigned int h, unsigned int iw,
                         unsigned int ih);

void show_image_x11(float *data, unsigned int iw,       /* Image Size, data points */
                    unsigned int ih, float xmin,        /* Limits of the data */
                    float xmax, float ymin, float ymax, float x,        /* View space Coordinates [0,1] */
...
void xwindow_double_buffer(XWindow * xw, int flag);
void xwindow_resize(XWindow * xw, int width, int height, void *data);
void xwindow_expose(XWindow * xw, void *data);
