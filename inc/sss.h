...
#define SSS_EXTERN \
    extern struct t_kmsss kmsss; \
    extern struct t_cmsss cmsss; \
...
void definelimits(double beginwindow, double endwindow, double begindata,
                  double enddata, double delta, int *indexwindow,
                  int *indexdata, int *numintersect);
void iaspmodel(double zs, float *dstart, double dinc, int nloops, int *nerr);
void inisss(void);
void bkin(int lu, int nrec, int len, double buf[]);
void brnset(int nn, char *pcntl, int pcntl_s, int prflg[]);
void depcor(int nph);
void depset(double dep, float usrc[]);
void findtt(int jb, double x0[], int max_, int *n, float tt[], float dtdd[],
            float dtdh[], float dddp[], char *phnm, int phnm_s);
void fitspl(int i1, int i2, double tau[][4], double x1, double xn,
            double coef[][5]);
int iupcor(char *phnm, double dtdd, float *xcor, float *tcor);
void pdecu(int i1, int i2, double x0, double x1, double xmin, int int_,
           int *len);
void r4sort(int n, float rkey[], int iptr[]);
void spfit(int jb, int int_);
void tabin(char *modnam, int modnam_s);
void tauint(double ptk, double ptj, double pti, double zj, double zi,
            double *tau, double *x);
void tauspl(int i1, int i2, double pt[], double coef[][5]);
void trtm(double delta, int max_, int *n, float tt[], float dtdd[],
          float dtdh[], float dddp[], char *phnm, int phnm_s);
...
void phxpnd(int *n, char list[][PHASE_NAME_LENGTH], int nmax);
...
void ttint(float darray[], float tarray[], int npts, float *dvint, float *tvint,
           int *nerr);
void velocityadj(double velocity, double bdist, double dist, float *atime,
                 int *nerr);
void vmcalc(int jvm, int *nerr);
void vmdly(int *nerr);
void vmline(double dstmn, double dstmx, int *nerr);
