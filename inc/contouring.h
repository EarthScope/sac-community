...
void alloclabels(int maxsegments, int maxlabels, int *indexseglabelst,
                 ...
                 int *indexlabelpoint, int *indexlabeltype,
                 int *indexlabelangle, int *indexlabeltext, int *nerr);
void allocpoints(int maxpoints, int *indexpoints, int *indexlinks,
                 int *indexrlinks, int *indexaction, int *nerr);
void allocsegments(int maxsegments, int *indexlevels, int *indexstarts,
                   int *indexstops, int *nerr);
void calccontlabel1(void);
void calccontlabel4(void);
void calccontlabels(void);
void calccontrlinks(void);
void calccontsegs(float array[], int nxsize, int nysize, int *nerr);
void calccontticks(void);
void calclastpoint(int jstop, double skiplength, int *jlast);
void calcsegangle(float point1[], float point2[], float *angle);
void fastcontdata(float array[], int nxsize, int nysize, int *nerr);
void getcontlabel(int number, int *jpoint, int *jtype, float *angle,
                  int *jtext);
void getcontpoint(int number, float point[], int *link, int *action);
void getcontrlink(int number, int *rlink);
void getcontseg(int number, int *level, int *start, int *stop);
void getcontseglabel(int number, int *status, int *numlocs, int *firstloc);
void initcontattr(void);
void linkcontsegs(int level, float point1[], float point2[]);
void listcontsegs(void);
void markcontlabel(int index, int ilabel, double width, float *angle);
void mergecontsegs(void);
void newcontlabel(int jpoint, int jtype, double angle, int jtext, int *number);
void newcontpoint(float point[], int link, int action, int *number);
void newcontseg(int level, int start, int stop, int *number);
int nextcontseg(int *number, int *level, int *start, int *stop);
void plotcontdata(float zdata[], int nxsize, int nysize, int *nerr);
void plotcontsegs(void);
int pointsequal(float point1[], float point2[]);
void putcontlabel(int number, int jpoint, int jtype, double angle, int jtext);
void putcontpoint(int number, float point[], int link, int action);
void putcontrlink(int number, int rlink);
void putcontseg(int number, int level, int start, int stop);
void putcontseglabel(int number, int status, int numlocs, int firstloc);
void releaselabels(void);
void releasepoints(void);
void releasesegments(void);
void setcontdatalim(int ixstart, int ixstop, int iystart, int iystop);
