...
settextfont(int ifont) {
    ...
    jfont = ((ifont - 1) % 8) + 1;
    if (jfont == cmgdm.nfont ||
        ((jfont - 1) == cmgdm.nfont && (jfont % 2) == 0))
        goto L_8888;
    ...
    if (fgetsp(kiline, MCMSG, nun) == NULL)
        goto L_9100;
    if (sscanf
        (kiline, "%3d%3d%3d%3d%5d", &cmgdm.iofset, &cmgdm.ispace, &cmgdm.iyoff,
         &cmgdm.iyhigh, &cmgdm.maxstr) != 5) {
    ...
    for (i = 1; i <= 121; i += 8) {
        ...
        for (j = i; j <= (i + 7); j++) {
            ...
            if (sscanf
                (fontbuf, "%4hd%3hd%3hd", &Ascstr[j], &Stxmin[j],
                 &Stxmax[j]) != 3) {
