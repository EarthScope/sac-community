...
getepoch(char *kfunction, int *nc, int ic, char *kvalue, int *nerr)
{
    ...
    *nerr = 0;
    ...
    for (pBuff = buff; *pBuff == ' '; pBuff++) {        /* do nothing */
    }
...
double
hrToEpoch(int year, int month, int day, int hour, int min, float second,
          int *nerr) {
...
void
doyToMD(int d, int lp, int *pm, int *pd) {
    int i;
    int *et = eomnth[lp];
    for (i = 1; d > et[i]; i++);
    *pm = i;
...
int
MDtoDoy(int month, int day, int leap) {
    int *et = eomnth[leap];
...
int
mdyToJul(int mm, int id, int iyyy) {
    int jul;
    int ja, jy = iyyy, jm;
    int IGREG = 588829;
