...
macroline(char *kmacroline, int kmacroline_s, int *ncmacroline, int *nerr)
{
    ...
    int ic, ic1, ic2, itype, nc, niline, ntemp1, ntemp2, nchars, numsave;
    FILE *nrfun;
    FILE *nun;
    char *cattemp;
    char *s1, *s2;
  ...
  L_1000:
    if (nun != MUNINP) {
        ...
        getvvstring(kmcpf.kvarsname, 9, "prompt", 7, &nchars, ktemp1, MCPFN + 1,
                    nerr);
        if (*nerr != 0)
            goto L_8888;
    ...
    if (kiline[ic1 - 1] == '$') {
        ...
        if (memcmp(ktoken, "TERMINAL", 8) == 0) {
            ...
            fstrncpy(ktemp1 + 4 + min(ntemp2, MCPFN),
                     MCPFN - 4 - min(ntemp2, MCPFN), ")> $", 4);
            ntemp1 = indexb(ktemp1, MCPFN + 1);
            putvvstring(kmcpf.kvarsname, 9, "prompt", 7, ntemp1, ktemp1,
                        MCPFN + 1, nerr);
            if (*nerr != 0)
                goto L_8888;
            if (ic < niline) {
        ...
        /* -- If "RUN", initialize runfile. */
            ...
            zrunname(s1, ic2 - ic1 + 2, s2, niline - ic + 2, &nrfun, runfile,
                     MCPFN + 1, nerr);
