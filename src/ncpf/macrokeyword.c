             ...
             int keys_s, int *nerr)
{
    ...
    int lfirst;
    int ic1, ic2, ick1, ick2, iktype, itype, jkeys, jline, nc, nkey, nkeys,
        nline, numsave;
    char *s1, *p;
    Token *t;
    Token *list, *last;
    ...
    *nerr = 0;
    ...
    putvvstring(kmcpf.kvarsname, 9, "keys", 5, nkeys, keys, keys_s, nerr);
  ...
  L_1000:
    ...
    if (itype > 0) {
        ...
        p = upcase_dup(s1);
        putvvstring(kmcpf.kvarsname, 9, p, nkey + 1, MCPW, kmcpf.knoval, 9,
                    nerr);
        free(s1);
        free(p);
        if (*nerr != 0)
