...
void
clamp_lat_lon(int meridian, float *minlon, float *maxlon, float *minlat,
              float *maxlat) {
...
void
plot_legend_symbols(FILE * fp, char *symbol, float x, float y, float min,
                    float mean, float max, char *output) {
    fprintf(fp,
            "psxy -K -O -JX -R -Xa%.1fi -Ya%.1fi %s -N  " " <<" __EOF__
            " >> %s \n" "%.3f %.3f %.3f\n" "%.3f %.3f %.3f\n" "%.3f %.3f %.3f\n"
            __EOF__ "\n\n", x, y, symbol, output, 0.25, 0.25, min, 0.25, 0.50,
            mean, 0.25, 0.75, max);
}

void
plot_legend_text(FILE * fp, float x, float y, float min, float mean, float max,
                 char *output) {
    fprintf(fp,
            "pstext -K -O -JX -R -N -Xa%.1fi -Ya%.1fi " " <<" __EOF__
            " >> %s \n" "%.3f %.3f 14 0 5 5 %.2f \n"
            "%.3f %.3f 14 0 5 5 %.2f \n" "%.3f %.3f 14 0 5 5 %.2f \n" __EOF__
            "\n\n", x, y, output, 0.50, 0.25, min, 0.50, 0.50, mean, 0.50, 0.75,
            max);
}
...
/* -- Identify Projection: */
        ...
        /* eventfile reads named file for additional events; maf 960620 */
                 ...
                 ("EV#ENTFILE$", 12, MCPFN, eventFileName, MCPFN,
                  &eventFileNameLength)) {
            leventfile = TRUE;
        ...
        /* -- SCALE min max:  
           define min and max sizes of marks in gmap, maf 960702 */
                 ...
                 ("SC#ALED$", 9, -100.0, 100.0, &minSizeInput, &maxSizeInput)) {
            lscale = TRUE;
        }
    ...
    if (lsize || lresid) {
        for (i = 0; i < nch; i++) {
            if (lsize) {
                ...
                maxsize =
                    lscale ? fmax(minSizeInput, maxSizeInput) : fmax(maxsize,
                                                                     fabs(size
                                                                          [i]));
                ...
                if (size[i] < 0) {
                    ...
                    maxneg =
                        lscale ? fabs(minSizeInput) : fmax(maxneg,
                                                           fabs(size[i]));
                } else {
                    npos += 1;
                    minpos = fmin(minpos, size[i]);
                    maxpos =
                        lscale ? fabs(maxSizeInput) : fmax(maxpos, size[i]);
                }
            }
        }
    }
...
/* compute area for min resolution in pscoast */
    if (mapJ == MERCATOR || mapJ == EQUIDISTANT || mapJ == LAMBERT ||
        mapJ == UTM) {
    ...
    if ((int) (lat_annot_spac * 100) <= 1) {
        ...
        sprintf(clengthscale, "%1.3f",
                (float) ((int) (lat_annot_spac * 100.0)));
    }
    ...
    /* Lambert Projection */
        ...
        sprintf(cprojection, "-JL%.3f/%.3f/%.3f/%.3f/%.1fi",
                ...
                (minlat + maxlat) / 4.0, 3.0 * (minlat + maxlat) / 4.0, width);
    }
    /* UTM Projection */
    else if (mapJ == UTM) {
        /* Determine the utm zone as (long) centerlat/6.0 */
        sprintf(cprojection, "-JU%d/%.1f", (int) ((minlat + maxlat) / 12.0),
                width);
    }

    /* Map Bounds */
    sprintf(cbounds, "-R%.3f/%.3f/%.3f/%.3f", minlon, maxlon, minlat, maxlat);
    ...
    fprintf(fp,
            "psbasemap %s %s -Ba%.3ff%.3f/a%.3ff%.3fNEWS:.\"%s\": "
            " -X0.75i -Y1.0i -P -K > " PSVAR " \n\n", cprojection, cbounds,
            lon_annot_spac, lon_tick_spac, lat_annot_spac, lat_tick_spac,
            ctitle);
    ...
    if (*nerr == 0) {
        ...
        memset(psxy, ' ', MAXSTRING);
        sprintf(psxy,
                "psxy -K -O -J -R " "-SV0.025/0.2/0.2 -G125 <<" __EOF__ " >> "
                PSVAR " \n" "%.3f %.3f %s 1 \n" __EOF__ "\n\n",
        ...
        sprintf(psxy,
                ...
                "%.3f %.3f \n" __EOF__ "\n\n", stalon[irefsta - 1],
                stalat[irefsta - 1], evlon[irefsta - 1], evlat[irefsta - 1]);
    ...
    if (neventlocs > 0) {
        ...
        if (lsize) {
            sprintf(psxy,
                    "psxy -O -K -J -R " "-Sc -W10/0/0/0 <<" __EOF__ " >> " PSVAR
                    " \n");
        } else if (lresid) {
            sprintf(psxy,
                    "psxy -O -K -J -R " "-Sx -W10/255/0/0 <<" __EOF__ " >> "
                    PSVAR " \n");
        } else {
        ...
        if (plotlegend) {
            if (lsize) {
                ...
                plot_legend_symbols(fp, "-Sc -W10/0/0/0 ", legendx, legendy,
                                    minscale, (minscale + maxscale) / 2,
                                    maxscale, PSVAR);
                plot_legend_text(fp, legendx, legendy, minsize,
                                 (minsize + maxsize) / 2, maxsize, PSVAR);
            } else if (lresid) {
                ...
                if (nneg > 0) {
                    ...
                    plot_legend_symbols(fp, "-Sc -W10/0/0/0 ", legendx, legendy,
                                        ...
                                        slope * ((minneg + maxneg) / 2) +
                                        intercept, slope * maxneg + intercept,
                                        PSVAR);
                    plot_legend_text(fp, legendx, legendy, -minneg,
                                     (-minneg - maxneg) / 2, -maxneg, PSVAR);
                }
                ...
                if (npos > 0) {
                    ...
                    plot_legend_box(fp, legendx, legendy, PSVAR);
                    plot_legend_symbols(fp, "-Sx -W10/255/0/0 ", legendx,
                                        legendy, slope * minpos + intercept,
                                        slope * ((minpos + maxpos) / 2) +
                                        intercept, slope * maxpos + intercept,
                                        PSVAR);
                    plot_legend_text(fp, legendx, legendy, minpos,
                                     (minpos + maxpos) / 2, maxpos, PSVAR);
                }
            }
        }
    }
    ...
    if ((viewer = getenv("SACPSVIEWER")) != NULL) {
        ...
        /* Use the default. */
        strcpy(syscommand,
               (use_tty())? SACPSVIEWER_DEFAULT : SACPSVIEWER_DEFAULT_NO_TTY);
        fprintf(stderr,
                "Using Default Postscript Viewer\n" "\t%s\n"
                "\tSet an alternative through the SACPSVIEWER "
                "environment variable\n" "\t%s\n",
                (use_tty())? SACPSVIEWER_DEFAULT : SACPSVIEWER_DEFAULT_NO_TTY,
                (use_tty())? "Press any key to continue" :
