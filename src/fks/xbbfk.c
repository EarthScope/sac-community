...
xbbfk(int *nerr) {
    ...
    int idx, iasiz[2], isacm, issq, ix, iy, jdx, jdfl, nch, nckofbbfk, ncktitle,
        ns, nsiz, nssav, wvaz;
    float buffer[MXLENB], curntval, offset, trace, xr[MXLENS], yr[MXLENS],
        zr[MXLENS];
    ...
    while (lcmore(nerr)) {
        if (lklog("F#ILTER$", 9, &cmfks.lkfilter)) {    /* do nothing */
        }

        else if (lklog("NO#RMALIZE$", 12, &cmfks.lknorm)) {     /* do nothing */
        }
        ...
        else if (lklogi("EX#P$", 6, &cmfks.lexp, &cmfks.iexp)) {        /* do nothing */
        }
            ...
            if ((((cmfks.iazs % 2) != 0) || ((cmfks.iwvs % 2) != 0)) ||
                ...
                fprintf(MUNOUT,
                        "ERR: For SIZE, you must give 2 even nums whose product is < %d \n",
                        MXLENB);
                goto L_8888;
            }
        }

        else if (lkint("LE#VELS$", 9, &cmfks.nlcontour)) {      /* do nothing */
        }
        ...
        else if (lklogc("WR#ITE$", 8, &cmfks.lwr, cmfks.kofbbfk, MCPFN + 1)) {  /* do nothing */
        }

        else if (lkint("SS#Q$", 6, &issq)) {
            lssq_specified = TRUE;
            if (((cmfks.idimsq % 2) != 0) ||
                cmfks.idimsq * cmfks.idimsq > MXLENB) {
                fprintf(MUNOUT,
                        "ERR: For WRITE, num of samples squared must be < %d \n",
                        MXLENB);
                goto L_8888;
            }
        }
            ...
            for (jdx = 1; jdx <= ns; jdx++) {
                Scm[idx + (jdx - 1) * ns] =
                    cmplxmul(Scm[idx + (jdx - 1) * ns],
                             flttocmplx(s->h->scale, 0.));
                Scm[jdx + (idx - 1) * ns] =
                    cmplxmul(Scm[jdx + (idx - 1) * ns],
                             flttocmplx(s->h->scale, 0.));
            }
        }
    }
    ...
    if (cmfks.leps) {
        ...
        for (idx = 1; idx <= ns; idx++) {
            Scm[idx + (idx - 1) * ns] =
                cmplxadd(Scm[idx + (idx - 1) * ns], flttocmplx(offset, 0.));
        }
    }
    ...
    if (strcmp(cmfks.kfkmethod, "MLM ") == 0) {
        if (cmfks.neigenmlm == 0) {
            ...
            eigenanal(ns, ns, scm, cmfks.kfkmethod, cmfks.neigenmlm, sinv,
                      nerr);
        }
     ...
     * */
        ...
        if (cmfks.neigenmus == 0) {
            ...
            eigenanal(ns, ns, scm, cmfks.kfkmethod, cmfks.neigenmus, sinv,
                      nerr);
        }
        for (idx = 1; idx <= (ipow(ns, 2)); idx++) {
            Scm[idx] = Sinv[idx];
    ...
    fkevalp(scm, xr, yr, ns, cmfks.iazs, cmfks.rkhor, cmfks.iwvs, buffer,
            cmfks.fkmax, cmfks.fkmax_azimuth, cmfks.fkmax_wavenum, maxflag);
...
void
covmat(int nch, int nsamples, int ltofilter, complexf * scm, int *nerr) {

    int decrate, idx, index1, index2, iptr, jdx, jptr, jptr0, kdx, mcount,
        ncurrent, ns2;
    float a[MXNSECTS], buffer[100], g, output1[MXLENS][100],
        output2[MXLENS][100], qstates[MXLENS][MXLENS - 1], sdd[2 * MXNSECTS],
        snn[2 * MXNSECTS], states[MXLENS][2 * MXNSECTS], xiyi, xiyr, xryi, xryr;
    ...
    if (ltofilter) {
        ...
        if (cmfir3.nsects == 0 ||
            (strcmp(cmfir3.filttype, "bp") != 0 &&
             strcmp(cmfir3.filttype, "BP") != 0)) {
            fprintf(MUNOUT,
                    "You have not run FILTERDESIGN or filter type is not BP.\n");
            *nerr = 9999;
            goto L_9999;
        }
  ...
  L_4:
    ...
    for (jdx = 0; jdx < nch; jdx++) {
        ...
        if (ltofilter) {
            ...
            /*    bandpas    */
            iirfilter(s->y + iptr - 1, ncurrent, a, snn, sdd, cmfir3.nsects,
                      &states[jdx][0], buffer);
        ...
        if (mcount == decrate) {
            ...
            for (jdx = 0; jdx < nch; jdx++) {
                ...
                for (kdx = 0; kdx < jdx + 1; kdx++) {
                    ...
                    Scm[index1] =
                        cmplxadd(Scm[index1],
                                 flttocmplx(xryr + xiyi, xiyr - xryi));
...
void
fkevalp(complexf * scm, float *x, float *y, int nch, int naz, double wvnum,
        int iwvs, float *fks, float fkmax, float fkmax_az, float fkmax_wav,
        int maxflag) {

    int half, idx, ic, id, ifks, jdx, mdx, ndx, offset;
    float azimuth, caz, dec[LENRXY], deltaz, deltwv, des[LENRXY], doti, doti2,
        dotr, dotr2, ec[LENRXY], es[LENRXY], pi, ri[LENRXY], ri2[LENRXY],
        rr[LENRXY], rr2[LENRXY], s0, saz, t, theta, twopi, zx[LENRXY],
        zy[LENRXY];
...
void
fkevalr(complexf * scm, float *x, float *y, int nch, int nsamps, double wvnum,
        float *fks) {

    int halfns, idx, ic, id, ifks, ifksr, jdx, mdx, maxfks, ndx;
    float dec[LENRXY], deltwv, des[LENRXY], doti, dotr, ec[LENRXY], es[LENRXY],
        fks0, ri[LENRXY], rr[LENRXY], t, theta, twopi, wv, zx[LENRXY],
        zy[LENRXY];
...
#define S(I_,J_)	(*(s+(I_)*(nm)+(J_)))
    ...
    if (memcmp(hires, "MLM", 3) == 0 || memcmp(hires, "mlm", 3) == 0) {
        ...
        fprintf(MUNOUT,
                "   BBFK computed with eigen terms [vectors & values]\n");
        fprintf(MUNOUT, "   that have the HIGHEST%d  eigenvalues.\n", m);
        fputc('\n', MUNOUT);
        fprintf(MUNOUT,
                "(*)used  ascend(MUS)  descend(MLM)          eigenvalue\n");
