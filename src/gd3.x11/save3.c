...
png_write(display_t * out, char *file) {
    ...
    img =
        XGetImage(DISPLAY(xw), xw->buffer, 0, 0, xw->width, xw->height,
                  AllPlanes, ZPixmap);
    ...
    for (i = 0; i < img->height; i++) {
        image[i] =
            (png_byte *) malloc(sizeof(png_byte) * RGB_COLORS *
                                PNG_SAMPLE_DEPTH * img->width);
    ...
    png_ptr =
        png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL, user_error_fn,
                                user_warning_fn);
    if (!png_ptr) {
    ...
    png_set_IHDR(png_ptr, info_ptr, img->width, img->height, 16,
