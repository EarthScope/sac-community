...
xwindow_compress_event(XWindow * xw, XEvent * event) {
    while (XCheckTypedWindowEvent(DISPLAY(xw), xw->win, event->type, event) ==
           True) {
    }
}
            ...
            case ButtonRelease:
                {
                    ...
                    if (is_plotw) {
                        ...
                        if ((start_x != e->x || start_y != e->y) &&
                            ...
                            ycursor_p3 = e->y;
                            cursor_on3 = FALSE;
                        } else if (button_state == TRUE && start_x == e->x &&
                                   start_y == e->y) {
                            char_cursor3[0] = 'L';
                            xcursor_p3 = e->x;
                            ycursor_p3 = e->y;
            ...
            case MotionNotify:
                {
                    ...
                    if (button_state ||
                        ((cursor_on3 || cursortext_on3) &&
                         use_massive_crosshairs)) {
                        xwindow_expose(plot, plot->expose_data);
                    }
                    if (button_state) {
                        GC igc;
                        ...
                        igc =
                            XCreateGC(DISPLAY(plot), plot->win, GCFunction,
                                      &val);
                        if (start_x <= e->x) {
                            XFillRectangle(DISPLAY(plot), xw->win, igc, start_x,
                                           0, e->x - start_x, xw->height);
                        } else {
                            XFillRectangle(DISPLAY(plot), xw->win, igc, e->x, 0,
                                           start_x - e->x, xw->height);
                        }
