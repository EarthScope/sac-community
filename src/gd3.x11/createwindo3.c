...
xwindow_init(XScreen * xs, XWindow * xw, XWindow * parent, int x, int y,
    ...
    valuemask = CWBackPixel | CWOverrideRedirect | CWBitGravity;

    xw->win =
        XCreateWindow(DISPLAY(xw), (parent) ? parent->win : ROOT(xw), x, y,
                      width, height, xw->border, CopyFromParent, InputOutput,
...
xwindow_fill_background(XWindow * xw) {
    ...
    XFillRectangle(DISPLAY(xw), xw->buffer, xw->gc, 0, 0, xw->width,
                   xw->height);
...
xwindow_double_buffer_new(XWindow * xw) {
    if (xw->use_buffer) {
        ...
        xw->buffer =
            XCreatePixmap(DISPLAY(xw), xw->win, xw->width, xw->height,
                          DEPTH(xw));
...
xwindow_expose(XWindow * xw, void *data) {
    ...
    if (xw->use_buffer) {
        if (xw->buffer) {
            XCopyArea(DISPLAY(xw), xw->buffer, xw->win, xw->gc, 0, 0, xw->width,
                      xw->height, 0, 0);
        }
    } else {
        xw->draw(xw, xw->draw_data);
...
void
createwindow3(int *win_num, float *xmin_vp, float *xmax_vp, float *ymin_vp,
    ...
    *nerr = 0;
    ...
    height_p =
        (unsigned int) ((float) (xs->height - 1) * (*ymax_vp - *ymin_vp));
    ...
    if (!xwindow_init
        (xs, base, NULL, xul_p, yul_p, width_p,
         height_p + title->height + base->border)) {
        *nerr = 1;
        return;
    }
    ...
    /* Icon */
    SacPixmap =
        XCreateBitmapFromData(xs->display, base->win, (const char *) sac_bits,
                              sac_width, sac_height);
    ...
    *fu = *win_num;
    ...
    cursorgc3[*win_num] =
        XCreateGC(xs->display, plot->win, GCFunction, &xgccurs);
