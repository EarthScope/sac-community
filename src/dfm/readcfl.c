 ...
 */
void
readcfl(int lmore, char *kdirin, int kdirin_s, string_list * list, int Verbose,
        int isASCII, float MaxMem, int *nerr) {
    ...
    *nerr = 0;
    ...
    if (kmdfm.lorient) {
        ...
        if (kmdfm.korient[0] != '*')
            Orientation =
                TokenLineToStrgList(tmpOrientString, &Norientation, 0);
    }
    ...
    if (isASCII) {
        ...
        for (i = 0; i < string_list_length(files_out); i++) {
            ...
            filesReturned +=
                cssReadFlatFiles(kwfdisc, WorkSetName, Replace, MaxTraces,
                                 Station, Nstation, Channel, Nchannel, Band,
                                 Nband, Code, NCode, Orientation, Norientation,
                                 Author, Nauthor, Filelist, Nfiles, Phaselist,
                                 Nphases, Verbose, MaxMem, &takeEvid);
        ...
        for (j = 0; j < string_list_length(files_out); j++) {
            ...
            filesReturned +=
                ReadCSSBfile(kwfdisc, WorkSetName, Replace, MaxTraces, Verbose,
                             MaxMem, takeEvid);
