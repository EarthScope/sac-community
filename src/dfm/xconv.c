...
xconv(int *nerr) {
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        else if (lclist((char *) kmdfm.krwfmt, 9, cmdfm.nrwfmt, &cmdfm.icfmt[iinout - 1])) {    /* do nothing */
        }
        ...
        /* -- "FMT string":  define a card-image format for in/out file. */
                        ...
                        , strlen((char *) kmdfm.kcfmt[iinout - 1]) + 1, &junk)) {       /* do nothing */
        }

        /* -- "filename":  define in/out filename. */
        else if (lcchar((char *) kmdfm.kcfile[iinout - 1], sizeof(kmdfm.kcfile[iinout - 1]))) { /* do nothing */
        }
