...
MatchingArrival(DBlist tree, char *sta, char *chan, double time, double end,
...
*/
struct arrivalList *
sacFindNextMatchingArrival(struct arrivalList *ar, DBlist tree, char *sta,
                           char *chan, double time, double end, int wfid) {
    ...
    do {
        result =
            (struct arrivalList *) dblNextTableInstance(result, tree,
                                                        dbl_LIST_ARRIVAL);
        if (!result)
            break;
...
struct arrivalList *
sacFindNextMatchingPick(struct arrivalList *ar, DBlist tree, int wfid) {
    struct arrivalList *result;
...
sacFindSiteChan(DBlist tree, char *sta, char *chan, int jdate, int wfid) {
    ...
    do {
        ...
        if (sc) {
            if (CSSchrDefined(sta) && CSSchrDefined(chan))
                if (!strcmp(sta, sc->element->sta) &&
                    !strcmp(chan, sc->element->chan))
...
sacFindSite(DBlist tree, char *sta, int jdate, int wfid) {
    ...
    if (CSSchrDefined(sta)) {
        do {
            si = (struct siteList *) dblNextTableInstance(si, tree,
                                                          dbl_LIST_SITE);
            DEBUG("%p [%p]\n", si, tree);
            if (!si)
                break;
            if (!strcmp(si->element->sta, sta))
                if (si->element->ondate <= jdate)
                    if (si->element->offdate >= jdate ||
                        si->element->offdate < 0)
                        return si;
        } while (si);
    } else {
...
sacFindMatchingOrigin(DBlist tree, int orid) {
    struct originList *orig = 0;
    do {
        orig =
            (struct originList *) dblNextTableInstance(orig, tree,
                                                       dbl_LIST_ORIGIN);
        if (orig && orig->element->orid == orid)
            return orig;
    } while (orig);
...
static struct originList *
sacFindOrigin(DBlist tree, struct wfdiscList *w) {
    struct originList *orig = 0;
...
sacFindAssoc(DBlist tree, int orid, char *sta) {
...
static struct assocList *
sacFindAssocFromArid(DBlist tree, int arid) {
    struct assocList *as = 0;
...
sacFindEvent(DBlist tree, int evid) {
    struct eventList *ev = 0;
...
sacFindWftag(DBlist tree, int wfid, char *tagname) {
    struct wftagList *wt = 0;
...
sacSetEvType(char *etype) {
    ...
    if (!strcmp(etype, "Unk"))
        return IUNKNOWN;
    if (!strcmp(etype, "PreN"))
        return IPREN;
    if (!strcmp(etype, "PostN"))
        return IPOSTN;
    if (!strcmp(etype, "ForeShk"))
        return IPREQ;
    if (!strcmp(etype, "AftrShk"))
        return IPOSTQ;
    if (!strcmp(etype, "ChemExp"))
        return ICHEM;
    if (!strcmp(etype, "Other"))
        return IOTHER;
...
void
SetMagnitudeSource(struct SACheader *h, struct originList *orig) {
    if (!CSSchrDefined(orig->element->auth)) {
        h->imagsrc = SAC_INT_UNDEFINED;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "UNKNOWN")) {
        h->imagsrc = IUNKNOWN;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "NEIC")) {
        h->imagsrc = INEIC;
        return;
    }
    if (!strncmp(CpUpstring(orig->element->auth), "PDE", 3)) {
        h->imagsrc = IPDE;
        return;
    }
    if (!strncmp(CpUpstring(orig->element->auth), "PDE-Q", 5)) {
        h->imagsrc = IPDEQ;
        return;
    }
    if (!strncmp(CpUpstring(orig->element->auth), "PDE-W", 5)) {
        h->imagsrc = IPDEW;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "ISC")) {
        h->imagsrc = IISC;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "REB")) {
        h->imagsrc = IREB;
        return;
    }
    if (!strncmp(CpUpstring(orig->element->auth), "USGS", 4)) {
        h->imagsrc = IUSGS;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "BRK")) {
        h->imagsrc = IBRK;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "CALTECH")) {
        h->imagsrc = ICALTECH;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "LLNL")) {
        h->imagsrc = ILLNL;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "EVLOC")) {
        h->imagsrc = IEVLOC;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "JSOP")) {
        h->imagsrc = IJSOP;
        return;
    }
    if (!strcmp(CpUpstring(orig->element->auth), "USER")) {
        h->imagsrc = IUSER;
        return;
    }
...
void
SetMagnitudeInHeader(struct SACheader *h, struct originList *orig,
                     MagType Mtype) {
...
SetDistAzValues(struct SACheader *header) {
    ...
    if (header->gcarc == SAC_FLOAT_UNDEFINED)
        header->gcarc = delt;
    if (header->baz == SAC_FLOAT_UNDEFINED)
        header->baz = bazim;
    if (header->az == SAC_FLOAT_UNDEFINED)
        header->az = azim;
    return;
}
...
void
sacHeaderFromCSS(DBlist tree, struct SACheader *header, struct wfdiscList *w,
...
/* Find an origin struct matched to w and copy its info to header. */
    if ((orig = sacFindOrigin(tree, w))) {
        ...
        if (CSSlngDefined(orig->element->grn))
            header->ievreg = orig->element->grn + REGCONV;
        if (CSSlngDefined(orig->element->orid))
            header->norid = orig->element->orid;
        ...
        if ((as = sacFindAssoc(tree, orig->element->orid, w->element->sta))) {
            ...
            if (CSSfltDefined(as->element->esaz))
                header->az = as->element->esaz;
        }
    ...
    if (w->seis->Cmplx) {
        ...
        for (i = 0; i < header->npts; i++) {
            ...
            if (header->depmin > *(w->seis->i + i))
                header->depmin = *(w->seis->i + i);
            if (header->depmax < *(w->seis->i + i))
                header->depmax = *(w->seis->i + i);
        }
        if (header->npts)
            header->depmen /= header->npts;
    }
    ...
    strcpy(SACfield, "A");
    arm =
        MatchingArrival(tree, w->element->sta, w->element->chan,
    ...
    for (i = 0; i < 10; i++) {
        ...
        if (!arm) {             /* If you can't find an arrival marked for Ti, take the next unmarked arrival. */
            ar = sacFindNextMatchingArrival(ar, tree, w->element->sta,
                                            w->element->chan, w->element->time,
                                            w->element->endtime,
                                            w->element->wfid);
            if (!ar)
                continue;
        ...
        if (aso &&
            ...
            if (toupper(arm->element->iphase[0]) == 'F')
                continue;
            char *kt = sac_header_kt_number(header, i);
            if (kt) {
