...
void
ValidateDay(int year, int month, int *day) {
    int maxDay;
    if (*day < 1) {
        *day = 1;
        warningLevel = ProbableTimeError;
...
void
ValidateDOY(int year, int *DayOfYear) {
    int maxDay;
    if (*DayOfYear < 1) {
        *DayOfYear = 1;
        warningLevel = ProbableTimeError;
...
void
caldat(int julian, int *mm, int *id, int *iyyy) {
...
double
tmMakeEpochTime(int year, int month, int day, int hour, int min, float second) {
    int err;
...
double
julianToEpoch(int jDate, int begin) {
...
/* Decode EpochTime */
int
tmDecodeEpochTime(double time, int *year, int *month, int *day, int *hour,
                  int *min, float *second) {
    int ElapsedDays;
...
/* Display EpochTime  */
char *
tmListEpochTime(double time, int form) {
    ...
    switch (form) {
        ...
        case 5:
            ...
            break;
        case 6:
            sprintf(outstr, "%s %d, %d %02d:%02d %06.3f GMT", mon[month], day,
    ...
    if (!token) {               /* No more tokens so use defaults */
        ...
        if (strlen(token) == 3) {       /* must be day of year */
            ...
            if (token == (char *) NULL) {       /* No more tokens so use defaults */
                EpochTime =
                    tmMakeEpochTime(year, month, day, hour, min, second);
                return EpochTime;
            } else {
                if (NotNumericString(token))
                    return EpochTime;
