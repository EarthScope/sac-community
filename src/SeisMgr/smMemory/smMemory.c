...
static void
AddToMemList(void *ptr, size_t size) {
...
void *
smCalloc(size_t count, size_t size) {
...
void *
smRealloc(void *ptr, size_t size) {
    DropFromMemList(ptr);
...
size_t
smTotalPhysicalBytes(void) {
#ifdef SOLARIS
    if (!TotalSystemPages)
        TotalSystemPages = sysconf(_SC_PHYS_PAGES);
    if (!PageSize)
