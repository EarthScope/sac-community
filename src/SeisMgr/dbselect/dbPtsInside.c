...
static int CheckVertex(struct point *p, struct line lp, struct line lt,
...
*/
static int
CheckVertex(struct point *p, struct line lp, struct line lt, int count, int *i,
            int N) {
    struct line lp2;
    ...
    if (lt.p1.y > lp.p1.y && lt.p1.y < lp2.p2.y)
        return (count + 1);
    if (lt.p1.y < lp.p1.y && lt.p1.y > lp2.p2.y)
        return (count + 1);
}
...
*/
int
inside(struct point t, struct point *p, int N) {
    int i;
    int count = 0;
    int j = 0;
    ...
    if (i == N)
        return count % 2;       /* intersection was on last segment of polygon */
    lp.p1 = lp.p2;
    lp.p2 = p[0];
    if (intersect(lp, lt))
        count = CheckVertex(p, lp, lt, count, &i, N);
