...
float
GetSrcCircleLatC(void) {
    return SrcCircleLatC;
}

float
GetSrcCircleLonC(void) {
    return SrcCircleLonC;
}

float
GetSrcCircleRadius(void) {
    return SrcCircleRadius;
}
...
int
dbRestrictionApplied(enum Restriction Restrict) {
    switch (Restrict) {
        case Origin:
...
static char *
BuildWfidRestriction(int index) {
    int j;
    char *string;
    int stringLen;
    char Tmp[20];
...
static void
SetDistanceLimits(int index, const char *type) {
    float tmp;
...
static void
SetQuadrantLimits(int index) {
    float tmp;

    if (numParams[index] != 2) {
        sprintf(dbErrorString, "ERROR: Quadrant option requires 2 values. \n");
...
int
dbDistRestrictionApplied(void) {
    return DistRestrictionApplied;
}
...
static char *
FormatQueryString(char *QueryString) {
    int LineLen = 80;
    char *fromPos;
    char *wherePos;
    char *andPos;
    ...
    *(fromPos - 1) = '\n';
    ...
    for (j = 0; j < selectClauseLen; j++) {
        if (QueryString[j] == ',') {
            ...
            commaPos = j;
            if (placeInLine < oldPlaceInLine)
                QueryString[oldCommaPos + 1] = '\n';
...
char *
AddToSelectString(char *SelectString, char *string) {
...
char *
AddToFromString(char *FromString, char *string) {
...
static char *
AddToWhereString(char *WhereString, char *string) {
    int len1, len2;
    len2 = strlen(string);
...
*/
static char *
GetBoxExtrema(int index, char *TableDot, char *latlon) {
    char minC[10];
    char maxC[10];
    float min, max, temp;
    char *string;
    ...
    if (!strcmp(latlon, "LAT")) {
        for (j = 0; j < numParams[index]; j += 2) {
            if (floatParams[index][j] < min)
                min = floatParams[index][j];
            if (floatParams[index][j] > max)
                max = floatParams[index][j];
        }
    } else {
    ...
    string =
        stringcat(TableDot, latlon, " >= ", minC, " AND ", TableDot, latlon,
                  " <= ", maxC, " ", 0);
    return string;
}
...
static char *
GetSrcCircleBoxExtrema(int index) {
    char minC1[10];
    char maxC1[10];
    char minC2[10];
    char maxC2[10];
    ...
    string =
        stringcat("O.LAT >= ", minC1, " AND ", "O.LAT <= ", maxC1, " ", " AND ",
                  "O.LON >= ", minC2, " ", " AND ", "O.LON <= ", maxC2, " ", 0);
...
GetSrcDistBoxExtrema(void) {
    ...
    if (!strcmp(DistanceType, "DIST"))
        dbGetEnclosingBox(latc, lonc, MaxDistValue, &minLat, &maxLat, &minLon,
                          &maxLon);
    else {
...
static char *
BuildMinMaxRestrict(int index, char *TableDot, char *option, char *latlon) {
    char minC[10];
    char maxC[10];
    float min, max, temp;
    char *string;
    ...
    string =
        stringcat(TableDot, latlon, " BETWEEN ", minC, " AND ", maxC, " ", 0);
    return string;
}
...
char *
dbBuildSQLstring(void) {
    char *tmp;
    int j, k;
    char *p;
    char baseString[] =
        "SELECT DISTINCT W.STA, W.CHAN, W.TIME, W.WFID, W.CHANID, "
    ...
    char originWhereClause[] =
        " UPPER(WT.TAGNAME) = 'EVID' AND E.EVID = WT.TAGID AND E.PREFOR = O.ORID ";

    char siteFields[] = ", S.LAT SLAT, S.LON SLON, S.ELEV SELEV ";
    char siteFromClause[100] = "";
    char siteWhereClause[] =
        ", W.STA = S.STA \nAND W.JDATE BETWEEN S.ONDATE AND S.OFFDATE ";

    char siteChanFields[] = ", SC.EDEPTH SCEDEPTH ";
    char siteChanFromClause[100] = "";
    char siteChanWhereClause[] =
        ", SC.STA = W.STA AND SC.CHAN = W.CHAN\nAND W.JDATE "
        "BETWEEN SC.ONDATE AND SC.OFFDATE ";

    char EvidRestriction[] =
        " WT.TAGNAME = 'evid' AND W.WFID = WT.WFID AND "
    ...
    sprintf(originFromClause, ", %s O , %s E ", dbGetOriginTableName(),
            dbGetEventTableName());
    ...
    for (j = 0; j < NUMOPTIONS; j++) {
        ...
        if (optionSelected[j] && !strcmp(options[j], "INSTDEPTH")) {
            if (!useSiteChanRestrict) {
                ...
                WhereString =
                    AddToWhereString(WhereString, siteChanWhereClause);
            }
        ...
        if (optionSelected[j] && !strcmp(options[j], "WFID")) {
            WhereString =
                AddToWhereString(WhereString, BuildWfidRestriction(j));
        }
    ...
    if (!TablesAvailable
        (useOriginRestrict, useSiteRestrict, useSiteChanRestrict, 0)) {
        free(QueryString);
        QueryString = 0;
    }
