...
void
AddAlias(char *newKey, char *origKey) {
    int j;
    int KeyOK;
    alias *newA, *curA;
    ...
    for (j = 0; j < NUMOPTIONS; j++)
        if (!strcmp(newKey, options[j])) {
            printf
                ("Alias (%s) is the name of a built-in option and cannot be used.\n",
                 newKey);
            return;
        }
    KeyOK = 0;
    ...
    if (!KeyOK) {
        printf
            ("(%s) cannot be aliased to (%s) because (%s) is not a valid option.\n",
...
*/
static void
AddValsFromFile(char *token, int tokenID, enum DATATYPE dtype) {
    ...
    if (dtype == REAL) {
        while (fscanf(paramFile, "%f %f", &lat, &lon) == 2) {
            numParams[tokenID] += 2;
            floatParams[tokenID] =
                (float *) realloc(floatParams[tokenID],
        ...
        while (fgetsp(lineBuffer, MAXLINELENGTH, paramFile) != NULL) {
            ...
            numParams[tokenID]++;
            charParams[tokenID] =
                (char **) realloc(charParams[tokenID],
...
*/
static int
ParseToReal(char *token, int tokenID) {
    ...
    for (j = 0; j < strlen(token); j++) {
        if (isalpha(token[j])) {
            ...
            dbPrintToDevice(errString);
            sprintf(errString,
                    "   You may have misspelled an option name.\n\n");
            dbPrintToDevice(errString);
...
*/
static void
ParseToString(char *token, int tokenID) {
...
/* Function to display a help screen for using the dbselect command */
static void
DispDBselectHelp(char *option, char *subOption) {
...
static void
ShowOracleDefaults(void) {
    char string[200];
    if (!dbGetQueryMaxRows())
        dbSetQueryDefaults();
...
*/
static int
TokenInOptionList(char *token, int *tokenID) {
...
*/
static int
SetOptionParams(int tokenID, int *nextTokenID) {
                    ...
                    if (!strchr(token, '=')) {
                        if (!ParseToReal(token, tokenID)) {
                            printf("Error getting values for option: %s.\n",
                                   options[tokenID]);
                            printf
                                ("   The token (%s) is not a valid number. \n",
                                 token);
                            printf
                                ("   You may have misspelled an option name.\n\n");
                        }
...
char *
LoadCommandsFromFile(char *filename) {
    FILE *fptr;
    char *cmd;
    int j, nbytes = 0;
    int c;
