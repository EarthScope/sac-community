...
void
dbSetQueryLoginString(const char *string) {
    char *charptr;
...
void
dbSetQueryPhaselist(char *string) {
    if (!string)
        return;
    if (strlen(string) < 1)
        return;
...
void
dbSetQueryAuthlist(char *string) {
    if (!string)
        return;
    if (strlen(string) < 1)
        return;
...
void
dbSetPrependToPath(char *string) {
    if (!string)
        return;
    if (strlen(string) < 1)
        return;
...
static char *
FullyQualified(const char *TableName, char *fullname) {
...
void
dbSetCaseSensitivity(char *string) {
...
void
dbSetWfdiscTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetWftagTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetOriginTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetEventTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetArrivalTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetAssocTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetSiteTableName(char *string) {
...
void
dbSetSitechanTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetSearchlinkTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetInstrumentTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbSetSensorTableName(char *string) {
    if (!string || !strlen(string))
        return;
...
void
dbUpdateDefaultsFromFile(char *ConFigFileName) {
    FILE *fptr;
    char buffer[100];
    char *c;
    char *keyword;
    ...
    if (!(fptr = fopen(ConFigFileName, "rb")))
        return;
    while (fgetsp(buffer, 99, fptr)) {
        ...
        if (!strcmp(Upstring(keyword), "CASESENSITIVITY")) {
            if (!strcmp(Upstring(value), "ON"))
                ...
                dbPrintToDevice
                    ("WARNING: Bad value for CaseSensitivity in config file.\n");
        }
