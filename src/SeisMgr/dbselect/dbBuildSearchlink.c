...
float
GetSrcCircleLatCSL(void) {
    return SrcCircleLatC;
}

float
GetSrcCircleLonCSL(void) {
    return SrcCircleLonC;
}

float
GetSrcCircleRadiusSL(void) {
...
int
dbRestrictionAppliedSL(enum RestrictionSL Restrict) {
    switch (Restrict) {
        case SrcPolySL:
            return SrcBox;
            break;
...
static char *
BuildWfidRestrictionSL(int index) {
    int j;
    char *string;
    int stringLen;
    char Tmp[20];
    ...
    if (numParams[index] < 1) {
        ...
        strcat(dbErrorString,
               "In: BuildWfidRestrictionSL in dbBuildSearchlink");
        dblSetError(1, dbErrorString);
    }
...
static void
SetEdepthLimits(int index) {
    float tmp;

    if (numParams[index] != 2) {
        sprintf(dbErrorString, "ERROR: instdepth option requires 2 values. \n");
...
char *
dbDistanceTypesSL(void) {
    return DistanceType;
}

static void
SetSQLONOFFSL(int index) {
    int state;
    if (numParams[index] > 1) {
        sprintf(dbErrorString,
...
*/
static char *
GetBoxExtremaSL(int index, char *latlon) {
    char minC[10];
    char maxC[10];
    float min, max, temp;
    char *string;
    ...
    string =
        stringcat(latlon, " >= ", minC, " AND ", latlon, " <= ", maxC, " ", 0);
    return string;
}
...
GetSrcCircleBoxExtrema(int index) {
    ...
    if (numParams[index] < 3) {
        ...
        strcat(dbErrorString,
               "In: GetSrcCircleBoxExtrema in dbBuildSearchlink");
        dblSetError(1, dbErrorString);
    }
    ...
    string =
        stringcat("SL.OLAT >= ", minC1, " AND ", "SL.OLAT <= ", maxC1, " ",
                  " AND ", "SL.OLON >= ", minC2, " ", " AND ", "SL.OLON <= ",
                  maxC2, " ", 0);
...
static char *
BuildMinMaxRestrictSL(int index, char *option, char *latlon) {
    char minC[10];
    char maxC[10];
...
char *
dbBuildSearchlink(void) {
    /* Declare Variables. */
    char *tmp;
    int j, k, jdx;
    char baseString[] =
        "SELECT DISTINCT SL.EVID, SL.ORID, SL.OLAT, SL.OLON, SL.DEPTH, "
    ...
    char siteChanWhereClause[] =
        "SC.STA = SL.STA AND SC.CHAN = SL.CHAN\n"
        ...
        if (optionSelected[j]) {
            ...
            if (!strcmp(options[j], "EVENTID")) {
                ...
                for (jdx = 0; jdx < numParams[24]; jdx++) {
                    int temp =
                        strlen(charParams[24][jdx]) + strlen(EvidRestriction) +
                        2;
                ...
                EvidRestriction =
                    (char *) realloc(EvidRestriction,
                                     strlen(EvidRestriction) + 3);
                strcat(EvidRestriction, " ) ");
            ...
            if (!strcmp(options[j], "STARTTIME")) {
                if (numParams[j] > 0) {
                    ...
                    for (k = 1; k < numParams[j]; k++) {
                        ...
                        RequestTimeC =
                            strAllocCat(RequestTimeC, charParams[j][k]);
                    }
    ...
    if (useSiteChanRestrict)
        sprintf(FromString, "FROM %s SL, %s SC ", dbGetSearchlinkTableName(),
                dbGetSitechanTableName());
    else
        sprintf(FromString, "FROM %s SL ", dbGetSearchlinkTableName());

    QueryString =
        (char *) malloc(3 + strlen(baseString) + strlen(FromString) +
                        strlen(WhereString));

    sprintf(QueryString, "%s\n%s\n%s", baseString, FromString, WhereString);
