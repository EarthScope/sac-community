...
int
gcCheckWftag(struct wftag *wt, struct wfdiscList *wfL) {
    int ID;
...
int
gcCheckSensor(struct sensor *se, struct wfdiscList *wfL) {
    char *ST, *CH;
    double TI, ET;
    ...
    for (; wfL; wfL = wfL->next) {
        ...
        if (!strcmp(ST, se->sta) && !strcmp(CH, se->chan) && TI >= se->time &&
            ET <= se->endtime)
            return TRUE;
    }
...
int
gcCheckSitechan(struct sitechan *sc, struct wfdiscList *wfL) {
    char *ST, *CH;
    double TI, ET;
    ...
    for (; wfL; wfL = wfL->next) {
        ...
        if (!strcmp(ST, sc->sta) && !strcmp(CH, sc->chan) &&
            TI >= julianToEpoch(sc->ondate, TRUE) && (sc->offdate < 0 ||
                                                      ET <=
                                                      julianToEpoch(sc->
                                                                    offdate <
                                                                    0 ? FUTURE :
...
int
gcCheckSite(struct site *si, struct wfdiscList *wfL) {
    char *ST;
    double TI, ET;
    ...
    for (; wfL; wfL = wfL->next) {
        ...
        if (!strcmp(ST, si->sta) && TI >= julianToEpoch(si->ondate, TRUE) &&
            (si->offdate < 0 ||
             ET <= julianToEpoch(si->offdate < 0 ? FUTURE : si->offdate,
                                 FALSE)))
            return TRUE;
    }
...
int
gcCheckInstrument(struct instrument *in, struct sensorList *sL) {
    int ID;
...
int
gcCheckEvent(struct event *ev, struct wftagList *wtL) {
    char *tagname;
...
int
gcCheckOrigin(struct origin *orig, struct eventList *evL) {
