...
struct wftagList *
gcCollectWftag(struct wftagList *wtL, DBlist dblList) {
...
struct sensorList *
gcCollectSensor(struct sensorList *seL, DBlist dblList) {
    struct CSStree *tree;
...
struct sitechanList *
gcCollectSitechan(struct sitechanList *scL, DBlist dblList) {
    struct CSStree *tree;
...
struct siteList *
gcCollectSite(struct siteList *siL, DBlist dblList) {
    ...
    if (!gcCheckSite(siL->element, tree->wfHead)) {
        ...
        for (afPntr = tree->afHead; afPntr;) {
            ...
            if (!strcmp(afPntr->element->sta, siL->element->sta)) {
                dblDeleteTableInstance(dbl_LIST_AFFILIATION, dblList, afPntr);
            }
            afPntr = tmp;
...
gcCollectInstrument(struct instrumentList *inL, DBlist dblList) {
    struct CSStree *tree;
...
struct originList *
gcCollectOrigin(struct originList *orL, DBlist dblList) {
    ...
    if (!gcCheckOrigin(orL->element, tree->evHead)) {
        ...
        for (asPntr = tree->asHead; asPntr;) {
            ...
            if (asPntr->element->orid == orL->element->orid) {
                dblDeleteTableInstance(dbl_LIST_ASSOC, dblList, asPntr);
            }
            asPntr = tmp;
        }
...
struct eventList *
gcCollectEvent(struct eventList *evL, DBlist dblList) {
    struct CSStree *tree;
