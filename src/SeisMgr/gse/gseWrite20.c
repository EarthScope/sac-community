...
int
stringMatch(char *first, char *second, int num) {
    int fiLen, seLen, idx;
...
void
GetChannelInfo(struct sitechanList *sc, char *Station, char *comp, double *hang,
               double *vang) {
...
void
GetSiteInfo(struct siteList *si, char *sta, int ondate, int offdate, float *lat,
    ...
    *lat = *lon = *elev = -999.0;
    ...
    while (si) {
        if (stringMatch(sta, si->element->sta, 5) &&
            ondate == si->element->ondate && offdate == si->element->offdate) {
            *lat = si->element->lat;
            *lon = si->element->lon;
...
void
GetWfdiscInfo(struct wfdiscList *wf, char *sta, char *chan, int ondate,
              int offdate, char *inst) {
    strcpy(inst, "-");
    ...
    while (wf) {
        if (stringMatch(sta, wf->element->sta, 5) &&
            ...
            ondate <= wf->element->jdate && offdate >= wf->element->jdate) {
...
void
PutIntro(FILE * ptr, char *datasource) {
...
int
PutWaveformData(FILE * ptr, DBlist tree, int cm6) {
    char id[5], time[24], station[6], channel[4], auxid[5], datatype[4],
        samps[9], samprat[12], calib[11], calper[9];
    char instype[] = "      ", hang[6], vang[5];
...
int
PutStationData(FILE * ptr, DBlist tree) {
    char sta[6], statype[5], lat[10], lon[11], elev[8], ondate[25], offdate[25];
...
int
PutChannelData(FILE * ptr, DBlist tree) {
    char sta[6], chan[4], auxid[5], lat[10], lon[11], elev[8], depth[7],
        hang[7], vang[6], samprat[12], inst[8], ondate[25], offdate[25];
    ...
    while (sc) {
        ...
        /* inst */
        GetWfdiscInfo(wf, sta, chan, sc->element->ondate, sc->element->offdate,
                      inst);
        ...
        /* ondate */
        if (isValidInt
            (dbl_LIST_SITECHAN, dbl_SITEC_ONDATE, sc->element->ondate)) {
        ...
        /* offdate */
        if (isValidInt
            (dbl_LIST_SITECHAN, dbl_SITEC_OFFDATE, sc->element->offdate)) {
        ...
        fprintf(ptr, "%s %s %s %s %s %s %s %s %s %s %s %s %s\n", sta, chan,
                auxid, lat, lon, elev, depth, hang, vang, samprat, inst, ondate,
                offdate);
...
int
PutArrivalData(FILE * ptr, DBlist tree) {
    char sta[6], dist[7], evaz[6], picktype =
        ' ', direction, detchar, phase[8], time[25], tres[] =
        "     ", azim[6], azres[7], slow[6], sres[6], tdef, adef, sdef, snr[6],
        amp[10], per[6], mdef1[3], mag1[5], mdef2[3], mag2[5], id[9];
    ...
    int dots = 0;
    char HeaderLine[] =
        "Sta     Dist  EvAz     Phase      Date       Time     TRes  Azim"
    ...
    while (ar) {
        ...
        if (orig && si &&
            ...
            float slat = si->element->lat, slon = si->element->lon, elat =
                ...
                orig->element->lon, delt, fDist, fAzim, bazim;
        ...
        /* phase */
        if (isValidString
            (dbl_LIST_ARRIVAL, dbl_ARRIV_IPHASE, ar->element->iphase) &&
        ...
        fprintf(ptr,
                "%s %s %s %c%c%c %s %s %s %s %s %s %s %c%c%c %s %s %s %s%s %s%s %s",
                sta, dist, evaz, picktype, direction, detchar, phase, time,
                tres, azim, azres, slow, sres, tdef, adef, sdef, snr, amp, per,
...
int
PutOriginData(FILE * ptr, DBlist tree) {
    char HeaderLine1[] =
        "   Date       Time       Latitude Longitude    Depth    Ndef Nsta "
        "Gap    Mag1  N    Mag2  N    Mag3  N  Author          ID\n";
    char HeaderLine2[] =
        "       rms   OT_Error      Smajor Sminor Az        Err   mdist"
        "  Mdist     Err        Err        Err     Quality\n";

    char time[25], fixf = ' ', lat[9], lon[10], depth[6], ndef[5], nsta[] =
        "    ", gap[] =
        "   ", magtype1[3], mag1[5], magtype2[3], mag2[5], magtype3[3], mag3[5],
        N[] = "  ", author[9], id[9], line2[] =
        "\n             +-                                +-                       +-         +-         +-              \n\n";
    ...
    while (orig) {
        ...
        /* out put the strings */
        fprintf(ptr,
                "%s %c  %s %s %c  %s %c  %s %s %s  %s%s %s  %s%s %s  %s%s %s  %s  %s%s",
                time, fixf, lat, lon, fixf, depth, fixf, ndef, nsta, gap,
                magtype1, mag1, N, magtype2, mag2, N, magtype3, mag3, N, author,
                id, line2);
...
int
WriteGSEFile(char *outFile, DBlist tree, char *datasource, int cm6) {
    int Ntraces;
    FILE *ptr;
