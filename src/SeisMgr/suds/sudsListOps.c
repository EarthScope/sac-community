...
void *
AddSudsListElement(SUDS * S, short StructID) {
    deList *newDE;
    adList *newAD;
    tsList *newTS;
    esList *newES;
    ...
    switch (StructID) {
        ...
        case TIMECORRECTION:
            ...
            newTC->tc =
                (SUDS_TIMECORRECTION *) malloc(sizeof(SUDS_TIMECORRECTION));
            *(newTC->tc) = NullTC;
            newTC->next = 0;
            if (S->tcTail) {
                S->tcTail->next = newTC;
...
AllocateArray(dtList * dt) {
    ...
    switch (dt->dt->datatype) {
        ...
        case 'i':
            ...
            if (!dt->i2data) {
                printf
                    ("ERROR: Could not allocate data array in AllocateArray.\n");
                exit(1);
            }
            return;
...
void
Mux2Trace(SUDS * S, muList * mu, int Nsamples, int BlockNumber) {
    int j = 0;
