...
static int
ReadStatIdent(SUDS_STATIDENT * si, FILE * ptr) {
    int bytesRead;
...
int
FillDetectorStruct(SUDS_DETECTOR * de, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillAtoDStruct(SUDS_ATODINFO * ad, SUDS_STRUCTTAG Tag, FILE * ptr) {

    if (!ad)
        return 0;

    if (fread(ad, 1, Tag.len_struct, ptr) != (size_t) Tag.len_struct)
        return 0;
    if (CheckByteOrder() == ENDIAN_BIG) {
...
int
FillTrigSettingStruct(SUDS_TRIGSETTING * ts, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillEventSettingStruct(SUDS_EVENTSETTING * es, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillStationCompStruct(SUDS_STATIONCOMP * sc, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillMuxDataStruct(SUDS * S, SUDS_MUXDATA * mu, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
    int j;
        ...
        switch (mu->typedata) {
            ...
            case 'i':
                ...
                if (bytesRead != Tag.len_data) {
                    printf
                        ("ERROR: Expected to read %d bytes but got %d instead.\n",
                         Tag.len_data, bytesRead);
                    return 0;
                }
                if (CheckByteOrder() == ENDIAN_BIG) {
                    for (j = 0; j < mu->numsamps; j++)
                        Convert2(S->muTail->i2data + j);
                }
                return 1;
...
int
FillDescripTraceStruct(SUDS * S, SUDS_DESCRIPTRACE * dt, SUDS_STRUCTTAG Tag,
                       FILE * ptr) {
    int bytesRead;
    int j;
    ...
    if (Tag.len_data) {
        switch (dt->datatype) {
            ...
            case 'i':
                S->dtTail->i2data =
                    (short *) malloc(dt->length * sizeof(short));
                if (!S->dtTail->i2data) {
                    printf
                ...
                bytesRead =
                    fread(S->dtTail->i2data, 1, sizeof(short) * dt->length,
                          ptr);
                if (bytesRead != Tag.len_data) {
                    printf
                        ("ERROR: Expected to read %d bytes but got %d instead.\n",
            ...
            case 'l':
                ...
                bytesRead =
                    fread(S->dtTail->i4data, 1, sizeof(int) * dt->length, ptr);
                if (bytesRead != Tag.len_data) {
                    printf
                        ("ERROR: Expected to read %d bytes but got %d instead.\n",
                         Tag.len_data, bytesRead);
                ...
                if (CheckByteOrder() == ENDIAN_BIG) {
                    for (j = 0; j < dt->length; j++)
                        Convert4(S->dtTail->i4data + j);
                }
                return 1;

            case 'f':
                S->dtTail->r4data =
                    (float *) malloc(dt->length * sizeof(float));
                if (!S->dtTail->r4data) {
                    printf
                        ("ERROR: Could not allocate data array in FillDescripTraceStruct.\n");
                    return 0;
                ...
                bytesRead =
                    fread(S->dtTail->r4data, 1, sizeof(float) * dt->length,
                          ptr);
                if (bytesRead != Tag.len_data) {
                    printf
                        ("ERROR: Expected to read %d bytes but got %d instead.\n",
                ...
                if (CheckByteOrder() == ENDIAN_BIG) {
                    for (j = 0; j < dt->length; j++)
                        Convert4(S->dtTail->r4data + j);
                }
                return 1;

            case 'd':
                S->dtTail->r8data =
                    (double *) malloc(dt->length * sizeof(double));
                if (!S->dtTail->r8data) {
                    printf
                        ("ERROR: Could not allocate data array in FillDescripTraceStruct.\n");
                    return 0;
                ...
                bytesRead =
                    fread(S->dtTail->r8data, 1, sizeof(double) * dt->length,
                          ptr);
                if (bytesRead != Tag.len_data) {
                    printf
                        ("ERROR: Expected to read %d bytes but got %d instead.\n",
...
int
FillFeatureStruct(SUDS_FEATURE * fe, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillCalibStruct(SUDS_CALIBRATION * ca, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillCommentStruct(SUDS * S, SUDS_COMMENT * co, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillChansetStruct(SUDS * S, SUDS_CHANSET * cs, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
    int j;
    ...
    if (Tag.len_data) {
        S->csTail->entry =
            (CHANSETENTRY *) malloc(cs->entries * sizeof(CHANSETENTRY));
        bytesRead = 0;
...
int
FillEquipmentStruct(SUDS_EQUIPMENT * eq, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillErrorStruct(SUDS_ERROR * er, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
    int j;
...
int
FillEventStruct(SUDS_EVENT * ev, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillTriggerStruct(SUDS_TRIGGERS * tr, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillEVDescriptStruct(SUDS_EVDESCR * ed, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillFocalMechStruct(SUDS_FOCALMECH * fo, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillInstrumentStruct(SUDS_INSTRUMENT * in, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillLayersStruct(SUDS_LAYERS * la, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillLoctraceStruct(SUDS_LOCTRACE * lo, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillMomentStruct(SUDS_MOMENT * mo, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
    int j;
...
int
FillOriginStruct(SUDS_ORIGIN * orig, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillProfileStruct(SUDS_PROFILE * pr, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillResidualStruct(SUDS_RESIDUAL * re, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillShotgatherStruct(SUDS_SHOTGATHER * sh, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillTimeCorrectionStruct(SUDS_TIMECORRECTION * tc, SUDS_STRUCTTAG Tag,
                         FILE * ptr) {
    int bytesRead;
...
int
FillVelocityModelStruct(SUDS_VELMODEL * vm, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
...
int
FillTerminatorStruct(SUDS_TERMINATOR * te, SUDS_STRUCTTAG Tag, FILE * ptr) {
    int bytesRead;
