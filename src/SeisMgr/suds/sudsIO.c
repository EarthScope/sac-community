...
int FillMuxDataStruct(SUDS * S, SUDS_MUXDATA * mu, SUDS_STRUCTTAG Tag,
                      FILE * ptr);
...
void
ApplyRateCorrection(SUDS * S, double RateCorrect, int Verbose) {
    dtList *dt;
...
void
ApplyTimeCorrection(SUDS * S, double TimeCorrect, int Verbose) {
    dtList *dt;
...
void
ApplyCorrectionsFromIrigStruct(SUDS * S, int Verbose) {
    dtList *dt;
...
void
ApplyExistingTimeCorrections(SUDS * S, int Verbose) {
    tcList *tc;
    double RateCorrect = 0;
    double TimeCorrect = 0;
    ...
    if (!tc) {
        if (Verbose) {
            printf
                ("No time-correction struct found! Trace times and rates are uncorrected.\n");
            printf
                ("Attempting to extract information from IRIG descriptrace struct.\n");
        }
    ...
    if (!SuccessfulDecode) {
        if (Verbose) {
            printf
                ("%d Time-correct structs found, but none showed successful sync.\n",
                 NumTCstructs);
            printf
                ("Attempting to extract information from IRIG descriptrace struct.\n");
        }
...
static int
AddToSuds(SUDS * S, SUDS_STRUCTTAG Tag, FILE * ptr, int Verbose) {
    SUDS_DETECTOR *de;
    SUDS_ATODINFO *ad;
    SUDS_TRIGSETTING *ts;
...
int
TagFound(FILE * ptr, SUDS_STRUCTTAG * Tag) {
    /* Read in the tag */
...
SUDS *
ReadSUDSHeaders(char *fname, int Verbose) {
    SUDS *S = 0;
    FILE *ptr;
    SUDS_STRUCTTAG Tag;
    int offset;
...
SUDS *
ReadSUDSfile(char *fname, int Verbose) {
    SUDS *S = 0;
    FILE *ptr;
    SUDS_STRUCTTAG Tag;
    /* Open the file */
...
int
WriteSudsfile(SUDS * S, const char *fname) {
    FILE *ptr;
    SUDS_STRUCTTAG Tag;
    ...
    if (S->stHead)
        printf("FreeSTList(S->stHead)\n");
    if (S->adHead)
        if (!WriteADlist(S->adHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->caHead)
        if (!WriteCAlist(S->caHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->coHead)
        if (!WriteCOlist(S->coHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->csHead)
        if (!WriteCSlist(S->csHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->dtHead)
        if (!WriteDTlist(S->dtHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->deHead)
        if (!WriteDElist(S->deHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->eqHead)
        if (!WriteEQlist(S->eqHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->erHead)
        if (!WriteERlist(S->erHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->evHead)
        if (!WriteEVlist(S->evHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->esHead)
        if (!WriteESlist(S->esHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->edHead)
        if (!WriteEDlist(S->edHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->feHead)
        if (!WriteFElist(S->feHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->foHead)
        if (!WriteFOlist(S->foHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->inHead)
        if (!WriteINlist(S->inHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->laHead)
        if (!WriteLAlist(S->laHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->loHead)
        if (!WriteLOlist(S->loHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->moHead)
        if (!WriteMOlist(S->moHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->muHead)
        printf("Muxdata not supported on output.\n");
    if (S->orHead)
        if (!WriteORlist(S->orHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->prHead)
        if (!WritePRlist(S->prHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->reHead)
        if (!WriteRElist(S->reHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->shHead)
        if (!WriteSHlist(S->shHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->scHead)
        if (!WriteSClist(S->scHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->teHead)
        if (!WriteTElist(S->teHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->tcHead)
        if (!WriteTClist(S->tcHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->trHead)
        if (!WriteTRlist(S->trHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->tsHead)
        if (!WriteTSlist(S->tsHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
    if (S->vmHead)
        if (!WriteVMlist(S->vmHead, &Tag, ptr)) {
            fclose(ptr);
            return 0;
        }
