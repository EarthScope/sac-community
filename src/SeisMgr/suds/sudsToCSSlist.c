...
char *
sudsGetEventType(SUDS * S, int ev_num) {
    evList *ev = S->evHead;
    while (ev) {
        if (ev->ev->number == ev_num) {
            switch (ev->ev->ev_type) {
                case 'e':
                    strcpy(StaticString, "eq");
                    return StaticString;
                case 'E':
                    strcpy(StaticString, "qb");
                    return StaticString;
                case 'n':
                    strcpy(StaticString, "ex");
                    return StaticString;
                case 'i':
                    strcpy(StaticString, "o");
                    return StaticString;
                case 'r':
                    strcpy(StaticString, "r");
                    return StaticString;
                case 't':
                    strcpy(StaticString, "t");
                    return StaticString;
            }
        }
        ev = ev->next;
...
sudsAddOriginStruct(SUDS * S, DBlist tree, orList * orS) {
    ...
    if (orS->orig->or_lat != NODATA)
        orig->element->lat = orS->orig->or_lat;
    if (orS->orig->or_long != NODATA)
        orig->element->lon = orS->orig->or_long;
    if (orS->orig->depth != NODATA)
        orig->element->depth = orS->orig->depth;
    if (orS->orig->orgtime != NOTIME) {
        orig->element->time = orS->orig->orgtime;
        tmDecodeEpochTime(orS->orig->orgtime, &year, &month, &day, &hour, &min,
                          &second);
    ...
    if (orS->orig->region != NODATA)
        orig->element->grn = orS->orig->region;
...
static int
sudsAddAffiliationStruct(DBlist tree, char *sta, char *net) {
    struct affiliationList *af = 0;

    do {
        af = (struct affiliationList *) dblNextTableInstance(af, tree,
                                                             dbl_LIST_AFFILIATION);
        if (af && !strcmp(sta, af->element->sta) &&
            !strcmp(net, af->element->net)) {
            return 1;
        }
    } while (af);
...
static int
sudsAddAssoc(DBlist tree, char *sta, int arid, char *iphase) {
    ...
    if (siMatch) {
        dbDelaz((float *) &(si->element->lat), (float *) &(si->element->lat),
                (float *) &(orig->element->lat),
                (float *) &(orig->element->lon), &delt, &dist, &azim, &bazim);
...
int
InSiteList(scList * sc, DBlist tree) {
            ...
            if (sc->sc->st_lat == si->element->lat)
                if (sc->sc->st_long == si->element->lon)
                    if (sc->sc->elev == si->element->elev * 1000)
                        if (sc->sc->effective ==
                            (int) julianToEpoch(si->element->ondate, 1)) {
                            return 1;
                        }
        si = si->next;
    }
...
static int
sudsAddSiteStruct(SUDS * S, DBlist tree, scList * sc) {
    struct siteList *si;
    double Time;
    int year, month, day, hour, min;
    float second;
    ...
    if (sc->sc->st_lat == NODATA && sc->sc->st_long == NODATA &&
        sc->sc->elev == NODATA)
...
static int
sudsAddArrivalStruct(SUDS * S, DBlist tree, feList * fe) {
    ...
    if (!strlen(fe->fe->fe_name.st_name)) {
        if (Verbose)
            printf("SUDS FEATURE with no station name --- Cannot associate!\n");
        return 1;               /* Can't associate without a station name. */
    }

    if (fe->fe->fe_name.component == NOCHAR) {
        if (Verbose)
            printf("SUDS FEATURE with no channel name --- Cannot associate!\n");
        return 1;               /* Also need a component to associate. */
    }

    if (fe->fe->obs_phase == NODATA) {
        if (Verbose)
            printf("SUDS FEATURE with no phase code --- Cannot associate!\n");
    ...
    if (!sudsAddAssoc
        (tree, ar->element->sta, ar->element->arid, ar->element->iphase))
        return 0;
...
static int
sudsAddSiteChanStruct(SUDS * S, DBlist tree, scList * sc) {
    ...
    si = (struct sitechanList *) dblCreateTableInstance(tree,
                                                        dbl_LIST_SITECHAN);
    if (!si)
        return 0;
    ...
    if (sc->sc->channel != NODATA) {
        if (dblChanidInUse(tree, sc->sc->channel))
            dblReplaceChanid(tree, sc->sc->channel,
                             dblNextAvailableChanid(tree));
...
static int
sudsAddWfdiscStruct(DBlist tree, dtList * dt, struct originList *orig,
                    const char *fname) {
    struct wfdiscList *w;
    struct wftagList *wt;
    int year, month, day, hour, min;
    ...
    w->element->endtime =
        w->element->time + (w->element->nsamp) / w->element->samprate;
    ...
    tmDecodeEpochTime(w->element->time, &year, &month, &day, &hour, &min,
                      &second);
    ...
    if (strlen(dt->dt->dt_name.st_name) && strlen(dt->dt->dt_name.network))
        if (!sudsAddAffiliationStruct
            (tree, dt->dt->dt_name.st_name, dt->dt->dt_name.network))
            return 0;
    return 1;
}
...
int
sudsToCSSlist(const char *filename, const char *WorkSetName, int Replace,
              int Maxfiles, int verbose, double MaxPhysMem) {
    DBlist tree;
    DBlist tree2;
    SUDS *S;
    dtList *dt;
