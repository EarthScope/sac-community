...
static int
WriteStatIdent(SUDS_STATIDENT * si, FILE * ptr) {
    int bytesOut;
...
static void
FillTagIn(SUDS_STRUCTTAG * Tag, short id, int len, int dataLen) {
...
static int
WriteADstruct(SUDS_ATODINFO * ad, FILE * ptr, int length) {
    SUDS_ATODINFO A;
    if (!ad)
        return 0;
    A = *ad;
    ...
    if (fwrite(&A, 1, length, ptr) != (size_t) length)
        return 0;
    return 1;
}
...
int
WriteADlist(adList * adHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    adList *ad;
    short type = ATODINFO;
    int length = AD_LEN;
    ...
    while (ad) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteADstruct(ad->ad, ptr, length))
            return 0;
        ad = ad->next;
    }
...
static int
WriteDEstruct(SUDS_DETECTOR * de, FILE * ptr, int length) {
    int bytesOut;
    SUDS_DETECTOR D;
    if (!de)
        return 0;
...
int
WriteDElist(deList * deHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    deList *de;
    short type = DETECTOR;
    int length = DE_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (de) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteDEstruct(de->de, ptr, length))
            return 0;
        de = de->next;
    }
...
static int
WriteESstruct(SUDS_EVENTSETTING * es, FILE * ptr, int length) {
    int bytesOut;
    SUDS_EVENTSETTING E;
...
int
WriteESlist(esList * esHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    esList *es;
    ...
    while (es) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteESstruct(es->es, ptr, length))
            return 0;
        es = es->next;
    }
...
static int
WriteTRstruct(SUDS_TRIGGERS * tr, FILE * ptr, int length) {
    int bytesOut;
    SUDS_TRIGGERS T;
    if (!tr)
        return 0;
...
int
WriteTRlist(trList * trHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    trList *tr;
    short type = TRIGGERS;
    int length = TR_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (tr) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteTRstruct(tr->tr, ptr, length))
            return 0;
        tr = tr->next;
    }
...
static int
WriteFEstruct(SUDS_FEATURE * fe, FILE * ptr, int length) {
    int bytesOut;
    SUDS_FEATURE F;
    if (!fe)
        return 0;
...
int
WriteFElist(feList * feHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    feList *fe;
    short type = FEATURE;
    int length = FE_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (fe) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteFEstruct(fe->fe, ptr, length))
            return 0;
        fe = fe->next;
    }
...
static int
WriteSCstruct(SUDS_STATIONCOMP * sc, FILE * ptr, int length) {
    int bytesOut;
    SUDS_STATIONCOMP S;
    if (!sc)
        return 0;
...
int
WriteSClist(scList * scHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    scList *sc;
    ...
    while (sc) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteSCstruct(sc->sc, ptr, length))
            return 0;
        sc = sc->next;
    }
...
static int
WriteTSstruct(SUDS_TRIGSETTING * ts, FILE * ptr, int length) {
    int bytesOut;
    SUDS_TRIGSETTING T;
...
int
WriteTSlist(tsList * tsHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    tsList *ts;
    ...
    while (ts) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteTSstruct(ts->ts, ptr, length))
            return 0;
        ts = ts->next;
    }
...
static int
WriteDTstruct(SUDS_DESCRIPTRACE * dt, FILE * ptr, int length) {
    int bytesOut;
    SUDS_DESCRIPTRACE D;
...
static int
WriteDTdata(dtList * dt, FILE * ptr) {
    short *i2data;
    int *i4data;
    float *r4data;
    double *r8data;
    ...
    switch (dt->dt->datatype) {
        ...
        case 'i':
            ...
            if ((size_t) bytesOut != sizeof(short) * dt->dt->length) {
                printf
                    ("ERROR: Expected to write %d bytes but wrote %d instead.\n",
...
int
WriteDTlist(dtList * dtHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    dtList *dt;
    char Type[] = "\0\0";
...
static int
WriteVMstruct(SUDS_VELMODEL * vm, FILE * ptr, int length) {
    int bytesOut;
    SUDS_VELMODEL V;
...
int
WriteVMlist(vmList * vmHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    vmList *vm;
    short type = VELMODEL;
    int length = VM_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (vm) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteVMstruct(vm->vm, ptr, length))
            return 0;
        vm = vm->next;
    }
...
static int
WriteTCstruct(SUDS_TIMECORRECTION * tc, FILE * ptr, int length) {
    int bytesOut;
...
int
WriteTClist(tcList * tcHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    tcList *tc;
    ...
    while (tc) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteTCstruct(tc->tc, ptr, length))
            return 0;
        tc = tc->next;
    }
...
static int
WriteTEstruct(SUDS_TERMINATOR * te, FILE * ptr, int length) {
    int bytesOut;
    SUDS_TERMINATOR T;
...
int
WriteTElist(teList * teHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    teList *te;
    ...
    while (te) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteTEstruct(te->te, ptr, length))
            return 0;
        te = te->next;
    }
...
static int
WriteCAstruct(SUDS_CALIBRATION * ca, FILE * ptr, int length) {
    int bytesOut;
    SUDS_CALIBRATION C;
...
int
WriteCAlist(caList * caHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    caList *ca;
    ...
    while (ca) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteCAstruct(ca->ca, ptr, length))
            return 0;
        ca = ca->next;
    }
...
static int
WriteCOstruct(SUDS_COMMENT * co, FILE * ptr, int length) {
    int bytesOut;
    SUDS_COMMENT C;
...
int
WriteCOlist(coList * coHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    coList *co;
    short type = COMMENT;
    int length = CO_LEN;
    ...
    while (co) {
        ...
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteCOstruct(co->co, ptr, length))
            return 0;
        if (fwrite(co->text, 1, co->co->length, ptr) != (size_t) co->co->length)
            return 0;
        co = co->next;
    }
...
static int
WriteEVstruct(SUDS_EVENT * ev, FILE * ptr, int length) {
    int bytesOut;
    SUDS_EVENT E;
...
int
WriteEVlist(evList * evHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    evList *ev;
    short type = EVENT;
    int length = EV_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (ev) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteEVstruct(ev->ev, ptr, length))
            return 0;
        ev = ev->next;
    }
...
static int
WriteEDstruct(SUDS_EVDESCR * ed, FILE * ptr, int length) {
    int bytesOut;
    SUDS_EVDESCR E;
...
int
WriteEDlist(edList * edHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    edList *ed;
    ...
    while (ed) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteEDstruct(ed->ed, ptr, length))
            return 0;
        ed = ed->next;
    }
...
static int
WriteFOstruct(SUDS_FOCALMECH * fo, FILE * ptr, int length) {
    int bytesOut;
    SUDS_FOCALMECH F;
...
int
WriteFOlist(foList * foHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    foList *fo;
    ...
    while (fo) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteFOstruct(fo->fo, ptr, length))
            return 0;
        fo = fo->next;
    }
...
static int
WriteINstruct(SUDS_INSTRUMENT * in, FILE * ptr, int length) {
    int bytesOut;
    SUDS_INSTRUMENT I;
...
int
WriteINlist(inList * inHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    inList *in;
    ...
    while (in) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteINstruct(in->in, ptr, length))
            return 0;
        in = in->next;
    }
...
static int
WriteLAstruct(SUDS_LAYERS * la, FILE * ptr, int length) {
    int bytesOut;
    SUDS_LAYERS L;
...
int
WriteLAlist(laList * laHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    laList *la;
    short type = LAYERS;
    int length = LA_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (la) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteLAstruct(la->la, ptr, length))
            return 0;
        la = la->next;
    }
...
static int
WriteLOstruct(SUDS_LOCTRACE * lo, FILE * ptr, int length) {
    int bytesOut;
    SUDS_LOCTRACE L;
...
int
WriteLOlist(loList * loHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    loList *lo;
    short type = LOCTRACE;
    int length = LO_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (lo) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteLOstruct(lo->lo, ptr, length))
            return 0;
        lo = lo->next;
    }
...
static int
WriteMOstruct(SUDS_MOMENT * mo, FILE * ptr, int length) {
    int bytesOut;
    int j;
    SUDS_MOMENT M;
...
int
WriteMOlist(moList * moHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    moList *mo;
    short type = MOMENT;
    int length = MO_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (mo) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteMOstruct(mo->mo, ptr, length))
            return 0;
        mo = mo->next;
    }
...
static int
WriteORstruct(SUDS_ORIGIN * orig, FILE * ptr, int length) {
    int bytesOut;
    SUDS_ORIGIN O;
...
int
WriteORlist(orList * orHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    orList *orig;
    short type = ORIGIN;
    int length = OR_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (orig) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteORstruct(orig->orig, ptr, length))
            return 0;
        orig = orig->next;
    }
...
static int
WritePRstruct(SUDS_PROFILE * pr, FILE * ptr, int length) {
    int bytesOut;
    SUDS_PROFILE P;
...
int
WritePRlist(prList * prHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    prList *pr;
    short type = PROFILE;
    int length = PR_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (pr) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WritePRstruct(pr->pr, ptr, length))
            return 0;
        pr = pr->next;
    }
...
static int
WriteREstruct(SUDS_RESIDUAL * re, FILE * ptr, int length) {
    int bytesOut;
    SUDS_RESIDUAL R;
...
int
WriteRElist(reList * reHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    reList *re;
    short type = RESIDUAL;
    int length = RE_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (re) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteREstruct(re->re, ptr, length))
            return 0;
        re = re->next;
    }
...
static int
WriteSHstruct(SUDS_SHOTGATHER * sh, FILE * ptr, int length) {
    int bytesOut;
    SUDS_SHOTGATHER S;
...
int
WriteSHlist(shList * shHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    shList *sh;
    ...
    while (sh) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteSHstruct(sh->sh, ptr, length))
            return 0;
        sh = sh->next;
    }
...
static int
WriteERstruct(SUDS_ERROR * er, FILE * ptr, int length) {
    int bytesOut;
    int j;
    SUDS_ERROR E;
...
int
WriteERlist(erList * erHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    erList *er;
    short type = ERROR;
    int length = ER_LEN;
    FillTagIn(Tag, type, length, 0L);
    ...
    while (er) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteERstruct(er->er, ptr, length))
            return 0;
        er = er->next;
    }
...
static int
WriteEQstruct(SUDS_EQUIPMENT * eq, FILE * ptr, int length) {
    int bytesOut;
    SUDS_EQUIPMENT E;
...
int
WriteEQlist(eqList * eqHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    eqList *eq;
    ...
    while (eq) {
        if (fwrite(Tag, TagLen, 1, ptr) != 1)
            return 0;
        if (!WriteEQstruct(eq->eq, ptr, length))
            return 0;
        eq = eq->next;
    }
...
static int
WriteCSstruct(SUDS_CHANSET * cs, FILE * ptr, int length) {
    int bytesOut;
    SUDS_CHANSET C;
...
static int
WriteCSEstruct(CHANSETENTRY cse, FILE * ptr, int length) {
    int bytesOut;
...
int
WriteCSlist(csList * csHead, SUDS_STRUCTTAG * Tag, FILE * ptr) {
    csList *cs;
    short type = CHANSET;
    int length = CS_LEN;
    int entries;
    ...
    while (cs) {
        ...
        if (!WriteCSstruct(cs->cs, ptr, length))
            return 0;
        length = CSE_LEN;
        for (j = 0; j < entries; j++)
            if (!WriteCSEstruct(cs->entry[j], ptr, length))
                return 0;
