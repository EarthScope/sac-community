...
TimeStaChanMatch(double time, char *sta, char *chan, DBlist tree) {
    ...
    do {
        if (!(w = (struct wfdiscList *)
              dblNextTableInstance(w, tree, dbl_LIST_WFDISC)))
            break;
...
AddToWfdisc(char *buffer, DBlist tree, char **Station, int Nstation,
    ...
    strncpy(w.sta, buffer + wOff[2], 6);
    Deblank(w.sta, 7);
    strncpy(w.chan, buffer + wOff[3], 2);
    Deblank(w.chan, 3);
    if (!InSpecList
    ...
    strncpy(w.instype, buffer + wOff[8], 6);
    Deblank(w.instype, 7);
    strncpy(w.segtype, buffer + wOff[9], 1);
    Deblank(w.segtype, 2);
    strncpy(w.dattype, buffer + wOff[10], 2);
    Deblank(w.dattype, 3);
    strncpy(w.clip, buffer + wOff[11], 1);
    Deblank(w.clip, 2);
    strncpy(w.dfile, buffer + wOff[15], 20);
    Deblank(w.dfile, 21);
...
static void
AddArrivalLine(char *buffer, int *Off, DBlist tree, int wfid) {
...
AddMatchingArrivalStruct(char *buffer, DBlist tree, char **Author, int Nauthor,
    ...
    strncpy(sta, buffer + arOff[2], 6);
    Deblank(sta, 7);
    strncpy(chan, buffer + arOff[3], 2);
    Deblank(chan, 3);
    strncpy(auth, buffer + arOff[22], 15);
    Deblank(auth, 16);
    strncpy(iphase, buffer + arOff[6], 8);
    Deblank(iphase, 9);
...
int
cssRead2_8FlatFiles(char *Root, char *WorkSetName, int Replace, char **Station,
                    int Nstation, char **Channel, int Nchannel, char **Band,
                    int Nband, char **Code, int Ncode, char **Orientation,
                    int Norientation, char **Author, int Nauthor,
                    char **FileList, int Nfiles, char **Phaselist, int Nphases,
                    int MaxAllowableWaveforms, int verbose, double MaxPhysMem) {
    DBlist tree;
    DBlist tree2;
    int LinesUsed;
