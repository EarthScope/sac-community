...
static void
FreeMatrix(ComplexFloat ** matrix, int Nrows, int Ncols) {
    int j;
...
static int
WriteAffiliationTable(struct affiliationList *af, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(af->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteArrivalTable(struct arrivalList *ar, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(ar->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteAssocTable(struct assocList *as, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(as->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteEventTable(struct eventList *ev, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(ev->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteGregionTable(struct gregionList *gr, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(gr->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteInstrumentTable(struct instrumentList *in, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(in->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteOrigerrTable(struct origerrList *oe, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(oe->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteOriginTable(struct originList *orig, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(orig->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteRemarkTable(struct remarkList *re, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(re->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteSensorTable(struct sensorList *se, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(se->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteSiteTable(struct siteList *si, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(si->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteSitechanTable(struct sitechanList *sc, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(sc->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteStassocTable(struct stassocList *sa, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(sa->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteWfdiscTable(struct wfdiscList *w, FILE * ptr) {
    CSSBTAG tag;
    int Cmplx;
    ...
    if (w->element->nsamp < 1 || !w->seis)
        tag.len_data = 0;
    else if (w->seis->Cmplx && w->seis->r && w->seis->i) {
        Cmplx = 1;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(w->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    if (w->element->nsamp < 1)
        return 1;

    if (w->seis && w->seis->Cmplx && w->seis->r && w->seis->i) {
        if (fwrite(&Cmplx, sizeof(int), 1, ptr) != 1)
            return 0;
        if (fwrite(w->seis->r, sizeof(float), w->element->nsamp, ptr) !=
            (size_t) w->element->nsamp)
            return 0;
        if (fwrite(w->seis->i, sizeof(float), w->element->nsamp, ptr) !=
            (size_t) w->element->nsamp)
            return 0;
        return 1;
    }
...
static int
WriteWftagTable(struct wftagList *wt, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(wt->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteSacdataTable(struct sacdataList *sd, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(sd->element, tag.len_struct, 1, ptr) != 1)
        return 0;
    return 1;
}                               /* ----------------------------------------------------------------- */

static int
WriteUserDataComment(char *comment, FILE * ptr) {
    CSSBTAG tag;
    ...
    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(comment, tag.len_data, 1, ptr) != 1)
        return 0;
    return 1;
}
...
static int
WriteUserDataCommentStruct(ComplexFloat * value, int Npts, char *comment,
    ...
    tag.len_data = 0;
    if (value)
        tag.len_data += Npts * sizeof(ComplexFloat);
    if (comment)
        tag.len_data += strlen(comment);

    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(&DC, sizeof(DataCom), 1, ptr) != 1)
        return 0;
    if (value)
        if (fwrite(value, sizeof(ComplexFloat), Npts, ptr) != (size_t) Npts)
            return 0;

    if (comment)
        if (fwrite(comment, strlen(comment), 1, ptr) != 1)
            return 0;
...
static int
WriteUserDataMatrix(ComplexFloat ** matrix, int Nrows, int Ncols, char *comment,
                    FILE * ptr) {
    ...
    tag.len_data = 0;
    if (matrix)
        tag.len_data += Nrows * Ncols * sizeof(ComplexFloat);
    if (comment)
        tag.len_data += strlen(comment);

    if (fwrite(&tag, sizeof(CSSBTAG), 1, ptr) != 1)
        return 0;
    if (fwrite(&UD, sizeof(UDmatrix), 1, ptr) != 1)
        return 0;
    for (j = 0; j < Nrows; j++)
...
int
WriteCSSBfile(const char *WorkSetName, const char *fname, int verbose) {
    FILE *ptr;
    ...
    do {
        gr = (struct gregionList *) dblNextTableInstance(gr, tree,
                                                         dbl_LIST_GREGION);
        if (gr)
    ...
    do {
        oe = (struct origerrList *) dblNextTableInstance(oe, tree,
                                                         dbl_LIST_ORIGERR);
        if (oe)
    ...
    do {
        re = (struct remarkList *) dblNextTableInstance(re, tree,
                                                        dbl_LIST_REMARK);
        if (re)
    ...
    do {
        sa = (struct stassocList *) dblNextTableInstance(sa, tree,
                                                         dbl_LIST_STASSOC);
        if (sa)
    ...
    if (!WriteUserDataComment(comment, ptr)) {
        fclose(ptr);
        return 0;
    }
    smFree(comment);
    ...
    do {
        if (!(DCpntr = dblGetNextDataComment(DCpntr, tree)))
            break;
...
static int
CSSTagFound(FILE * ptr, CSSBTAG * Tag) {
    /* Read in the tag */
...
static int
AddAffiliationStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(af->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(af->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
...
static int
AddArrivalStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(ar->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(ar->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddAssocStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    struct assocList *as;
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(as->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(as->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddEventStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    struct eventList *ev;
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(ev->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(ev->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddGregionStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(gr->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(gr->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddInstrumentStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    struct instrumentList *in;
    int StructLen = sizeof(struct instrument);

    in = (struct instrumentList *) dblCreateTableInstance(tree,
                                                          dbl_LIST_INSTRUMENT);
    if (!in)
        return 0;

    if (StructLen < Tag.len_struct) {
        if (fread(in->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(in->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddOrigerrStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(oe->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(oe->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddOriginStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(orig->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(orig->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddRemarkStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(re->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(re->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddSensorStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(se->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(se->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddSiteStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    struct siteList *si;
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(si->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(si->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddSitechanStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(sc->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(sc->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddStassocStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(st->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(st->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddWfdiscStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    int Cmplx;
    int npts;
    int BytesRead = 0;
    struct wfdiscList *w;
    ...
    if (dblGetNumWaveformsInMemory(tree) + TracesInExistingTree >=
        MaxAllowableTraces) {
        if (FirstInstance) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(w->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen, SEEK_CUR);
    } else {
        if (fread(w->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
    ...
    if (fread(&Cmplx, sizeof(int), 1, ptr) != 1)
        return 0;
    ...
    if (Cmplx) {
        ...
        if (fread(w->seis->r, npts * sizeof(float), 1, ptr) != 1)
            return 0;
        BytesRead += npts * sizeof(float);
    }

    if (fread(w->seis->i, npts * sizeof(float), 1, ptr) != 1)
        return 0;
    BytesRead += npts * sizeof(float);
...
static int
AddWftagStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    struct wftagList *wt;
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(wt->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(wt->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddSacdataStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    ...
    if (StructLen < Tag.len_struct) {
        if (fread(sd->element, StructLen, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_struct - StructLen + Tag.len_data, SEEK_CUR);
    } else {
        if (fread(sd->element, Tag.len_struct, 1, ptr) != 1)
            return 0;
        fseek(ptr, Tag.len_data, SEEK_CUR);
    }
...
static int
AddDataComment(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    char *comment;
...
static int
AddDCStruct(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    DataCom DC;
    char *comment;
    ComplexFloat *data;
    int Expected;
    ...
    if (fread(&DC, sizeof(DataCom), 1, ptr) != 1)
        return 0;
    if (!Tag.len_data)
        return 1;
...
static int
AddUDmatrix(DBlist tree, CSSBTAG Tag, FILE * ptr) {
    UDmatrix UD;
    char *comment;
    ComplexFloat **matrix;
    int Expected, j;
    ...
    comment = NULL;
    matrix = NULL;
    if (fread(&UD, sizeof(UDmatrix), 1, ptr) != 1)
        return 0;
    if (!Tag.len_data)
        return 1;
    ...
    for (j = 0; j < UD.nrows; j++) {
        ...
        if (fread(matrix[j], UD.ncols * sizeof(ComplexFloat), 1, ptr) != 1)
            goto ERROR;
    }
    ...
    if (fread(comment, UD.CommentLen, 1, ptr) != 1)
        goto ERROR;
...
static int
AddToTree(DBlist tree, CSSBTAG Tag, FILE * ptr) {

    switch (Tag.id_struct) {
        case AFFILIATION:
            if (!AddAffiliationStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case ARRIVAL:
            if (!AddArrivalStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case ASSOC:
            if (!AddAssocStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case EVENT:
            if (!AddEventStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case GREGION:
            if (!AddGregionStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case INSTRUMENT:
            if (!AddInstrumentStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case ORIGERR:
            if (!AddOrigerrStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case ORIGIN:
            if (!AddOriginStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case REMARK:
            if (!AddRemarkStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case SENSOR:
            if (!AddSensorStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case SITE:
            if (!AddSiteStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case SITECHAN:
            if (!AddSitechanStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case STASSOC:
            if (!AddStassocStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case WFDISC:
            if (!AddWfdiscStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case WFTAG:
            if (!AddWftagStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case SACDATA:
            if (!AddSacdataStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case UDCOMMENT:
            if (!AddDataComment(tree, Tag, ptr))
                return 0;
            return 1;
        case DATACOM:
            if (!AddDCStruct(tree, Tag, ptr))
                return 0;
            return 1;
        case UDMATRIX:
            if (!AddUDmatrix(tree, Tag, ptr))
                return 0;
            return 1;
        default:
            fseek(ptr, Tag.len_struct + Tag.len_data, SEEK_CUR);
...
int
ReadCSSBfile(const char *fname, const char *WorkSetName, int Replace,
