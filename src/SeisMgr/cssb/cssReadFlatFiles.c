...
int
FileExists(char *dir, char *dfile) {
    int stringLen;
    char *filename;
    FILE *fptr;
...
char *
cssConstructFilename(char *Root, char *Suffix) {
...
int cssRead2_8FlatFiles(char *Root, char *WorkSetName, int Replace,
                        ...
                        char **Author, int Nauthor, char **Filelist, int Nfiles,
                        char **Phaselist, int Nphases, int MaxWaveforms,
                        int Verbose, double MaxPhysMemToUse);
...
char **
TokenLineToStrgList(char *line, int *Ntokens, char *Delimit) {
    char *tok;
    char **list;
    char D[10];
    char *ptr;
...
void
FreeStringArray(char **S, int Nlines) {
    int j;
    if (!S || !Nlines)
        return;
...
static int
GetWfdiscVersion(char *FileName) {
    FILE *ptr;
    int testlong;
    double testdouble;
    ...
    if ((int) strlen(buffer) >= Min30LineLength) {
        ...
        if (testlong < 0)
            return 0;           /* before year 0 */
        testlong = atol(buffer + 79);   /* Should be nsamp. */
        if (testlong < 0)
            return 0;           /* Can't have negative nsamp. */
        testdouble = atof(buffer + 88); /* Should be samprate. */
        if (testdouble < 0)
            return 0;           /* Can't have negative samprate. */
...
int
InList(char *str, char **List, int Nitems) {
#ifdef POSIX
    int j;
    for (j = 0; j < Nitems; j++)
        if (!fnmatch(List[j], str, 0))
            return 1;
#else
    UNUSED(str);
    UNUSED(List);
...
int
InOneCharList(char *chan, char **List, int Nitems, int pos) {
    int j;
    char c;
    ...
    for (j = 0; j < Nitems; j++) {
        if (List[j][0] == '*' || List[j][0] == '?' || c == toupper(List[j][0]))
            return 1;
    }
    return 0;
}
...
static int
InSpecList(char *sta, char *chan, char **Station, int Nstation, char **Channel,
           int Nchannel, char **Band, int Nband, char **Code, int Ncode,
           char **Orientation, int Norientation) {
    int position;

    if (Station && Nstation)
        if (!InList(sta, Station, Nstation))
            return 0;

    if (Channel && Nchannel)
        if (!InList(chan, Channel, Nchannel))
            return 0;

    if (Band && Nband) {
        position = 0;
        if (!InOneCharList(chan, Band, Nband, position))
            return 0;
    }

    if (Code && Ncode) {
        position = 1;
        if (!InOneCharList(chan, Code, Ncode, position))
            return 0;
    }

    if (Orientation && Norientation) {
        position = 2;
        if (!InOneCharList(chan, Orientation, Norientation, position))
            return 0;
    }
...
static int
AddToWfdisc(char *buffer, DBlist tree, char **Station, int Nstation,
            char **Channel, int Nchannel, char **Band, int Nband, char **Code,
            int Ncode, char **Orientation, int Norientation, char **FileList,
            int Nfiles, char *WfdiscPath) {
    struct wfdisc w;
    struct wfdiscList *wf;
    char AlternateDir[256];
    int wOff[] =
        { 0, 7, 16, 34, 43, 52, 61, 79, 88, 100, 117, 134, 141, 143, 146, 148,
        213, 246, 257, 266, 284
    };

    strncpy(w.sta, buffer + wOff[0], 6);
    Deblank(w.sta, 7);
    strncpy(w.chan, buffer + wOff[1], 8);
    Deblank(w.chan, 9);
    if (!InSpecList
        (w.sta, w.chan, Station, Nstation, Channel, Nchannel, Band, Nband, Code,
         Ncode, Orientation, Norientation))
        return 0;
    ...
    strncpy(w.instype, buffer + wOff[11], 6);
    Deblank(w.instype, 7);
    strncpy(w.segtype, buffer + wOff[12], 1);
    Deblank(w.segtype, 2);
    strncpy(w.dattype, buffer + wOff[13], 2);
    Deblank(w.dattype, 3);
    strncpy(w.clip, buffer + wOff[14], 1);
    Deblank(w.clip, 2);
    strncpy(w.dfile, buffer + wOff[16], 32);
    Deblank(w.dfile, 33);
    strncpy(w.dir, buffer + wOff[15], 64);
    Deblank(w.dir, 65);
    ...
    if (!FileExists(w.dir, w.dfile)) {
        ...
        if (!FileExists(AlternateDir, w.dfile)) {
            ...
            if (!FileExists(w.dir, w.dfile)) {
                css_warning
                    ("Cannot open waveform file(%s). Skipping trace... [Flat file]\n",
                     w.dfile);
                return 0;
            }
        } else
    ...
    strncpy(w.lddate, buffer + wOff[19], 17);
    Deblank(w.lddate, 18);
...
static int
ReadWfdiscFile(char *Root, char *WorkSetName, DBlist tree, char **Station,
    ...
    while (fgetsp(buffer, buflen, ptr)) {
        if (!strlen(buffer) || *buffer == '\n')
            continue;
        Lines =
            AddToWfdisc(buffer, tree, Station, Nstation, Channel, Nchannel,
...
static int
WfidInWfdiscList(int wfid, DBlist tree) {
...
static void
AddMatchingWftagStruct(char *buffer, DBlist tree) {
    struct wftagList *wt;
    int wfid = atol(buffer + 18);

    if (!WfidInWfdiscList(wfid, tree))
        return;

    wt = (struct wftagList *) dblCreateTableInstance(tree, dbl_LIST_WFTAG);
    if (!wt)
        return;
    strncpy(wt->element->tagname, buffer, 8);
    Deblank(wt->element->tagname, 9);
    wt->element->tagid = atol(buffer + 9);
    wt->element->wfid = wfid;
    strncpy(wt->element->lddate, buffer + 27, 17);
    Deblank(wt->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static struct originList *
findMatchingOrigin(DBlist tree, int evid, int orid) {
...
static void
AddOriginLine(char *buffer, int *orOffset, DBlist tree) {
    ...
    strncpy(o->element->etype, buffer + orOffset[12], 7);
    Deblank(o->element->etype, 8);
    o->element->depdp = atof(buffer + orOffset[13]);
    strncpy(o->element->dtype, buffer + orOffset[14], 1);
    ...
    strncpy(o->element->algorithm, buffer + orOffset[21], 15);
    Deblank(o->element->algorithm, 16);
    strncpy(o->element->auth, buffer + orOffset[22], 15);
    Deblank(o->element->auth, 16);
    o->element->commid = atol(buffer + orOffset[23]);
    strncpy(o->element->lddate, buffer + orOffset[24], 17);
    Deblank(o->element->lddate, 18);
...
static int
WfTagMatchEvid(int evid, DBlist tree) {
    char Tmp[10];
...
WfTagMatchOrid(int orid, DBlist tree) {
    char Tmp[10];
    struct wftagList *wt = 0;
    do {
        if (!
...
static struct wftagList *
WfTagMatchWfid(int wfid, DBlist tree) {
    struct wftagList *wt = 0;
    do {
        if (!
            (wt =
...
static void
AddWftagEntries(DBlist tree, int evid) {
    struct wfdiscList *w = 0;
    struct wftagList *wt;
    do {
        if (!
...
static void
BuildEventList(DBlist tree, int evid, int orid, int set) {
...
static int
GetPreforFromEvent(int evid, DBlist tree) {
    struct eventList *ev = 0;
...
int
oneEvidInArray(int evid, int *evidArray, int cnt) {
    int idx, nevids = 0;
...
static int
AddMatchingOriginStruct(char *buffer, DBlist tree, int *orOffset) {
    int evid, orid;
    struct wftagList *wt = 0;
...
static void
ReadOriginFile(char *Root, DBlist tree) {
    FILE *ptr;
    char *FileName;
    ...
    int *evidArray = NULL;
    int orOffset[] =
        { 0, 10, 20, 30, 48, 57, 66, 75, 80, 85, 90, 99, 108, 116, 126, 128,
        136,
    ...
    while (fgetsp(buffer, buflen, ptr)) {
        ...
        if (cnt % ALLOCINC == 0)
            evidArray =
                (int *) realloc(evidArray, (cnt + ALLOCINC) * sizeof(int));
    ...
    if (!NumAdded) {
        ...
        if (cnt == 1) {         /* one and only one evid in evidArray */
            ...
            while (fgetsp(buffer, buflen, ptr)) {
                ...
                if (Verbose) {
                    css_warning
                        ("No origins match any prefor in the Event table.\n");
                    css_warning
                        ("One origin matched one and only one evid in Wftag table.\n");
                    css_warning("That origin will be loaded.\n");
                }

                /* Handle event */
                if (!EventExists(evid, tree)) {
                    ReadStructureFile(Root, tree, ".event", "Event",
                                      AddMatchingEventStruct);
        ...
        if (cnt == 1) {
            if (Verbose) {
                css_warning
                    ("No origins were matched to wfdisc rows using either evids orig wftags.\n");
                css_warning
                    ("Attempting to add first line in origin file, but cannot guarantee ");
...
static int
OridInOriginList(int orid, DBlist tree) {
    struct originList *o = 0;
    do {
        if (!
            (o =
             (struct originList *) dblNextTableInstance(o, tree,
                                                        dbl_LIST_ORIGIN)))
            break;
        if (o->element->orid == orid)
            return 1;
    } while (o);
    return 0;
}
...
static int
AridInArrivalList(int arid, DBlist tree) {
    struct arrivalList *ar = 0;
    do {
        if (!
            (ar =
             (struct arrivalList *) dblNextTableInstance(ar, tree,
                                                         dbl_LIST_ARRIVAL)))
            break;
        if (ar->element->arid == arid)
            return 1;
    } while (ar);
    return 0;
}
...
static void
AddMatchingAssocStruct(char *buffer, DBlist tree) {
    int asOff[] =
        { 0, 9, 18, 25, 34, 39, 48, 56, 64, 73, 75, 83, 85, 93, 95, 103, 110,
        126, 135
    };
    ...
    if (!OridInOriginList(orid, tree))
        return;
    if (!AridInArrivalList(arid, tree))
        return;

    as = (struct assocList *) dblCreateTableInstance(tree, dbl_LIST_ASSOC);
    as->element->arid = arid;
    as->element->orid = orid;
    strncpy(as->element->sta, buffer + asOff[2], 6);
    Deblank(as->element->sta, 7);
    strncpy(as->element->phase, buffer + asOff[3], 8);
    Deblank(as->element->phase, 9);
    ...
    strncpy(as->element->timedef, buffer + asOff[9], 1);
    Deblank(as->element->timedef, 2);
    as->element->azres = atof(buffer + asOff[10]);
    strncpy(as->element->azdef, buffer + asOff[11], 1);
    Deblank(as->element->azdef, 2);
    as->element->slores = atof(buffer + asOff[12]);
    strncpy(as->element->slodef, buffer + asOff[13], 1);
    Deblank(as->element->slodef, 2);
    as->element->emares = atof(buffer + asOff[14]);
    as->element->wgt = atof(buffer + asOff[15]);
    strncpy(as->element->vmodel, buffer + asOff[16], 15);
    Deblank(as->element->vmodel, 16);
    as->element->commid = atol(buffer + asOff[17]);
    strncpy(as->element->lddate, buffer + asOff[18], 17);
    Deblank(as->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static int
TimeStaChanMatch(double time, char *sta, char *chan, DBlist tree) {
    struct wfdiscList *w = 0;

    if (!CSSchrDefined(sta))
        return 0;
    if (!CSSchrDefined(chan))
        return 0;
...
static void
AddArrivalWftag(DBlist tree, int arid, int wfid) {
    struct wftagList *wt =
        (struct wftagList *) dblCreateTableInstance(tree, dbl_LIST_WFTAG);
    if (!wt)
        return;
...
static void
AddArrivalLine(char *buffer, int *Off, DBlist tree, int arid) {
    struct arrivalList *a =
        (struct arrivalList *) dblCreateTableInstance(tree, dbl_LIST_ARRIVAL);
    if (!a)
        return;

    strncpy(a->element->sta, buffer, 6);
    Deblank(a->element->sta, 7);
    ...
    strncpy(a->element->chan, buffer + Off[6], 8);
    Deblank(a->element->chan, 9);
    strncpy(a->element->iphase, buffer + Off[7], 8);
    Deblank(a->element->iphase, 9);
    strncpy(a->element->stype, buffer + Off[8], 1);
    ...
    strncpy(a->element->clip, buffer + Off[19], 1);
    Deblank(a->element->clip, 2);
    strncpy(a->element->fm, buffer + Off[20], 2);
    Deblank(a->element->fm, 3);
    a->element->snr = atof(buffer + Off[21]);
    strncpy(a->element->qual, buffer + Off[22], 1);
    Deblank(a->element->qual, 2);
    strncpy(a->element->auth, buffer + Off[23], 15);
    Deblank(a->element->auth, 16);
    a->element->commid = atol(buffer + Off[24]);
    strncpy(a->element->lddate, buffer + Off[25], 17);
    Deblank(a->element->lddate, 18);
...
static int
InAuthorList(char *auth, char **Author, int Nauthor) {
    int j;
    if (!Nauthor || !Author)
        return 1;               /* No List means match anything! */

    for (j = 0; j < Nauthor; j++)
        if (!strcmp(auth, Author[j]))
            return 1;
...
static int
InPhaseList(char *iphase, char **Phaselist, int Nphases) {
    int j;
    if (!Phaselist || !Nphases)
        return 1;               /* No List means match anything! */

    for (j = 0; j < Nphases; j++)
        if (!strcmp(iphase, Phaselist[j]))
            return 1;
...
static int
WfidInList(DBlist tree, int wfid) {
    struct wfdiscList *w = 0;
...
static int
WftagJoinExists(DBlist tree, int arid) {
    char Tmp[10];
    struct wftagList *wt = 0;
...
static void
AddMatchingArrivalStruct(char *buffer, DBlist tree, char **Author, int Nauthor,
                         char **Phaselist, int Nphases) {
    int arOff[] =
        { 0, 7, 25, 34, 43, 52, 61, 70, 79, 81, 88, 96, 104, 112, 120, 128, 136,
    ...
    strncpy(sta, buffer, 6);
    Deblank(sta, 7);
    strncpy(chan, buffer + arOff[6], 8);
    Deblank(chan, 9);
    strncpy(auth, buffer + arOff[23], 15);
    Deblank(auth, 16);
    strncpy(iphase, buffer + arOff[7], 8);
    Deblank(iphase, 9);
    if (!InAuthorList(auth, Author, Nauthor))
        return;
    if (!InPhaseList(iphase, Phaselist, Nphases))
        return;

    if (arid < 1)
        arid = dblNextAvailableArid(tree);
...
static void
ReadArrivalFile(char *Root, DBlist tree, char **Author, int Nauthor,
                char **Phaselist, int Nphases) {
    FILE *ptr;
    char *FileName;
    ...
    if (!(ptr = fopen(FileName, "rb"))) {
        ...
        while (fgetsp(buffer, buflen, ptr)) {
            if (!strlen(buffer) || *buffer == '\n')
                continue;
            AddMatchingArrivalStruct(buffer, tree, Author, Nauthor, Phaselist,
                                     Nphases);
        }
...
static int
SiteChanExists(char *sta, char *chan, int ondate, int offdate, DBlist tree) {
    struct sitechanList *s = 0;
    do {
        if (!
            (s =
             (struct sitechanList *) dblNextTableInstance(s, tree,
                                                          dbl_LIST_SITECHAN)))
            break;
        if (!strcmp(sta, s->element->sta) && !strcmp(chan, s->element->chan) &&
            ondate == s->element->ondate && offdate == s->element->offdate)
            return 1;
    } while (s);
    return 0;
}
...
static int
MatchesWfdiscEntry(char *sta, char *chan, int ondate, int offdate, DBlist tree) {
    struct wfdiscList *w = 0;
    do {
        if (!
            (w =
        ...
        if (!strcmp(sta, w->element->sta) && !strcmp(chan, w->element->chan) &&
            ondate <= w->element->jdate && (offdate < 0 ||
                                            offdate >= w->element->jdate))
            return 1;
    } while (w);
    return 0;
}
...
static void
AddMatchingSiteChanStruct(char *buffer, DBlist tree) {
    ...
    strncpy(sta, buffer, 6);
    Deblank(sta, 6);
    strncpy(chan, buffer + Off[1], 8);
    Deblank(chan, 9);
    if (SiteChanExists(sta, chan, ondate, offdate, tree))
        return;
    if (!MatchesWfdiscEntry(sta, chan, ondate, offdate, tree))
        return;
    ...
    strncpy(s->element->ctype, buffer + Off[5], 4);
    Deblank(s->element->ctype, 5);
    s->element->edepth = atof(buffer + Off[6]);
    s->element->hang = atof(buffer + Off[7]);
    s->element->vang = atof(buffer + Off[8]);
    strncpy(s->element->descrip, buffer + Off[9], 50);
    Deblank(s->element->descrip, 51);
    strncpy(s->element->lddate, buffer + Off[10], 17);
    Deblank(s->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static int
SiteExists(char *sta, int ondate, int offdate, DBlist tree) {
    struct siteList *s = 0;
    do {
        if (!
            (s =
             (struct siteList *) dblNextTableInstance(s, tree, dbl_LIST_SITE)))
            break;
        if (!strcmp(sta, s->element->sta) && ondate == s->element->ondate &&
            offdate == s->element->offdate)
            return 1;
    } while (s);
    return 0;
}
...
static int
StaTimeMatchWfdiscEntry(char *sta, int ondate, int offdate, DBlist tree) {
    struct wfdiscList *w = 0;
    ...
    do {
        ...
        if (!strcmp(sta, w->element->sta) && ondate <= w->element->jdate &&
            (offdate < 0 || offdate >= w->element->jdate))
            return 1;
    } while (w);
    return 0;
}
...
static void
AddMatchingSiteStruct(char *buffer, DBlist tree) {
    ...
    strncpy(sta, buffer, 6);
    Deblank(sta, 6);
    if (SiteExists(sta, ondate, offdate, tree))
        return;
    if (!StaTimeMatchWfdiscEntry(sta, ondate, offdate, tree))
        return;
    ...
    strncpy(s->element->staname, buffer + Off[6], 50);
    Deblank(s->element->staname, 51);
    strncpy(s->element->statype, buffer + Off[7], 4);
    Deblank(s->element->statype, 5);
    strncpy(s->element->refsta, buffer + Off[8], 6);
    Deblank(s->element->refsta, 7);
    s->element->dnorth = atof(buffer + Off[9]);
    s->element->deast = atof(buffer + Off[10]);
    strncpy(s->element->lddate, buffer + Off[11], 17);
    Deblank(s->element->lddate, 18);
}
...
static int
AffiliationExists(char *sta, DBlist tree) {
    struct affiliationList *a = 0;
    do {
        if (!
            (a =
             (struct affiliationList *) dblNextTableInstance(a, tree,
                                                             dbl_LIST_AFFILIATION)))
            break;
        if (!strcmp(sta, a->element->sta))
            return 1;
    } while (a);
    return 0;
}
...
static int
StaInSiteList(char *sta, DBlist tree) {
    struct siteList *s = 0;
    do {
        if (!
            (s =
...
static void
AddMatchingAffiliationStruct(char *buffer, DBlist tree) {
    int Off[] = { 0, 9, 16 };
    struct affiliationList *a;
    char sta[6];

    strncpy(sta, buffer + Off[1], 6);
    Deblank(sta, 7);
    if (AffiliationExists(sta, tree))
        return;
    if (!StaInSiteList(sta, tree))
        return;

    a = (struct affiliationList *) dblCreateTableInstance(tree,
                                                          dbl_LIST_AFFILIATION);
    strncpy(a->element->net, buffer, 8);
    Deblank(a->element->net, 9);
    strcpy(a->element->sta, sta);
    strncpy(a->element->lddate, buffer + Off[2], 17);
    Deblank(a->element->lddate, 18);
}
...
static int
OrigerrExists(int orid, DBlist tree) {
    struct origerrList *oe = 0;
    do {
        if (!
            (oe =
             (struct origerrList *) dblNextTableInstance(oe, tree,
                                                         dbl_LIST_ORIGERR)))
            break;
        if (orid == oe->element->orid)
            return 1;
    } while (oe);
    return 0;
}
...
static void
AddMatchingOrigerrStruct(char *buffer, DBlist tree) {
    int Off[] =
        { 0, 9, 25, 41, 57, 73, 89, 105, 121, 137, 153, 169, 179, 189, 199,
        206, 216, 225, 231, 240
    };
    struct origerrList *oe;
    int orid = atol(buffer);

    if (OrigerrExists(orid, tree))
        return;
    if (!OridInOriginList(orid, tree))
        return;
    ...
    strncpy(oe->element->lddate, buffer + Off[19], 17);
    Deblank(oe->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static struct eventList *
EventExists(int evid, DBlist tree) {
    struct eventList *e = 0;
    do {
        if (!
            (e =
             (struct eventList *) dblNextTableInstance(e, tree,
                                                       dbl_LIST_EVENT)))
            break;
        if (evid == e->element->evid)
            return e;
    } while (e);
    return 0;
}
...
static int
EvidInWftagList(int evid, DBlist tree) {
    struct wftagList *wt = 0;
    char tag[9];
    int idx;
    ...
    do {
        ...
        if (!
            ...
            break;
        len =
            strlen(wt->element->tagname) >=
            8 ? 8 : strlen(wt->element->tagname);
        for (idx = 0; idx < len; idx++)
            tag[idx] = toupper(wt->element->tagname[idx]);
        tag[idx] = '\0';
        if (!strcmp(tag, "EVID") && evid == wt->element->tagid)
            return 1;
    } while (wt);
    return 0;
}
...
static void
AddMatchingEventStruct(char *buffer, DBlist tree) {
    ...
    if (EventExists(evid, tree))
        return;
    if (!EvidInWftagList(evid, tree))
        return;

    e = (struct eventList *) dblCreateTableInstance(tree, dbl_LIST_EVENT);
    e->element->evid = evid;
    strncpy(e->element->evname, buffer + Off[1], 15);
    Deblank(e->element->evname, 16);
    e->element->prefor = atol(buffer + Off[2]);
    strncpy(e->element->auth, buffer + Off[3], 15);
    Deblank(e->element->auth, 16);
    e->element->commid = atol(buffer + Off[4]);
    strncpy(e->element->lddate, buffer + Off[5], 17);
    Deblank(e->element->lddate, 18);
...
static int
SensorExists(char *sta, char *chan, double time, double endtime, DBlist tree) {
    struct sensorList *s = 0;
    do {
        if (!
            (s =
             (struct sensorList *) dblNextTableInstance(s, tree,
                                                        dbl_LIST_SENSOR)))
            break;
        if (!strcmp(sta, s->element->sta) && !strcmp(chan, s->element->chan) &&
            time == s->element->time && endtime == s->element->endtime)
            return 1;
    } while (s);
    return 0;
}
...
static int
SensorInWfdiscList(char *sta, char *chan, double time, double endtime,
                   DBlist tree) {
    struct wfdiscList *w = 0;
    do {
        if (!
            ...
            break;
        if (!strcmp(sta, w->element->sta) && !strcmp(chan, w->element->chan) &&
            time <= w->element->time && endtime >= w->element->endtime)
            return 1;
    } while (w);
    return 0;
}
...
static void
AddMatchingSensorStruct(char *buffer, DBlist tree) {
    ...
    strncpy(chan, buffer + Off[1], 8);
    Deblank(chan, 9);
    if (SensorExists(sta, chan, time, endtime, tree))
        return;
    if (!SensorInWfdiscList(sta, chan, time, endtime, tree))
        return;
    ...
    strncpy(s->element->instant, buffer + Off[10], 1);
    Deblank(s->element->instant, 2);
    strncpy(s->element->lddate, buffer + Off[11], 17);
    Deblank(s->element->lddate, 18);
}
...
static int
InstrumentExists(int inid, DBlist tree) {
    struct instrumentList *i = 0;
    do {
        if (!
            (i =
             (struct instrumentList *) dblNextTableInstance(i, tree,
                                                            dbl_LIST_INSTRUMENT)))
            break;
        if (inid == i->element->inid)
            return 1;
    } while (i);
    return 0;
}
...
static int
InidInSensorList(int inid, DBlist tree) {
    struct sensorList *s = 0;
    do {
        if (!
            (s =
...
static void
AddMatchingInstrumentStruct(char *buffer, DBlist tree) {
    int Off[] = { 0, 9, 60, 67, 69, 71, 83, 100, 117, 182, 215, 222 };
    struct instrumentList *i;
    int inid = atol(buffer);

    if (InstrumentExists(inid, tree))
        return;
    if (!InidInSensorList(inid, tree))
        return;
    i = (struct instrumentList *) dblCreateTableInstance(tree,
                                                         dbl_LIST_INSTRUMENT);
    i->element->inid = inid;
    strncpy(i->element->insname, buffer + Off[1], 50);
    Deblank(i->element->insname, 51);
    strncpy(i->element->instype, buffer + Off[2], 6);
    Deblank(i->element->instype, 7);
    strncpy(i->element->band, buffer + Off[3], 1);
    Deblank(i->element->band, 2);
    strncpy(i->element->digital, buffer + Off[4], 1);
    Deblank(i->element->digital, 2);
    i->element->samprate = atof(buffer + Off[5]);
    i->element->ncalib = atof(buffer + Off[6]);
    i->element->ncalper = atof(buffer + Off[7]);
    strncpy(i->element->dir, buffer + Off[8], 64);
    Deblank(i->element->dir, 65);
    strncpy(i->element->dfile, buffer + Off[9], 32);
    Deblank(i->element->dfile, 33);
    strncpy(i->element->rsptype, buffer + Off[10], 6);
    Deblank(i->element->rsptype, 7);
    strncpy(i->element->lddate, buffer + Off[11], 17);
    Deblank(i->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static int
GregionExists(int grn, DBlist tree) {
    struct gregionList *g = 0;
    do {
        if (!
            (g =
             (struct gregionList *) dblNextTableInstance(g, tree,
                                                         dbl_LIST_GREGION)))
            break;
        if (grn == g->element->grn)
            return 1;
    } while (g);
    return 0;
}
...
static int
GregionInOriginList(int grn, DBlist tree) {
    struct originList *orig = 0;
    do {
        if (!
            (orig =
...
static void
AddMatchingGregionStruct(char *buffer, DBlist tree) {
    int Off[] = { 0, 9, 50 };
    struct gregionList *g;
    int grn = atol(buffer);

    if (GregionExists(grn, tree))
        return;
    if (!GregionInOriginList(grn, tree))
        return;

    g = (struct gregionList *) dblCreateTableInstance(tree, dbl_LIST_GREGION);
    g->element->grn = grn;
    strncpy(g->element->grname, buffer + Off[1], 40);
    Deblank(g->element->grname, 41);
    strncpy(g->element->lddate, buffer + Off[2], 17);
    Deblank(g->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static int
StassocExists(int stassid, DBlist tree) {
    struct stassocList *s = 0;
    do {
        if (!
            (s =
             (struct stassocList *) dblNextTableInstance(s, tree,
                                                         dbl_LIST_STASSOC)))
            break;
        if (stassid == s->element->stassid)
            return 1;
    } while (s);
    return 0;
}
...
static int
StassidInArrivalList(int stassid, DBlist tree) {
    struct arrivalList *a = 0;
    do {
        if (!
            (a =
             (struct arrivalList *) dblNextTableInstance(a, tree,
                                                         dbl_LIST_ARRIVAL)))
            break;
        if (stassid == a->element->stassid)
            return 1;
    } while (a);
    return 0;
}
...
static void
AddMatchingStassocStruct(char *buffer, DBlist tree) {
    int Off[] =
        { 0, 9, 16, 24, 57, 65, 73, 83, 93, 103, 121, 129, 137, 145, 161, 170 };
    struct stassocList *s;
    int stassid = atol(buffer);

    if (StassocExists(stassid, tree))
        return;
    if (!StassidInArrivalList(stassid, tree))
        return;

    s = (struct stassocList *) dblCreateTableInstance(tree, dbl_LIST_STASSOC);
    s->element->stassid = stassid;

    strncpy(s->element->sta, buffer + Off[1], 6);
    Deblank(s->element->sta, 7);
    strncpy(s->element->etype, buffer + Off[2], 7);
    Deblank(s->element->etype, 8);
    strncpy(s->element->location, buffer + Off[3], 32);
    Deblank(s->element->location, 33);
    ...
    strncpy(s->element->auth, buffer + Off[13], 15);
    Deblank(s->element->auth, 16);
    s->element->commid = atol(buffer + Off[14]);
    strncpy(s->element->lddate, buffer + Off[15], 17);
    Deblank(s->element->lddate, 18);
}
...
static int
CommidFound(int commid, DBlist tree) {
    struct arrivalList *ar = 0;
    ...
    do {
        if (!
            (as =
             (struct assocList *) dblNextTableInstance(as, tree,
                                                       dbl_LIST_ASSOC)))
            break;
        if (commid == as->element->commid)
            return 1;
    } while (as);
    ...
    do {
        if (!
            (sa =
             (struct stassocList *) dblNextTableInstance(sa, tree,
                                                         dbl_LIST_STASSOC)))
            break;
        if (commid == sa->element->commid)
            return 1;
    } while (sa);
    ...
    do {
        if (!
            (wf =
             (struct wfdiscList *) dblNextTableInstance(wf, tree,
                                                        dbl_LIST_WFDISC)))
            break;
        if (commid == wf->element->commid)
            return 1;
    } while (wf);
...
static int
RemarkExists(int commid, DBlist tree) {
    struct remarkList *r = 0;
    do {
        if (!
            (r =
             (struct remarkList *) dblNextTableInstance(r, tree,
                                                        dbl_LIST_REMARK)))
            break;
        if (commid == r->element->commid)
            return 1;
    } while (r);
    return 0;
}
...
static void
AddMatchingRemarkStruct(char *buffer, DBlist tree) {
    int Off[] = { 0, 9, 18, 99 };
    struct remarkList *r;
    int commid = atol(buffer);

    if (RemarkExists(commid, tree))
        return;

    if (!CommidFound(commid, tree))
        return;

    r = (struct remarkList *) dblCreateTableInstance(tree, dbl_LIST_REMARK);
    r->element->commid = commid;
    r->element->lineno = atol(buffer + Off[1]);
    strncpy(r->element->remark, buffer + Off[2], 80);
    Deblank(r->element->remark, 81);
    strncpy(r->element->lddate, buffer + Off[3], 17);
    Deblank(r->element->lddate, 18);
}

/* ---------------------------------------------------------------------------------- */

static void
AddMatchingSacdataStruct(char *buffer, DBlist tree) {
    int Off[] =
...
static void
ReadStructureFile(char *Root, DBlist tree, char *Suffix, char *StructType,
                  void (*fp) (char *, void *)) {
    FILE *ptr;
    char *FileName;
    char buffer[400];
...
int cssReadFlatFiles(char *wfdiscName, char *WorkSetName, int Replace,
                     int MaxWaveforms, char **Station, int Nstation,
                     char **Channel, int Nchannel, char **Band, int Nband,
                     char **Code, int Ncode, char **Orientation,
                     int Norientation, char **Author, int Nauthor,
                     char **Filelist, int Nfiles, char **Phaselist, int Nphases,
...
int
cssReadFlatFiles(char *wfdiscName, char *WorkSetName, int Replace,
                 int MaxWaveforms, char **Station, int Nstation, char **Channel,
                 int Nchannel, char **Band, int Nband, char **Code, int Ncode,
                 char **Orientation, int Norientation, char **Author,
                 int Nauthor, char **FileList, int Nfiles, char **Phaselist,
                 int Nphases, int verbose, double MaxPhysMem, int *takeEvid) {
    DBlist tree;
    DBlist tree2;
    int Version;
    char *Root;
    ...
    if (!WorkSetName || !strlen(WorkSetName)) {
        css_error
            ("Invalid worksetname! Cannot add CSS ASCII data to workset.\n");
        return 0;
    }
    ...
    if (MaxWaveforms <= TracesInExistingTree) {
        if (Verbose) {
            css_warning
                ("%d traces already in memory! No traces will be read.\n",
    ...
    if (Version == 28) {
        LinesUsed =
            cssRead2_8FlatFiles(Root, WorkSetName, Replace, Station, Nstation,
                                Channel, Nchannel, Band, Nband, Code, Ncode,
                                Orientation, Norientation, Author, Nauthor,
                                FileList, Nfiles, Phaselist, Nphases,
                                MaxWaveforms, Verbose, MaxPhysMemToUse);
        *takeEvid = FALSE;
        return LinesUsed;
    ...
    LinesUsed =
        ReadWfdiscFile(Root, WorkSetName, tree, Station, Nstation, Channel,
                       Nchannel, Band, Nband, Code, Ncode, Orientation,
                       Norientation, Replace, FileList, Nfiles, MaxWaveforms);

    if (!LinesUsed) {
        if (Verbose) {
            css_warning
                ("No rows returned from wfdisc file. Other tables ignored.\n");
        }
        return 0;
    }
    ...
    ReadStructureFile(Root, tree, ".sitechan", "Sitechan",
                      AddMatchingSiteChanStruct);

    ReadStructureFile(Root, tree, ".site", "Site", AddMatchingSiteStruct);

    ReadStructureFile(Root, tree, ".affiliation", "Affiliation",
                      AddMatchingAffiliationStruct);

    ReadStructureFile(Root, tree, ".origerr", "Origerr",
                      AddMatchingOrigerrStruct);

    ReadStructureFile(Root, tree, ".sensor", "Sensor", AddMatchingSensorStruct);

    ReadStructureFile(Root, tree, ".instrument", "Instrument",
                      AddMatchingInstrumentStruct);

    ReadStructureFile(Root, tree, ".gregion", "Gregion",
                      AddMatchingGregionStruct);

    ReadStructureFile(Root, tree, ".stassoc", "Stassoc",
                      AddMatchingStassocStruct);

    ReadStructureFile(Root, tree, ".remark", "Remark", AddMatchingRemarkStruct);

    ReadStructureFile(Root, tree, ".sacdata", "Sacdata",
                      AddMatchingSacdataStruct);
