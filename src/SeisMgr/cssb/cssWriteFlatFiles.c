...
static char *
MakeFileName(const char *basename, const char *tableName) {
    char *fname;
...
static void
StrAppend(char *buffer, char *fmt, int len, char *str) {
    char OneItem[100];
    char msg[100];
...
static void
DblAppend(char *buffer, char *fmt, int len, double val) {
    char OneItem[100];
    char msg[100];
...
static void
LngAppend(char *buffer, char *fmt, int len, int val) {
    char OneItem[100];
    char msg[100];
...
static int
WriteAffiliationRecord(struct affiliationList *af, FILE * ptr) {
    char buffer[50];
...
static int
WriteArrivalRecord(struct arrivalList *ar, FILE * ptr) {
    char buffer[250];
...
static int
WriteAssocRecord(struct assocList *as, FILE * ptr) {
    char buffer[180];
...
static int
WriteEventRecord(struct eventList *ev, FILE * ptr) {
    char buffer[80];
...
static int
WriteGregionRecord(struct gregionList *gr, FILE * ptr) {
    char buffer[80];
...
static int
WriteInstrumentRecord(struct instrumentList *in, FILE * ptr) {
    char buffer[250];
...
static int
WriteOrigerrRecord(struct origerrList *oe, FILE * ptr) {
    char buffer[260];
...
static int
WriteOriginRecord(struct originList *orig, FILE * ptr) {
    char buffer[250];

    if (!CSSdblDefined(orig->element->lat) || !CSSdblDefined(orig->element->lon)
        || !CSSdblDefined(orig->element->depth) ||
        !CSSdblDefined(orig->element->time))
...
static int
WriteRemarkRecord(struct remarkList *re, FILE * ptr) {
    char buffer[120];
...
static int
WriteSensorRecord(struct sensorList *se, FILE * ptr) {
    char buffer[160];
...
static int
WriteSiteRecord(struct siteList *si, FILE * ptr) {
    char buffer[160];
...
static int
WriteSitechanRecord(struct sitechanList *sc, FILE * ptr) {
    char buffer[150];
...
static int
WriteStassocRecord(struct stassocList *sa, FILE * ptr) {
    char buffer[200];
...
static int
WriteWfdiscRecord(const char *basename, struct wfdiscList *w, FILE * ptr) {
    char datafile[100];
    FILE *fptr;
    int ptsOut;
    char buffer[400];
...
static int
WriteWftagRecord(struct wftagList *wt, FILE * ptr) {
    char buffer[50];
...
static int
WriteSacdataRecord(struct sacdataList *sd, FILE * ptr) {
    char buffer[400];
...
static int
HasAffiliationStructs(DBlist tree) {
...
static int
HasInstrumentStructs(DBlist tree) {
...
static int
HasSitechanStructs(DBlist tree) {
...
int
WriteCSSflatFiles(const char *WorkSetName, const char *basename) {
    FILE *ptr;
    struct affiliationList *af = 0;
    struct arrivalList *ar = 0;
    struct assocList *as = 0;
    ...
    if (!WorkSetName || !strlen(WorkSetName)) {
        css_error
            ("Invalid worksetname. Cannot write CSS binary data to file.\n");
        return 0;
    }
    ...
    if (HasAffiliationStructs(tree)) {
        ...
        do {
            if (!
                (af =
                 (struct affiliationList *) dblNextTableInstance(af, tree,
                                                                 dbl_LIST_AFFILIATION)))
                break;
    ...
    if (HasGregionStructs(tree)) {
        ...
        do {
            if (!
                (gr =
                 (struct gregionList *) dblNextTableInstance(gr, tree,
                                                             dbl_LIST_GREGION)))
                break;
    ...
    if (HasInstrumentStructs(tree)) {
        ...
        do {
            if (!
                (in =
                 (struct instrumentList *) dblNextTableInstance(in, tree,
                                                                dbl_LIST_INSTRUMENT)))
                break;
    ...
    if (HasRemarkStructs(tree)) {
        ...
        do {
            if (!
                (re =
                 (struct remarkList *) dblNextTableInstance(re, tree,
                                                            dbl_LIST_REMARK)))
                break;
    ...
    if (HasSensorStructs(tree)) {
        ...
        do {
            if (!
                (se =
                 (struct sensorList *) dblNextTableInstance(se, tree,
                                                            dbl_LIST_SENSOR)))
                break;
    ...
    if (HasSiteStructs(tree)) {
        ...
        do {
            if (!
                (si =
                 (struct siteList *) dblNextTableInstance(si, tree,
                                                          dbl_LIST_SITE)))
                break;
    ...
    if (HasSitechanStructs(tree)) {
        ...
        do {
            if (!
                (sc =
                 (struct sitechanList *) dblNextTableInstance(sc, tree,
                                                              dbl_LIST_SITECHAN)))
                break;
    ...
    if (HasSacdataStructs(tree)) {
        ...
        do {
            if (!
                (sd =
                 (struct sacdataList *) dblNextTableInstance(sd, tree,
                                                             dbl_LIST_SACDATA)))
                break;
