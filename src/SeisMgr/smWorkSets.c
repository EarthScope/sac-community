...
void
smDeleteAllWorkSets(void) {
    struct WorkSet *cur;
    while (WShead) {
        cur = WShead;
        WShead = cur->next;
        if (cur->tree)
            dblDeleteTree(cur->tree);
        free(cur->name);
        free(cur);
    }
    WShead = 0;
...
int
smCreateEmptyWorkset(char *name) {
...
int
smChangeDefaultWorksetByIndex(int index) {
    struct WorkSet *cur;
    cur = WShead;
    while (cur) {
        if (index == cur->index) {
...
int
smChangeDefaultWorksetByName(char *name) {
    struct WorkSet *cur;
    cur = WShead;
    while (cur) {
...
void
smDeleteWorksetByIndex(int index) {
...
void
smDeleteWorksetByName(char *name) {
    struct WorkSet *cur, *prev;
    prev = cur = WShead;
...
int
smChangeWorksetName(char *OldName, char *NewName) {
    struct WorkSet *cur;
