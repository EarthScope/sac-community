...
void
swapShortArray(short *ptr, int NPTS) {
    int idx;
...
void
swapFloatArray(float *vArr, int NPTS) {
    unsigned int idx;
...
void
swapLongArray(int *vArr, int NPTS) {
    unsigned int idx;
    for (idx = 0; idx < (unsigned int) NPTS; ++idx)
        Convert4((void *) (vArr + idx));
}
...
void
swapDoubleArray(double *vArr, int NPTS) {
    unsigned int idx;
    for (idx = 0; idx < (unsigned int) NPTS; ++idx)
...
int
dblGetE1(int NPTS, FILE * fptr, struct wfdiscList *wfStruc) {
...
int
dblGetS2I2G2(int NPTS, FILE * fptr, struct wfdiscList *wfStruc, char *pType) {
    short *shortPtr;
    int idx;
    int ptsRead;
    float *fltPtr;
...
int
dblGetS3(int NPTS, FILE * fptr, struct wfdiscList *wfStruc) {
...
int
dblGetT4F4(int NPTS, FILE * fptr, struct wfdiscList *wfStruc, char *pType) {
    int ptsRead;
    float *fltPtr;
    int bigendin = CheckByteOrder() == ENDIAN_BIG;
...
int
dblGetS4I4(int NPTS, FILE * fptr, struct wfdiscList *wfStruc, char *pType) {
    int jdx;
    int ptsRead, *longPtr;
    float *fltPtr;
    int bigendin = CheckByteOrder() == ENDIAN_BIG;
    ...
    wfStruc->element->nsamp = ptsRead;
    for (jdx = 0; jdx < ptsRead; jdx++)
        *(fltPtr + jdx) = *(longPtr + jdx);
    smFree(longPtr);
    wfStruc->seis->i = fltPtr;
    wfStruc->seis->Cmplx = 0;
...
int
dblGetT8F8(int NPTS, FILE * fptr, struct wfdiscList *wfStruc, char *pType) {
...
int
dblGetRealImagFloats(int NPTS, FILE * fptr, struct wfdiscList *wfStruc) {
    int ptsRead;
    float *fltRPtr;
    float *fltIPtr;
