...
*/
userData *
dblCreateUserDataStruc(DBlist list) {
    userData *newudPntr;
    ...
    if (newudPntr == NULL_UD_LIST) {
        dblSetError(1,
                    "ERROR: Could not allocate User Data struct in  dblCreateUserDataStruc.\n");
...
*/
void
dblDestroyUserDataStruc(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
    ...
    if (tree->UserData->comment)
        dblFreeUserDataComment(list);
    if (tree->UserData->matrix)
        dblDestroyUserDataMatrixData(list);
    if (tree->UserData->dataComment)
        dblDestroyUserDataDataCommentList(list);
...
/* ................ UserData Comment access functions .............. */
   ...
   */
void
dblSetUserDataComment(DBlist list, const char *comment) {
    struct CSStree *tree = (struct CSStree *) list;
    ...
    if (tree->UserData) {
        tree->UserData->comment =
            (char *) smRealloc(tree->UserData->comment, strlen(comment) + 1);
...
*/
char *
dblGetUserDataComment(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
    char *string;
...
*/
static void
dblFreeUserDataComment(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
...
*/
static void
dblFreeMatrix(ComplexFloat ** matrix, int Nrows) {
    int j;
...
*/
static Matrix *
dblAddUserDataMatrixStruc(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;

    if (!tree) {
        dblSetError(1,
                    "ERROR: Null Tree struc in dblAddUserDataMatrixStruc.\n");
        return (Matrix *) NULL;
    }
...
*/
static void
dblDestroyUserDataMatrixData(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
    ...
    if (tree->UserData->matrix->comment)
        smFree(tree->UserData->matrix->comment);
...
*/
void
dblSetUserDataMatrixComment(DBlist list, const char *comment) {
    struct CSStree *tree = (struct CSStree *) list;

    if (!tree) {
        dblSetError(1,
                    "ERROR: Null Tree struc in dblSetUserDataMatrixComment.\n");
        return;
    }
    ...
    if (tree->UserData) {
        if (!tree->UserData->matrix)
            tree->UserData->matrix = dblAddUserDataMatrixStruc(list);
        if (tree->UserData->matrix) {
            if (tree->UserData->matrix->comment)
                smFree(tree->UserData->matrix->comment);
            tree->UserData->matrix->comment =
                (char *) smMalloc(strlen(comment) + 1);
...
*/
char *
dblGetUserDataMatrixComment(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
    char *string;
...
*/
void
dblSetUserDataMatrixData(DBlist list, int Nrows, int Ncols,
                         ComplexFloat ** matrix) {
    struct CSStree *tree = (struct CSStree *) list;
    int j, k;
...
*/
ComplexFloat **
dblGetUserDataMatrixData(DBlist list, int *Nrows, int *Ncols) {
    struct CSStree *tree = (struct CSStree *) list;
    int j, k;
    ComplexFloat **matrix;
    ...
    if (!tree->UserData->matrix->matrix)
        return (ComplexFloat **) NULL;
    if (tree->UserData->matrix->nrows < 1)
        return (ComplexFloat **) NULL;
    if (tree->UserData->matrix->ncols < 1)
        return (ComplexFloat **) NULL;
...
*/
DBdataComm
dblCreateDataComment(DBlist list) {
...
*/
void
dblDeleteUserDataComStruct(DBdataComm DCpntr, DBlist list) {
    DataComment *pntr = (DataComment *) DCpntr;
    struct CSStree *tree = (struct CSStree *) list;

    if (!tree) {
        dblSetError(1,
                    "ERROR: Null Tree struc in dblDeleteUserDataComStruct.\n");
        return;
    }
    if (!pntr)
        return;
...
*/
DBdataComm
dblGetNextDataComment(DBdataComm DCpntr, DBlist list) {
    DataComment *pntr = (DataComment *) DCpntr;
    struct CSStree *tree = (struct CSStree *) list;
    ...
    if (!pntr)
        return (void *) tree->UserData->dataComment;
...
DBdataComm
dblGetNextMatchingDataComment(DBdataComm DCpntr, DBlist list, char *comment) {
    DataComment *pntr = (DataComment *) DCpntr;
    struct CSStree *tree = (struct CSStree *) list;

    if (!tree) {
        dblSetError(1,
                    "ERROR: Null Tree struc in dblGetNextMatchingDataComment.\n");
        return (void *) NULL;
    }
    if (!tree->UserData)
        return (void *) NULL;
    ...
    if (!pntr)
        ...
        pntr = pntr->next;
    while (pntr) {
        if (pntr->comment && !strcmp(pntr->comment, comment))
            return (void *) pntr;
        pntr = pntr->next;
    }
    return (void *) NULL;
...
DBdataComm
dblGetNextDCMatchingIndex(DBdataComm DCpntr, DBlist list, int index) {
    DataComment *pntr = (DataComment *) DCpntr;
    struct CSStree *tree = (struct CSStree *) list;

    if (!tree) {
        dblSetError(1,
                    "ERROR: Null Tree struc in dblGetNextDCMatchingIndex.\n");
        return (void *) NULL;
    }
    if (!tree->UserData)
        return (void *) NULL;
...
void
dblSetDCIndex(DBdataComm DCpntr, int index) {
    DataComment *pntr = (DataComment *) DCpntr;
...
void
dblSetSacUserNum(DBdataComm DCpntr, int index) {
    DataComment *pntr = (DataComment *) DCpntr;
...
int
dblGetDCIndex(DBdataComm DCpntr) {
    DataComment *pntr = (DataComment *) DCpntr;
...
int
dblGetSacUserNum(DBdataComm DCpntr) {
    DataComment *pntr = (DataComment *) DCpntr;
...
void
dblSetDCType(DBdataComm DCpntr, enum dataType type) {
    DataComment *pntr = (DataComment *) DCpntr;
...
enum dataType
dblGetDCType(DBdataComm DCpntr) {
    DataComment *pntr = (DataComment *) DCpntr;
...
*/
DBdataComm
dblGetNextDCSacUserMatch(DBlist list, int index, int Num) {
    DataComment *pntr;
    struct CSStree *tree = (struct CSStree *) list;
    ...
    pntr = tree->UserData->dataComment;
    while (pntr) {
        if (pntr->reference == index && pntr->SacUserNum == Num)
            return (void *) pntr;
        pntr = pntr->next;
    }
    return 0;
...
*/
void
dblSetUserDataComComment(DBdataComm DCpntr, const char *comment) {
    DataComment *pntr = (DataComment *) DCpntr;
...
char *
dblGetUserDataComComment(DBdataComm DCpntr) {
    DataComment *pntr = (DataComment *) DCpntr;
    char *comment;
    if (!pntr)
        return (char *) NULL;
...
*/
void
dblSetUserDataComData(DBdataComm DCpntr, const ComplexFloat * data, int Npts) {
    int j;
    DataComment *pntr = (DataComment *) DCpntr;
    ...
    if (Npts < 1)
        return;
    pntr->data =
        (ComplexFloat *) smRealloc(pntr->data, Npts * sizeof(ComplexFloat));
    pntr->dataLen = Npts;
    for (j = 0; j < Npts; j++)
        pntr->data[j] = data[j];
    return;
...
*/
ComplexFloat *
dblGetUserDataComData(DBdataComm DCpntr, int *Npts) {
    int j;
    DataComment *pntr = (DataComment *) DCpntr;
    ComplexFloat *data;
    if (!pntr)
...
*/
void
dblPrintUserDataInstance(DBdataComm DCpntr, FILE * unit) {
    int j;
    DataComment *pntr = (DataComment *) DCpntr;
    int result = 0;
    char fmt[30];
        ...
        if (result != EOF) {
            if (!pntr->data) {
                ...
                if (pntr->dataLen == 1) {
                    strcpy(fmt, "(%f + %fi)");
                    result =
                        fprintf(unit, fmt, pntr->data[0].real,
                                pntr->data[0].imag);
                    if (result != EOF) {
                        fprintf(unit, "\n");
                        return;
                    ...
                    while (result != EOF && j < pntr->dataLen) {
                        result =
                            fprintf(unit, fmt, pntr->data[j].real,
                                    pntr->data[j].imag);
                        j++;
                    }
                    if (result != EOF) {
    ...
    dblSetError(1,
                "ERROR: Writing file instance in dblPrintUserDataInstance.\n");
    return;
}
...
*/
void
dblPrintUserDataList(DBlist list, FILE * unit) {
    DataComment *DCpntr;
    struct CSStree *tree = (struct CSStree *) list;
...
static void
dblDestroyUserDataDataCommentList(DBlist list) {
    struct CSStree *tree = (struct CSStree *) list;
