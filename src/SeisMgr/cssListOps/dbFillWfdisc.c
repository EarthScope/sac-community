...
static void
StripDuplicateSeparators(char *strg) {
    char *ptr;
...
int
dblGetSeismograms(struct wfdiscList *wfStruc, char *kpath1, char *kpath2) {
    ...
    if (!kpath1) {
        ...
        stringLen =
            strlen(wfStruc->element->dir) + strlen(wfStruc->element->dfile) +
            10;
    ...
    if (check == 1) {
        /* handle error */
        strcpy(errorStrg,
               "ERROR: Could not allocate data in dblGetSeismogram.\n");
        if (filename)
            smFree(filename);
        dbPrintToDevice(errorStrg);
        return FALSE;
