...
void
enlarge(FILE * fp, int outMax, int *data, /* int * byteOff, */ int *nerr)
/********************************************************************************
 *
...
static void
b_lshift(unsigned int *input, int nwords, int size) {
    register int i;
    unsigned int ltmp, rtmp;
    int left = size;
...
static int
b_lval(unsigned int input, int size) {
...
static int
e_decompress(unsigned int *in, int *out) {
    register int i, j, k;
    int nd, ns;
    int ms;
    int check;
        ...
        case 0:
            ...
            while (ms < ns) {
                ...
                for (j = 0; j < samples_per_block[idx]; j++) {
                    ...
                    b_lshift(&in[i], words_per_block[idx],
                             bits_per_sample[idx]);
                }
