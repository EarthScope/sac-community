...
static void dblPrintAFvalsToString(struct affiliationList *afPntr,
                                   int *attribute, int nCols, char *tmpstr);
...
static void dblPrintGRvalsToString(struct gregionList *grPntr, int *attribute,
                                   int nCols, char *tmpstr);
static void dblPrintINvalsToString(struct instrumentList *inPntr,
                                   int *attribute, int nCols, char *tmpstr);
...
void
dblPrintTableInstance(DBtable tableInstance, dblObject specifier, FILE * unit) {
    ...
    switch (specifier) {
        case dbl_LIST_AFFILIATION:{
                ...
                strcpy(fmt,
                       "(net = %8s) \n(sta = %6s) \n(lddate = %17s) \n(index = %5d)\n");
                if (fprintf
                    (unit, fmt, afPntr->element->net, afPntr->element->sta,
                     afPntr->element->lddate, afPntr->index) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
                break;
            }
        ...
        case dbl_LIST_ARRIVAL:{
                ...
                strcat(fmt, "\n(commid = %8d) \n(lddate = %17s)\n");
                if (fprintf
                    (unit, fmt, arPntr->element->sta,
                     tmListEpochTime(arPntr->element->time, 14),
                     arPntr->element->arid, arPntr->element->jdate,
                     arPntr->element->stassid, arPntr->element->chanid,
                     arPntr->element->chan, arPntr->element->iphase,
                     arPntr->element->stype, arPntr->element->deltim,
                     arPntr->element->azimuth, arPntr->element->delaz,
                     arPntr->element->slow, arPntr->element->delslo,
                     arPntr->element->ema, arPntr->element->rect,
                     arPntr->element->amp, arPntr->element->per,
                     arPntr->element->logat, arPntr->element->clip,
                     arPntr->element->fm, arPntr->element->snr,
                     arPntr->element->qual, arPntr->element->auth,
                     arPntr->element->commid, arPntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_ASSOC:{
                ...
                strcat(fmt,
                       "\n(vmodel = %16s) \n(commid = %8d) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, asPntr->element->arid, asPntr->element->orid,
                     asPntr->element->sta, asPntr->element->phase,
                     asPntr->element->belief, asPntr->element->delta,
                     asPntr->element->seaz, asPntr->element->esaz,
                     asPntr->element->timeres, asPntr->element->timedef,
                     asPntr->element->azres, asPntr->element->azdef,
                     asPntr->element->slores, asPntr->element->slodef,
                     asPntr->element->emares, asPntr->element->wgt,
                     asPntr->element->vmodel, asPntr->element->commid,
        ...
        case dbl_LIST_EVENT:{
                ...
                strcat(fmt, "\n(commid = %8d) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, evPntr->element->evid, evPntr->element->evname,
                     evPntr->element->prefor, evPntr->element->auth,
                     evPntr->element->commid, evPntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_GREGION:{
                ...
                strcpy(fmt,
                       "(grn = %8d) \n(grname = %41s) \n(lddate = %18s) \n ");
                if (fprintf
                    (unit, fmt, grPntr->element->grn, grPntr->element->grname,
        ...
        case dbl_LIST_INSTRUMENT:{
                ...
                strcat(fmt,
                       "\n(dfile = %33s) \n(rsptype = %7s) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, inPntr->element->inid, inPntr->element->insname,
                     inPntr->element->instype, inPntr->element->band,
                     inPntr->element->digital, inPntr->element->samprate,
                     inPntr->element->ncalib, inPntr->element->ncalper,
                     inPntr->element->dir, inPntr->element->dfile,
        ...
        case dbl_LIST_ORIGERR:{
                ...
                strcat(fmt,
                       "\n(conf = %8f) \n(commid = %8d) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, oePntr->element->orid, oePntr->element->sxx,
                     oePntr->element->syy, oePntr->element->szz,
                     oePntr->element->stt, oePntr->element->sxy,
                     oePntr->element->sxz, oePntr->element->syz,
                     oePntr->element->stx, oePntr->element->sty,
                     oePntr->element->stz, oePntr->element->sdobs,
                     oePntr->element->smajax, oePntr->element->sminax,
                     oePntr->element->strike, oePntr->element->sdepth,
                     oePntr->element->stime, oePntr->element->conf,
                     oePntr->element->commid, oePntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_ORIGIN:{
                ...
                strcat(fmt,
                       "\n(auth = %16s) \n(commid = %8d) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, orPntr->element->lat, orPntr->element->lon,
                     orPntr->element->depth,
                     tmListEpochTime(orPntr->element->time, 14),
                     orPntr->element->orid, orPntr->element->evid,
                     orPntr->element->jdate, orPntr->element->nass,
                     orPntr->element->ndef, orPntr->element->ndp,
                     orPntr->element->grn, orPntr->element->srn,
                     orPntr->element->etype, orPntr->element->depdp,
                     orPntr->element->dtype, orPntr->element->mb,
                     orPntr->element->mbid, orPntr->element->ms,
                     orPntr->element->msid, orPntr->element->ml,
                     orPntr->element->mlid, orPntr->element->algorithm,
                     orPntr->element->auth, orPntr->element->commid,
        ...
        case dbl_LIST_REMARK:{
                ...
                strcat(fmt, "\n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, rePntr->element->commid,
                     rePntr->element->lineno, rePntr->element->remark,
        ...
        case dbl_LIST_SENSOR:{
                ...
                strcat(fmt,
                       "\n(tshift = %8f) \n(instant = %2s) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, sePntr->element->sta, sePntr->element->chan,
                     tmListEpochTime(sePntr->element->time, 14),
                     tmListEpochTime(sePntr->element->endtime, 14),
                     sePntr->element->inid, sePntr->element->chanid,
                     sePntr->element->jdate, sePntr->element->calratio,
                     sePntr->element->calper, sePntr->element->tshift,
        ...
        case dbl_LIST_SITE:{
                ...
                strcat(fmt,
                       "\n(dnorth = %8f) \n(deast = %8f) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, slPntr->element->sta, slPntr->element->ondate,
                     slPntr->element->offdate, slPntr->element->lat,
                     slPntr->element->lon, slPntr->element->elev,
                     slPntr->element->staname, slPntr->element->statype,
                     slPntr->element->refsta, slPntr->element->dnorth,
                     slPntr->element->deast, slPntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_SITECHAN:{
                ...
                strcat(fmt,
                       "\n(vang = %8f) \n(descrip = %51s) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, scPntr->element->sta, scPntr->element->chan,
                     scPntr->element->ondate, scPntr->element->chanid,
                     scPntr->element->offdate, scPntr->element->ctype,
                     scPntr->element->edepth, scPntr->element->hang,
                     scPntr->element->vang, scPntr->element->descrip,
        ...
        case dbl_LIST_STASSOC:{
                ...
                strcat(fmt,
                       "\n(auth = %16s) \n(commid = %8d) \n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, saPntr->element->stassid, saPntr->element->sta,
                     saPntr->element->etype, saPntr->element->location,
                     saPntr->element->dist, saPntr->element->azimuth,
                     saPntr->element->lat, saPntr->element->lon,
                     saPntr->element->depth,
                     tmListEpochTime(saPntr->element->time, 14),
                     saPntr->element->imb, saPntr->element->ims,
                     saPntr->element->iml, saPntr->element->auth,
                     saPntr->element->commid, saPntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_WFDISC:{
                ...
                if (fprintf
                    (unit, fmt, wfPntr->element->sta, wfPntr->element->chan,
                     timestr, wfPntr->element->wfid, wfPntr->element->chanid,
                     wfPntr->element->jdate, timestr2, wfPntr->element->nsamp,
                     wfPntr->element->samprate, wfPntr->element->calib,
                     wfPntr->element->calper, wfPntr->element->instype,
                     wfPntr->element->segtype, wfPntr->element->dattype,
                     wfPntr->element->clip, wfPntr->element->dir,
                     wfPntr->element->dfile, wfPntr->element->foff,
                     wfPntr->element->commid, wfPntr->element->lddate) == EOF) {
                    dblSetError(1,
                                "ERROR: Error writing file instance in dblPrintTableInstance.\n");
                    return;
                }
        ...
        case dbl_LIST_WFTAG:{
                ...
                strcat(fmt, "\n(lddate = %18s) \n");
                if (fprintf
                    (unit, fmt, wtPntr->element->tagname,
                     wtPntr->element->tagid, wtPntr->element->wfid,
        ...
        case dbl_LIST_USERDATA:
            AFFI_CASES ARRIVAL_CASES ASSOC_CASES EVENT_CASES GR_CASES INS_CASES
                NET_CASES ORIGERR_CASES ORIGIN_CASES REMA_CASES SENS_CASES
                SITE_CASES SITECHAN_CASES SR_CASES STASSOC_CASES WFDISC_CASES
                WFTAG_CASES SACD_CASES break;
    }
...
void
dblDumpTable(DBlist list, dblObject StrucType, FILE * unit, ...) {
...
static void
dblPrintARvalsToString(struct arrivalList *arPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_ARRIV_AMP:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(arPntr,
                                                          dbl_ARRIV_AMP));
            ...
            case dbl_ARRIV_ARID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(arPntr, dbl_ARRIV_ARID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ARRIV_AUTH:{
                    sprintf(buffer, "%16s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_AUTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_ARRIV_CHAN:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_CHAN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ARRIV_CLIP:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_CLIP));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ARRIV_COMMID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(arPntr,
                                                       dbl_ARRIV_COMMID));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_ARRIV_FM:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_FM));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ARRIV_IPHASE:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(arPntr,
                                                       dbl_ARRIV_IPHASE));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_ARRIV_QUAL:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_QUAL));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ARRIV_RECT:{
            ...
            case dbl_ARRIV_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(arPntr, dbl_ARRIV_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
...
static void
dblPrintARheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numARvals = 26;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numARvals; j++)
            attribute[j] = dbl_ARRIV_AMP + j;
        *nCols = numARvals;
    }
...
static void
dblPrintAFvalsToString(struct affiliationList *afPntr, int *attribute,
                       int nCols, char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_AFFI_NET:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(afPntr, dbl_AFFI_NET));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_AFFI_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(afPntr, dbl_AFFI_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_AFFI_LDDATE:{
                    sprintf(buffer, "%18s",
                            (char *) dblGetTableObject(afPntr,
                                                       dbl_AFFI_LDDATE));
                    strcat(tmpstr, buffer);
                    break;
                }
...
static void
dblPrintAFheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numAFvals = 3;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numAFvals; j++)
            attribute[j] = dbl_AFFI_NET + j;
        *nCols = numAFvals;
    }
...
static void
dblPrintASheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numASvals = 19;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numASvals; j++)
            attribute[j] = dbl_ASSOC_ARID + j;
        *nCols = numASvals;
    }
...
static void
dblPrintASvalsToString(struct assocList *asPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_ASSOC_ARID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(asPntr, dbl_ASSOC_ARID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ASSOC_AZDEF:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(asPntr,
                                                       dbl_ASSOC_AZDEF));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ASSOC_AZRES:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(asPntr,
                                                          dbl_ASSOC_AZRES));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_ASSOC_ORID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(asPntr, dbl_ASSOC_ORID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ASSOC_PHASE:{
            ...
            case dbl_ASSOC_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(asPntr, dbl_ASSOC_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ASSOC_TIMEDEF:{
...
static void
dblPrintEVheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numEVvals = 6;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numEVvals; j++)
            attribute[j] = dbl_EVENT_AUTH + j;
        *nCols = numEVvals;
    }
...
static void
dblPrintEVvalsToString(struct eventList *evPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_EVENT_AUTH:{
                    sprintf(buffer, "%16s",
                            (char *) dblGetTableObject(evPntr, dbl_EVENT_AUTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_EVENT_COMMID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(evPntr,
                                                       dbl_EVENT_COMMID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_EVENT_EVID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(evPntr, dbl_EVENT_EVID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_EVENT_EVNAME:{
                    sprintf(buffer, "%16s",
                            (char *) dblGetTableObject(evPntr,
                                                       dbl_EVENT_EVNAME));
                    strcat(tmpstr, buffer);
                    break;
                }
...
static void
dblPrintGRheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numGRvals = 3;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numGRvals; j++)
            attribute[j] = dbl_GR_GRN + j;
        *nCols = numGRvals;
    }
...
static void
dblPrintGRvalsToString(struct gregionList *grPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_GR_GRN:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(grPntr, dbl_GR_GRN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_GR_GRNAME:{
                    sprintf(buffer, "%41s",
                            (char *) dblGetTableObject(grPntr, dbl_GR_GRNAME));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_GR_LDDATE:{
                    sprintf(buffer, "%18s",
                            (char *) dblGetTableObject(grPntr, dbl_GR_LDDATE));
                    strcat(tmpstr, buffer);
                    break;
                }
        }
...
static void
dblPrintINheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numINvals = 12;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numINvals; j++)
            attribute[j] = dbl_INS_BAND + j;
        *nCols = numINvals;
    }
...
static void
dblPrintINvalsToString(struct instrumentList *inPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_INS_BAND:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(inPntr, dbl_INS_BAND));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_INS_DFILE:{
                    sprintf(buffer, "%33s",
                            (char *) dblGetTableObject(inPntr, dbl_INS_DFILE));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_INS_DIGITAL:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(inPntr,
                                                       dbl_INS_DIGITAL));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_INS_DIR:{
                    sprintf(buffer, "%65s",
                            (char *) dblGetTableObject(inPntr, dbl_INS_DIR));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_INS_INID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(inPntr, dbl_INS_INID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_INS_INSNAME:{
            ...
            case dbl_INS_NCALIB:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(inPntr,
                                                          dbl_INS_NCALIB));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_INS_LDDATE:{
                    sprintf(buffer, "%18s",
                            (char *) dblGetTableObject(inPntr, dbl_INS_LDDATE));
                    strcat(tmpstr, buffer);
                    break;
                }
        }
...
static void
dblPrintOEheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numOEvals = 20;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numOEvals; j++)
            attribute[j] = dbl_ORIGE_COMMID + j;
        *nCols = numOEvals;
    }
...
static void
dblPrintOEvalsToString(struct origerrList *oePntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            ...
            case dbl_ORIGE_CONF:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(oePntr,
                                                          dbl_ORIGE_CONF));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGE_ORID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(oePntr, dbl_ORIGE_ORID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGE_SDEPTH:{
...
static void
dblPrintORheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numORvals = 25;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numORvals; j++)
            attribute[j] = dbl_ORIGI_ALGORITHM + j;
        *nCols = numORvals;
    }
...
static void
dblPrintORvalsToString(struct originList *orPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            ...
            case dbl_ORIGI_AUTH:{
                    sprintf(buffer, "%16s",
                            (char *) dblGetTableObject(orPntr, dbl_ORIGI_AUTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_COMMID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr,
                                                       dbl_ORIGI_COMMID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_DEPDP:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(orPntr,
                                                          dbl_ORIGI_DEPDP));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_ORIGI_EVID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_EVID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_GRN:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_GRN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_JDATE:{
            ...
            case dbl_ORIGI_MBID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_MBID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_ML:{
            ...
            case dbl_ORIGI_MLID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_MLID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_MS:{
            ...
            case dbl_ORIGI_MSID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_MSID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_NASS:{
            ...
            case dbl_ORIGI_NDEF:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_NDEF));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_NDP:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_NDP));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_ORID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_ORID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_SRN:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(orPntr, dbl_ORIGI_SRN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_ORIGI_TIME:{
...
static void
dblPrintREvalsToString(struct remarkList *rePntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
...
static void
dblPrintSEheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numSEvals = 12;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numSEvals; j++)
            attribute[j] = dbl_SENS_CALPER + j;
        *nCols = numSEvals;
    }
...
static void
dblPrintSEvalsToString(struct sensorList *sePntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_SENS_CALPER:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(sePntr,
                                                          dbl_SENS_CALPER));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_SENS_CHAN:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(sePntr, dbl_SENS_CHAN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SENS_CHANID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(sePntr,
                                                       dbl_SENS_CHANID));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_SENS_INID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(sePntr, dbl_SENS_INID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SENS_INSTANT:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(sePntr,
                                                       dbl_SENS_INSTANT));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SENS_JDATE:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(sePntr, dbl_SENS_JDATE));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SENS_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(sePntr, dbl_SENS_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SENS_TIME:{
...
static void
dblPrintSLheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numSEvals = 12;
    char buffer[30];
    *tmpstr = 0;
...
static void
dblPrintSLvalsToString(struct siteList *slPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_SITE_DEAST:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(slPntr,
                                                          dbl_SITE_DEAST));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_SITE_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(slPntr, dbl_SITE_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SITE_STANAME:{
                    sprintf(buffer, "%51s",
                            (char *) dblGetTableObject(slPntr,
                                                       dbl_SITE_STANAME));
                    strcat(tmpstr, buffer);
                    break;
                }
...
static void
dblPrintSCheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numSCvals = 11;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numSCvals; j++)
            attribute[j] = dbl_SITEC_CHAN + j;
        *nCols = numSCvals;
    }
...
static void
dblPrintSCvalsToString(struct sitechanList *scPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_SITEC_CHAN:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(scPntr, dbl_SITEC_CHAN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SITEC_CHANID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(scPntr,
                                                       dbl_SITEC_CHANID));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_SITEC_EDEPTH:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(scPntr,
                                                          dbl_SITEC_EDEPTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_SITEC_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(scPntr, dbl_SITEC_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SITEC_VANG:{
...
static void
dblPrintSAheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numSAvals = 16;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numSAvals; j++)
            attribute[j] = dbl_STASS_AUTH + j;
        *nCols = numSAvals;
    }
...
static void
dblPrintSAvalsToString(struct stassocList *saPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_STASS_AUTH:{
                    sprintf(buffer, "%16s",
                            (char *) dblGetTableObject(saPntr, dbl_STASS_AUTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_STASS_AZIMUTH:{
                    sprintf(buffer, "%16f",
                            *(double *) dblGetTableObject(saPntr,
                                                          dbl_STASS_AZIMUTH));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_STASS_STA:{
                    sprintf(buffer, "%7s",
                            (char *) dblGetTableObject(saPntr, dbl_STASS_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_STASS_STASSID:{
...
static void
dblPrintWFheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numWFvals = 20;
    char buffer[100];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numWFvals; j++)
            attribute[j] = dbl_WFDIS_CALIB + j;
        *nCols = numWFvals;
    }
...
static void
dblPrintWFvalsToString(struct wfdiscList *wfPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            case dbl_WFDIS_CALIB:{
                    sprintf(buffer, "%12f",
                            *(float *) dblGetTableObject(wfPntr,
                                                         dbl_WFDIS_CALIB));
                    strcat(tmpstr, buffer);
                    break;
                }
            ...
            case dbl_WFDIS_CHAN:{
                    sprintf(buffer, "%9s",
                            (char *) dblGetTableObject(wfPntr, dbl_WFDIS_CHAN));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_CHANID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(wfPntr,
                                                       dbl_WFDIS_CHANID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_CLIP:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(wfPntr, dbl_WFDIS_CLIP));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_COMMID:{
            ...
            case dbl_WFDIS_DIR:{
                    sprintf(buffer, "%65s",
                            (char *) dblGetTableObject(wfPntr, dbl_WFDIS_DIR));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_ENDTIME:{
            ...
            case dbl_WFDIS_FOFF:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(wfPntr, dbl_WFDIS_FOFF));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_INSTYPE:{
            ...
            case dbl_WFDIS_STA:{
                    sprintf(buffer, "%8s",
                            (char *) dblGetTableObject(wfPntr, dbl_WFDIS_STA));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_TIME:{
            ...
            case dbl_WFDIS_WFID:{
                    sprintf(buffer, "%12d",
                            *(int *) dblGetTableObject(wfPntr, dbl_WFDIS_WFID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFDIS_LDDATE:{
...
static void
dblPrintWTheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numWTvals = 20;
    char buffer[100];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numWTvals; j++)
            attribute[j] = dbl_WFTAG_TAGNAME + j;
        *nCols = numWTvals;
    }
...
static void
dblPrintWTvalsToString(struct wftagList *wtPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            ...
            case dbl_WFTAG_WFID:{
                    sprintf(buffer, "%8d",
                            *(int *) dblGetTableObject(wtPntr, dbl_WFTAG_WFID));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_WFTAG_LDDATE:{
...
static void
dblPrintSDheaderToString(int *attribute, int *nCols, char *tmpstr) {
    int j;
    int numSDvals = 6;
    char buffer[30];
    *tmpstr = 0;
    ...
    if (*nCols < 1) {
        for (j = 0; j < numSDvals; j++)
            attribute[j] = dbl_SACD_SYNTHFLG + j;
        *nCols = numSDvals;
    }
...
static void
dblPrintSDvalsToString(struct sacdataList *sdPntr, int *attribute, int nCols,
                       char *tmpstr) {
    char buffer[100];
    int j;
    ...
    *tmpstr = 0;
    for (j = 0; j < nCols; j++) {
        switch (attribute[j]) {
            ...
            case dbl_SACD_IDEP:{
                    sprintf(buffer, "%8d",
                            *(int *) dblGetTableObject(sdPntr, dbl_SACD_IDEP));
                    strcat(tmpstr, buffer);
                    break;
                }
            case dbl_SACD_IFTYPE:{
            ...
            case dbl_SACD_WFID:{
                    sprintf(buffer, "%8d",
                            *(int *) dblGetTableObject(sdPntr, dbl_SACD_WFID));
                    strcat(tmpstr, buffer);
                    break;
                }
        }
