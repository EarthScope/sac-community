...
void
matSetHeaderTimes(struct hdrTimes *times) {
    int j;
...
void
matUpdateTimes(struct hdrTimes times) {
    int j;
...
void
matSetHeaderStation(struct hdrStation *station) {
    station->stla = *stla;
    station->stlo = *stlo;
    station->stel = *stel;
    station->stdp = *stdp;
...
void
matUpdateStation(struct hdrStation station) {
    *stla = station.stla;
    *stlo = station.stlo;
    *stel = station.stel;
    *stdp = station.stdp;
...
void
matSetHeaderEvent(struct hdrEvent *event) {
    event->evla = s->h->evla;
    event->evlo = s->h->evlo;
    event->evel = s->h->evel;
    event->evdp = s->h->evdp;
...
void
matUpdateEvent(struct hdrEvent event) {
    s->h->evla = event.evla;
    s->h->evlo = event.evlo;
    s->h->evel = event.evel;
    s->h->evdp = event.evdp;
...
void
matSetHeaderUser(struct hdrUser *user) {
    int j;
    for (j = 0; j < 10; j++) {
        user[j].data = *(user0 + j);
        if (j < 3)
...
void
matUpdateUser(struct hdrUser *user) {
    int j;
    for (j = 0; j < 10; j++) {
        *(user0 + j) = user[j].data;
        if (j < 3)
...
void
matSetHeaderDataDescrip(struct hdrDataDescrip *descrip, int index) {
    int ic1, ic2;
    char *tmp;
    sac *s;
...
void
matUpdateDataDescrip(struct hdrDataDescrip descrip) {
...
void
matSetHeaderEvsta(struct hdrEventSta *evsta) {
    evsta->dist = *dist;
    evsta->az = *az;
    evsta->baz = *baz;
    evsta->gcarc = *gcarc;
...
void
matUpdateHeaderEvsta(struct hdrEventSta evsta) {
    *dist = evsta.dist;
    *az = evsta.az;
    *baz = evsta.baz;
    *gcarc = evsta.gcarc;
...
void
matSetHeaderLLNL(struct hdrLLNLextensions *llnl) {
...
void
matUpdateHeaderLLNL(struct hdrLLNLextensions llnl) {
...
void
matSetHeaderDepMec(struct hdrDepMec *depmec) {
...
void
matUpdateHeaderDepMec(struct hdrDepMec depmec) {
...
void
matSetHeaderResponse(double *response) {
    int j;
    for (j = 0; j < 10; j++) {
        response[j] = *(resp0 + j);
    }
...
void
matUpdateResponse(double *response) {
    int j;
    for (j = 0; j < 10; j++) {
        *(resp0 + j) = response[j];
    }
...
int
matAddBlock(int index) {
    if (index == 0)
        return TRUE;
    if (index % ARRAY_BLK_SIZE)
        return FALSE;
    return TRUE;
}
...
int
matAddSACdataElement(int index, float *DatafloatPtrReal,
                     float *DatafloatPtrImag, int nlen) {
    int newSize, j;
    ...
    if (matAddBlock(index)) {
        ...
        SACdataArray =
            (struct SACdata *) realloc(SACdataArray,
                                       newSize * sizeof(struct SACdata));
    ...
    if (SACdataArray[index].trcReal == (double *) NULL) {
        fprintf(stderr,
                "ERROR: Could not allocate real array in AddSACdataElement.");
        exit(-1);
    }
    for (j = 0; j < nlen; j++)
    ...
    if (s->h->iftype == IRLIM || s->h->iftype == IAMPH) {
        ...
        if (SACdataArray[index].trcImag == (double *) NULL) {
            fprintf(stderr,
                    "ERROR: Could not allocate imaginary array in AddSACdataElement.");
            exit(-1);
        }
        for (j = 0; j < nlen; j++)
...
int
matUpdateSACfromSACdataArray(int index, float *DatafloatPtrReal,
                             float *DatafloatPtrImag, int nlen) {
    int newSize, j;
...
int
matGetMaxTraceLen(void) {
    int j;
    int maxLen;
    maxLen = 0;

    for (j = 0; j < NumberofTraces; j++) {
        if (SACdataArray[j].traceLength > maxLen)
            maxLen = SACdataArray[j].traceLength;
    }
    return maxLen;
}

double *
matGetSeisRealPtr(int index) {
    return SACdataArray[index].trcReal;
}

double *
matGetSeisImagPtr(int index) {
    return SACdataArray[index].trcImag;
}

int
matGetSeisLength(int index) {
    return SACdataArray[index].traceLength;
}

struct hdrTimes *
matGetTimesPtr(int index) {
    return &(SACdataArray[index].times);
}

struct hdrStation *
matGetStationPtr(int index) {
    return &(SACdataArray[index].station);
}

struct hdrEvent *
matGetEventPtr(int index) {
    return &(SACdataArray[index].event);
}

struct hdrUser *
matGetUserPtr(int index) {
    return SACdataArray[index].user;
}

struct hdrDataDescrip *
matGetDescripPtr(int index) {
    return &(SACdataArray[index].descrip);
}

double *
matGetResponsePtr(int index) {
    return SACdataArray[index].response;
}

double *
matGetTraceLengthPtr(int index) {
    return &(SACdataArray[index].traceLength);
}

struct hdrEventSta *
matGetEvstaPtr(int index) {
    return &(SACdataArray[index].evsta);
}

struct hdrLLNLextensions *
matGetLLNLPtr(int index) {
    return &(SACdataArray[index].llnl);
}

struct hdrDepMec *
matGetDepMecPtr(int index) {
...
void
__matHeaderDataManage_undef_symbol() {
}
