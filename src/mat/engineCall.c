...
#include "matFuncInternal.h"
    ...
    if (DbleData == (double *) NULL) {
        fprintf(stderr,
                "ERROR: Could not allocate memory for buffer array. \n");
        exit(1);
    }
    ...
    if (StringData == (char *) NULL) {
        fprintf(stderr,
                "ERROR: Could not allocate memory for string array. \n");
        exit(1);
    }
    ...
    for (j = 1; j <= NumSets; j++) {
        ...
        if (dataPntr == (float *) NULL) {
            fprintf(stderr,
                    "ERROR: Could not get pointer for HORIZ1-Comp data \n");
            exit(1);
        }
        for (k = 0; k < NPTS; k++)
            DbleData[k] = dataPntr[k];
        memcpy(MxGetPr(SeisData) + (j - 1) * 3 * MaxDataLen, DbleData,
               NPTS * sizeof(double));

        /* Next copy float and int data into 2nd matlab array... */
        matCopyHeaderValues(HORIZ1, j, double2);
        memcpy(MxGetPr(HeaderData) + (j - 1) * 3 * NHEADERVALS, double2,
               NHEADERVALS * sizeof(double));
        ...
        if (dataPntr == (float *) NULL) {
            fprintf(stderr,
                    "ERROR: Could not get pointer for HORIZ2-Comp data \n");
            exit(1);
        }
        for (k = 0; k < NPTS; k++)
            DbleData[k] = dataPntr[k];
        memcpy(MxGetPr(SeisData) + (3 * j - 2) * MaxDataLen, DbleData,
               NPTS * sizeof(double));

        /* Next copy float and int data into 2nd matlab array... */
        matCopyHeaderValues(HORIZ2, j, double2);
        memcpy(MxGetPr(HeaderData) + (3 * j - 2) * NHEADERVALS, double2,
               NHEADERVALS * sizeof(double));
        ...
        if (dataPntr == (float *) NULL) {
            fprintf(stderr,
                    "ERROR: Could not get pointer for Vert-Comp data \n");
            exit(1);
        }
        for (k = 0; k < NPTS; k++)
            DbleData[k] = dataPntr[k];
        memcpy(MxGetPr(SeisData) + (3 * j - 1) * MaxDataLen, DbleData,
               NPTS * sizeof(double));

        /* Next copy float and int data into 2nd matlab array... */
        matCopyHeaderValues(VERTICAL, j, double2);
        memcpy(MxGetPr(HeaderData) + (3 * j - 1) * NHEADERVALS, double2,
               NHEADERVALS * sizeof(double));
    ...
    for (j = 1; j <= NumSets; j++) {
        ...
        if (SeisData2) {
            memcpy(DbleData, MxGetPr(SeisData2) + (j - 1) * 3 * MaxDataLen,
                   NPTS * sizeof(double));
            for (k = 0; k < NPTS; k++)
        ...
        if (HeaderData2) {
            memcpy(double2, (MxGetPr(HeaderData2) + (j - 1) * 3 * NHEADERVALS),
                   NHEADERVALS * sizeof(double));
        ...
        if (MatlabString2)
            matCopyStringsToHeader(HORIZ1, j,
                                   StringData + (j - 1) * 3 * STRINGLEN);
        ...
        if (SeisData2) {
            memcpy(DbleData, MxGetPr(SeisData2) + (3 * j - 2) * MaxDataLen,
                   NPTS * sizeof(double));
            for (k = 0; k < NPTS; k++)
                dataPntr[k] = DbleData[k];
        }
        ...
        if (HeaderData2) {
            memcpy(double2, (MxGetPr(HeaderData2) + (3 * j - 2) * NHEADERVALS),
                   NHEADERVALS * sizeof(double));
        ...
        /* Next copy string data from StringData array... */
        if (MatlabString2)
            matCopyStringsToHeader(HORIZ2, j,
                                   StringData + (3 * j - 2) * STRINGLEN);

        /* For vertical channel ... */

        /* Copy float and int data into 2nd matlab array... */
        if (HeaderData2) {
            memcpy(double2, (MxGetPr(HeaderData2) + (3 * j - 1) * NHEADERVALS),
                   NHEADERVALS * sizeof(double));
        ...
        /* Next copy string data from StringData array... */
        if (MatlabString2)
            matCopyStringsToHeader(VERTICAL, j,
                                   StringData + (3 * j - 1) * STRINGLEN);
