...
matValidatePath(void) {
    ...
    if (Nlen == 1 && matdir[0] == '.')
        return FALSE;
    if (access(matdir, F_OK | X_OK))
        return FALSE;
    return TRUE;
}

static void
RemoveDotMfromName(char *mfile) {
    char *loc;

    if (loc = strstr(mfile, ".m"))
        *loc = 0;
    return;
}
...
static char *
CreateDepMecMfilePathString(void) {
    char *temp = 0;
    char *mDepMec = 0;
    int len;
...
static char *
CreateEnginePathCommand(char *base) {
    int PathLength;
    char *MatlabPath = 0;
...
static char *
PrependEnginePathCommand(char *base) {
    int PathLength;
    char *MatlabPath = 0;
...
static int
ExecuteCommands(Engine * ep, char *ErrorBuffer) {
    int i;
    ...
    input[0] = '\0';
    printf
        ("     'closemat' or 'quit' or 'q'   to return to SAC and quit MATLAB. \n");
    printf
        ("     'exitmat'  or 'exit' or 'sac' to return to SAC and leave MATLAB engine running. \n");
    printf("     'inspector' to view workspace variables. \n");
    do {
        select_loop(SAC_MAT_PROMPT, strlen(SAC_MAT_PROMPT), input,
                    INBUF_LEN - 1, NULL, process_matlab_line, TRUE, TRUE);

        for (i = 0; i < sizeof(EXIT) / sizeof(char *); i++) {
            if (strncmp(input, EXIT[i], strlen(EXIT[i])) == 0) {
                fprintf(stderr,
                        "Type 'mat' to return to your Matlab session\n");
                free(input);
                input = NULL;
                return FALSE;
            }
...
static void
FillBlackBoardStruct(mxArray * BlackBoard) {
    int j;
...
static void
UpdateBlackBoardStruct(mxArray * BlackBoard) {
    int j;
    struct BlackBoardVars *bbPtr;
#  include "matFuncInternal.h"
    int Nrows, Ncols;
        ...
        switch (bbPtr->type) {
            case INT_BBVAL:
                if (MxIsNumeric(value))
                    bbPtr->intval = MxGetScalar(value);
                break;
            case FLOAT_BBVAL:
                if (MxIsNumeric(value))
                    bbPtr->floatval = MxGetScalar(value);
                break;
            case STRING_BBVAL:
                if (MxIsChar(value))
                    MxGetString(value, bbPtr->stringval, MxGetN(value) + 1);
                break;
        }
    }
}
...
static int
ReturnedDataOK(mxArray * SeisData, mxArray * structPtr) {
...
#  include "matFuncInternal.h"
    ...
    if (!MxIsDouble(SeisData)) {
        printf
            ("Data type for SeisData was changed in Matlab! No update in SAC.\n");
        return FALSE;
    }
...
static void
UpdateStructArray(mxArray * structPtr) {
    int nchans = matGetNumChans();
    mxArray *field_value;
    int j;
#  include "matFuncInternal.h"
    ...
    for (j = 0; j < nchans; j++) {
        ...
        if (field_value)
            matUpdateTimesStruct(field_value, matGetTimesPtr(j));

        /* update station struct */
        field_value = MxGetField(structPtr, j, "station");
        if (field_value)
            matUpdateStationStruct(field_value, matGetStationPtr(j));

        /* update event struct  */
        field_value = MxGetField(structPtr, j, "event");
        if (field_value)
            matUpdateEventStruct(field_value, matGetEventPtr(j));

        /* update user struct array  */
        field_value = MxGetField(structPtr, j, "user");
        if (field_value)
            matUpdateUserStruct(field_value, matGetUserPtr(j));

        /* update descrip struct  */
        field_value = MxGetField(structPtr, j, "descrip");
        if (field_value)
            matUpdateDescripStruct(field_value, matGetDescripPtr(j));

        /* update evsta struct  */
        field_value = MxGetField(structPtr, j, "evsta");
        if (field_value)
            matUpdateEvstaStruct(field_value, matGetEvstaPtr(j));

        /* update llnl struct */
        field_value = MxGetField(structPtr, j, "llnl");
        if (field_value)
            matUpdateLLNLStruct(field_value, matGetLLNLPtr(j));

        /* update depmec struct */
        field_value = MxGetField(structPtr, j, "depmec");
        if (field_value)
            matUpdateDepMecStruct(field_value, matGetDepMecPtr(j));
...
static void
UpdateSeisArray(mxArray * SeisData) {
    int nchans = matGetNumChans();
    int MaxDataLen = matGetMaxTraceLen();
    int Nlen;
    mxArray *field_value;
...
#  include "matFuncInternal.h"
    ...
    for (j = 0; j < nchans; j++) {
        ...
        memcpy((void *) matGetSeisRealPtr(j),
               (void *) (MxGetPr(SeisData) + j * MaxDataLen),
               Nlen * sizeof(double));
        if (MxIsComplex(SeisData))
            memcpy((void *) matGetSeisImagPtr(j),
                   (void *) (MxGetPi(SeisData) + j * MaxDataLen),
                   Nlen * sizeof(double));
...
static void
CopyDataToMatlabArrays(mxArray * SeisData, mxArray * structPtr) {
    int nchans = matGetNumChans();
    int MaxDataLen = matGetMaxTraceLen();
    int Nlen;
    mxArray *field_value;
...
#	include "matFuncInternal.h"
    ...
    if (NumBBvars > 0) {
        ...
        if (!
            (BlackBoard =
             MxCreateStructArray(ndim, dims, nBBfields, bbFieldNames))) {
            fprintf(stderr,
    ...
    if (NumBBvars > 0) {
        ...
        if (BlackBoard2)
            UpdateBlackBoardStruct(BlackBoard2);
    }
