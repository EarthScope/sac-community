...
void
matAddToBlackboardList(char *nameBuffer, char *valBuffer) {
...
void
matGetBlackboardVars() {        /* copy blackboard variables from old SAC internal storage into
    ...
    while (keys[i]) {
        ...
        if (!strcmp(v->name, "SACERROR"))
            continue;
        if (!strcmp(v->name, "NUMERROR"))
            continue;
        if (!strcmp(v->name, "SACNFILES"))
            continue;
        switch (v->type) {
            case VAR_VALUE:
...
void
matSetBlackboardVars() {        /* copy blackboard variables to old SAC internal storage from
                                   a linked list where each struc holds the name, type, and value */
    struct BlackBoardVars *cur;
    int index;
    int nerr = 0;
...
void
__matBlackBoardVars_undef_symbol() {
}
