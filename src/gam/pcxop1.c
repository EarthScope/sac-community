...
pcxop1(int iop1) {
    int ilines, j;
    float arrowd, arrowl, dtheta, hgap, hloc, hloc1, hloc2, hloc3, hloc4,
        radius, slope, theta, vgap, vloc, vloc1, vloc2, vloc3, vloc4, wtick, x,
        xa1, xa2, xlen, y, ya1, ya2, ylen;
  ...
  L_5600:
    radius =
        sqrt(powi(cmgam.xori - cmgam.xcdp, 2) +
             powi(cmgam.yori - cmgam.ycdp, 2));
  ...
  L_5700:
    radius =
        sqrt(powi(cmgam.xcdp - cmgam.xori, 2) +
             powi(cmgam.ycdp - cmgam.yori, 2));
  ...
  L_5800:
    ...
    if (cmgam.lbdrrq && cmgam.nhtick > 0) {
        hgap =
            (cmgem.view.xmax - cmgem.view.xmin - 2. * VSMALL) / (cmgam.nhtick +
                                                                 1);
        hloc = hgap;
    ...
    if (cmgam.lbdrrq && cmgam.nvtick > 0) {
        vgap =
            (cmgem.view.ymax - cmgem.view.ymin - 2. * VSMALL) / (cmgam.nvtick +
                                                                 1);
        vloc = vgap;
