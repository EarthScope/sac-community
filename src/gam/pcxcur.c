...
pcxcur(FILE * nunrpl) {
  ...
  L_5000:
    ...
    if (kchar == kmgam.kopmac) {
        ...
        sprintf(kmsg, "%c%10.5f%10.5f", kmgam.kopmac, cmgam.xcdp, cmgam.ycdp);
    ...
    if (kchar == kmgam.kopbe) {
      ...
      L_5500:
        ...
        if (iope < cmgam.nopei) {
            ...
            if (nerr == 0) {
                jope = jope + 1;
                worldcursor(&cmgam.xcdpe, &cmgam.ycdpe,
                            &kmgam.kopetx[jope - 1]);
    ...
    if (iop1 == 1) {
        ...
        sprintf(kmsg, "%c%10.5f%10.5f", kchar2, cmgam.xcdp, cmgam.ycdp);
