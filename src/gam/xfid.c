     ...
     int *nerr;
{
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        if (lclog(&cmgam.lfidrq)) {     /* do nothing */
        }

        /* -- "TYPE DEFAULT/STD/NAME/LIST list":  change type of file id. */
        else if (lklist
                 ("TYPE$", 6, (char *) kmgam.kfidtp, 9, cmgam.nfidtp,
                  &cmgam.ifidtp)) {
        ...
        else if (lklist("LOCATION$", 10, (char *) kmgam.kfidlc, 9, cmgam.nfidlc, &cmgam.ifidlc)) {      /* do nothing */
        }

        /* -- "FORMAT EQUALS/COLONS/NONAMES":  change format of display. */
        else if (lklist("FORMAT$", 8, (char *) kmgam.kfidfm, 9, cmgam.nfidfm, &cmgam.ifidfm)) { /* do nothing */
        }
