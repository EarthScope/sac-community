     ...
     int *nerr;
{
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        else if (lklist("TYPE$", 6, (char *) kmscm.krgltp, 9, MRGLTP, &cmscm.irgltp)) { /* do nothing */
        }

        /* -- "WINDOW rtw": define time window in which to remove glitches. */
        else if (lkrtw("WINDOW$", 8, &cmscm.lrglwin, (char *) kmscm.krglwin, 9, cmscm.orglwin)) {       /* do nothing */
        }

        /* -- "METHOD method":  select method of detecting.: */
        else if (lklist("METHOD$", 8, (char *) kmscm.krglmt, 9, MRGLMT, &cmscm.irglmt)) {       /* do nothing */
        }

        /* -- Bad syntax. */

        else if (lkreal("SWINLEN$", 9, &sWinLen)) {     /* do nothing */
        }

        else if (lkreal("THRESH2$", 9, &Thresh2)) {     /* do nothing */
        }

        else if (lkreal("MINAMP$", 9, &MinGlitchAmp)) { /* do nothing */
        }
