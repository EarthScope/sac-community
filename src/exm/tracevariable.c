 ...
 */
void
tracevariable(int activate, int blackboard, char *variable, int *nerr) {
    ...
    *nerr = 0;
    ...
    if (activate) {
      ...
      L_100:
        if (memcmp
            (variable, cmexm.ktracename[jtrace - 1],
             min(strlen(variable),
                 strlen(cmexm.ktracename[jtrace - 1]))) == 0) {
            ...
            fstrncpy(cmexm.ktracename[jtrace - 1], 16, variable,
                     strlen(variable));
            Lblackboard[jtrace] = blackboard;
            if (Lblackboard[jtrace]) {
                getbbv((char *) cmexm.ktracename[jtrace - 1],
                       (char *) cmexm.ktracevalue[jtrace - 1]
                       , &ngerr, 16, MCMSG);
            } else {
                gethv((char *) cmexm.ktracename[jtrace - 1], 17,
                      (char *) cmexm.ktracevalue[jtrace - 1]
                      , MCMSG + 1, &ngerr);
            }
            setmsg("OUTPUT", 99);
        ...
        for (jtrace = 1; jtrace <= cmexm.ntraces; jtrace++) {
            jtrace_ = jtrace - 1;
            if (memcmp
                (variable, cmexm.ktracename[jtrace_],
                 min(strlen(variable),
                     strlen(cmexm.ktracename[jtrace_]))) == 0) {
                if (Lblackboard[jtrace]) {
                    getbbv((char *) cmexm.ktracename[jtrace_],
                           (char *) cmexm.ktracevalue[jtrace_]
                           , &ngerr, 16, MCMSG);
                } else {
                    gethv((char *) cmexm.ktracename[jtrace_], 17,
                          (char *) cmexm.ktracevalue[jtrace_]
                          , MCMSG + 1, &ngerr);
                }
                setmsg("OUTPUT", 99);
