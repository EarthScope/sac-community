    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        /* -- "CONTENTS list": Define the contents of this transcript file */
                 ...
                 ("CO#NTENTS$", 11, (char *) kmmsg.ktpmsg, 9, MTPMSG,
                  lcontents)) {
            lnewcontents = TRUE;
        }
    ...
    if (lnewname) {
      ...
      L_2000:
        if (memcmp
            (kname, kmexm.knametranscript[jdx - 1],
             min(strlen(kname), strlen(kmexm.knametranscript[jdx - 1]))) == 0) {
            if (lnewcontents) {
                for (jtpmsg = 0; jtpmsg < MTPMSG; jtpmsg++) {
                    cmexm.lsendtranscript[jdx - 1][jtpmsg] = lcontents[jtpmsg];
                }
            }
