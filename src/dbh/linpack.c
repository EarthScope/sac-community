...
void htridi(int nm, int n, double *ar, double *ai, double *d, double *e,
            double *e2, double tau[][2]);
void htribk(int nm, int n, double *ar, double *ai, double tau[][2], int m,
            double *zr, double *zi);
 ...
 * */
void
ceigvv(int nd, int n, complexf * a, float *e, complexf * z, int *ierr) {
 ...
 * */
void
htridi(int nm, int n, double *ar, double *ai, double *d, double *e, double *e2,
       double tau[][2]) {
    ...
    for (ii = 1; ii <= n; ii++) {
      ...
      L_140:
        for (k = 1; k <= l; k++) {
            ...
            h = h + AR(k_, i - 1) * AR(k_, i - 1) + AI(k_, i - 1) * AI(k_,
                                                                       i - 1);
        }
        ...
        if (f == 0.0e0)
            goto L_160;
        tau[l - 1][0] =
            (AI(l - 1, i - 1) * tau[i - 1][1] -
             AR(l - 1, i - 1) * tau[i - 1][0]) / f;
        si = (AR(l - 1, i - 1) * tau[i - 1][1] +
              AI(l - 1, i - 1) * tau[i - 1][0]) / f;
        h = h + f * g;
        g = 1.0e0 + g / f;
      ...
      L_170:
        ...
        for (j = 1; j <= l; j++) {
            ...
            for (k = 1; k <= j; k++) {
                ...
                gi = gi - AR(k_, j_) * AI(k_, i - 1) + AI(k_, j_) * AR(k_,
                                                                       i - 1);
            }
            ...
            for (k = jp1; k <= l; k++) {
                ...
                gi = gi - AR(j_, k_) * AI(k_, i - 1) - AI(j_, k_) * AR(k_,
                                                                       i - 1);
            }
            /*     .......... FORM ELEMENT OF P ..........  */
          L_220:
        ...
        for (j = 1; j <= l; j++) {
            ...
            for (k = 1; k <= j; k++) {
                k_ = k - 1;
                AR(k_, j_) =
                    AR(k_, j_) - f * E[k] - g * AR(k_,
                                                   i - 1) + fi * tau[k_][1] +
                    gi * AI(k_, i - 1);
                AI(k_, j_) =
                    AI(k_, j_) - f * tau[k_][1] - g * AI(k_,
                                                         i - 1) - fi * E[k] -
                    gi * AR(k_, i - 1);
            }
        }
...
void
htribk(int nm, int n, double *ar, double *ai, double tau[][2], int m,
       double *zr, double *zi) {
#define AR(I_,J_)	(*(ar+(I_)*(nm)+(J_)))
#define AI(I_,J_)	(*(ai+(I_)*(nm)+(J_)))
...
void
tql2(int nm, int n, double *d, double *e, double *z, int *ierr) {
