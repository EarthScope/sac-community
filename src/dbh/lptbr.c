 ...
 */
void
lptbr(complexf * p, complexf * z, char *rtype, int rtype_s, double dcvalue,
    ...
    *nsects = 0;
    ...
    for (i = 1; i <= n; i++) {
        ...
        if (memcmp(RTYPE(i_, 0), "CPZ", 3) == 0) {
            ...
            ctemp =
                cmplxsub(cmplxpow
                         ((cmplxmul(flttocmplx(b, 0.), cinv)), (double) 2),
                         flttocmplx(4. * a, 0.));
            ctemp = cmplxsqrt(ctemp);
            z1 = cmplxmul(flttocmplx(0.5, 0.),
                          (cmplxadd(cmplxmul(flttocmplx(b, 0.), cinv), ctemp)));
            z2 = cmplxmul(flttocmplx(0.5, 0.),
                          (cmplxsub(cmplxmul(flttocmplx(b, 0.), cinv), ctemp)));
