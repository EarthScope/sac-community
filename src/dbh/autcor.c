...
void
autcor(float *data, double delta, int nsamps, int nwin, int wlen, char *type,
       char *stype, float *ac, int *nfft, int *nlags, char *err, int err_s,
       float *aux, float ridge_fac) {
    char temp[131];
    ...
    *nfft = 8;
    ...
    while (wcntr < nwin) {
        ...
        for (j = 1; j <= half; j++) {
            ...
            Ac[1 + j] =
                Ac[1 + j] + (powi(xr, 2) + powi(xi, 2)) + (powi(yr, 2) +
                                                           powi(yi, 2));
            Ac[1 + k] = Ac[1 + j];
        }
