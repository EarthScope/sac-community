...
/*        loop to store the data  */
    ...
    if ((nchars - copycount) > 0) {
        memcpy((char *) &Mfbuf[cmgd2.jfbpnt], &data[copycount],
               nchars - copycount);
        cmgd2.jfbpnt += (nchars - copycount) / 2;
        if ((nchars - copycount) % 2)
            cmgd2.jfbpnt += 1;
        flushbuffer2(nerr);
    }
...
void
show_image_sgf(float *data, unsigned int iw, unsigned int ih, float xmin,
    ...
    cdata =
        fill_image2(iph, ipw, data_image, zmin, zmax - zmin, npseudocolors,
