...
text_box_sgf(textbox * t) {
    ...
    if (t->use_style && t->location & TEXT_BOX_LEFT) {
        x += (0.5 * cmgem.chwid) +
            (TEXTBOX_LINE_LENGTH_PIXELS) / (SGF_PIXELS(XW));
    }
    ...
    for (i = 0; i < t->n; i++) {
        ...
        if (t->use_style) {
            text_box_line(&sgf, x, y, t->width[i], t->style[i], cmgem.chwid,
                          cmgem.chht);
        }
        if (t->use_symbol) {
            text_box_symbol(&sgf, x, y, t->symbol[i], t->use_style, cmgem.chwid,
                            cmgem.chht);
        }
        y = y - cmgem.chht;
    }
