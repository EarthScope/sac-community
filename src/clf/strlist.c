...
int
string_list_put(string_list * list, char *str, int len) {
    if (!list) {
        return FALSE;
    }
...
int
string_list_find(string_list * list, char *str, int len) {
    int i, k;
    char *tmp;
...
int
string_list_delete(string_list * list, int n) {
    if (n < 0 || n >= list->n) {
        return FALSE;
    }
    free(list->strings[n]);
    list->strings[n] = NULL;
    if (n < list->n - 1) {
        memmove(list->strings + n, list->strings + n + 1,
...
char *
string_list_get(string_list * list, int n) {
    if (n < 0) {
        n = n + list->n;
    }
    if (n >= 0 && n < list->n) {
...
void
string_list_extend(string_list * list, string_list * add) {
    int i;
    char *tmp;
    for (i = 0; i < add->n; i++) {
