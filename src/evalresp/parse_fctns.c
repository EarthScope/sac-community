...
int
parse_pref(int *blkt_no, int *fld_no, char *line) {
...
void
parse_pz(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_pz; %s%s%s%2.2d",
                     "(return_field) fld ",
...
void
parse_iir_coeff(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    int i, blkt_typ, check_fld;
    int blkt_read;
    int ncoeffs, ndenom;
    char field[MAXFLDLEN], line[MAXLINELEN];
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_coeff; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 or F05",
                     ", fld_found=F", FirstField);
    }
...
void
parse_coeff(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    int i, blkt_typ, check_fld;
    int blkt_read;
    int ncoeffs, ndenom;
    char field[MAXFLDLEN], line[MAXLINELEN];
...
void
parse_list(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    int i, blkt_typ = LIST;
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_list; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 or F05",
                     ", fld_found=F", FirstField);
    }
...
void
parse_generic(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_generic; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 or F05",
                     ", fld_found=F", FirstField);
    }
...
int
parse_deci(FILE * fptr, struct blkt *blkt_ptr) {
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_deci; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 or F05",
                     ", fld_found=F", FirstField);
    }
...
int
parse_gain(FILE * fptr, struct blkt *blkt_ptr) {
    int i, blkt_typ = GAIN;
    int blkt_read, check_fld;
    int sequence_no = 0;
    int nhist = 0;
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_gain; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 of F05",
                     ", fld_found=F", FirstField);
    }
...
void
parse_fir(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    int i, blkt_typ;
    int blkt_read, check_fld;
    int ncoeffs;
    char field[MAXFLDLEN], line[MAXLINELEN];
    ...
    if (FirstField != 3 && FirstField != 5) {
        error_return(PARSE_ERROR, "parse_fir; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03 or F05",
                     ", fld_found=F", FirstField);
    }
...
void
parse_ref(FILE * fptr, struct blkt *blkt_ptr, struct stage *stage_ptr) {
    ...
    if (FirstField != 3) {
        error_return(PARSE_ERROR, "parse_ref; %s%s%s%2.2d",
                     "(return_field) fld ",
                     "number does not match expected value\n\tfld_xpt=F03",
                     ", fld_found=F", FirstField);
    }
    parse_field(FirstLine, 0, field);
    if (!is_int(field))
        error_return(PARSE_ERROR, "parse_ref; value '%s' %s", field,
        ...
        if (i != (nstages - 1)) {       /* if not the last stage in this filter */
            ...
            if (lcl_nstages != nstages) {
                error_return(PARSE_ERROR,
                             ...
                             "\n\tstage expected = ", nstages,
                             ", stage found = ", lcl_nstages);
            }
            blkt_ptr->blkt_info.reference.num_stages = nstages;
...
int
parse_channel(FILE * fptr, struct channel *chan) {
    ...
    while ((FirstField =
            next_line(fptr, FirstLine, &blkt_no, &fld_no, ":")) != 0 &&
           blkt_no != 50) {
        switch (blkt_no) {
            case 53:
                blkt_ptr = alloc_pz();
        ...
        if (blkt_no != 60) {
            ...
            if (!read_blkt++) {
                ...
                if (this_stage != (struct stage *) NULL &&
                    tmp_stage2->next_stage != (struct stage *) NULL) {
...
int
get_channel(FILE * fptr, struct channel *chan) {
    int blkt_no, fld_no;
    char field[MAXFLDLEN], line[MAXLINELEN];
...
next_resp(FILE * fptr) {
    ...
    while ((test = check_line(fptr, &blkt_no, &fld_no, tmp_line)) != 0 &&
           blkt_no != 50);
