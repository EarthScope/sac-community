...
ev_parse_line(char *line) {
    ...
    if (nfields > 0) {
        ...
        for (i = 0; i < nfields; i++) {
            ...
            if ((lcl_strings->strings[i] =
                 (char *) malloc(fld_len * sizeof(char))) == (char *) NULL) {
        ...
        if ((lcl_strings->strings[0] =
             (char *) malloc(sizeof(char))) == (char *) NULL) {
            error_exit(OUT_OF_MEMORY,
                       "ev_parse_line; malloc() failed for (char) vector");
        }
...
struct string_array *
parse_delim_line(char *line, char *delim) {
    char *lcl_line, field[MAXFLDLEN];
    int nfields, fld_len, i = 0;
    struct string_array *lcl_strings;
...
int
get_field(FILE * fptr, char *return_field, int blkt_no, int fld_no, char *sep,
          int fld_wanted) {
...
int
get_line(FILE * fptr, char *return_line, int blkt_no, int fld_no, char *sep) {
...
int
next_line(FILE * fptr, char *return_line, int *blkt_no, int *fld_no, char *sep) {
    char *lcl_ptr, line[MAXLINELEN];
    int test;
    int tmpint;
    char tmpstr[200];
...
count_delim_fields(char *line, char *delim) {
    ...
    while (*lcl_ptr &&
           (tmp_ptr = strstr((lcl_ptr + line_len), delim)) != (char *) NULL) {
...
int
parse_field(char *line, int fld_no, char *return_field) {
    char *lcl_ptr, *new_ptr;
...
int
parse_delim_field(char *line, int fld_no, char *delim, char *return_field) {
...
/* SBH - 2004.079 added code to skip blank lines */
int
check_line(FILE * fptr, int *blkt_no, int *fld_no, char *in_line) {
    char line[MAXLINELEN];
    int test;
    char tmpstr[200];
    int tmpint;
...
int
string_match(const char *string, char *expr, char *type_flag) {
    ...
    if ((prog = evr_regcomp(regexp_pattern)) == NULL) {
        error_return(RE_COMP_FAILED,
                     "string_match; pattern '%s' didn't compile",
                     regexp_pattern);
    }
    lcl_ptr = lcl_string;
