...
calc_resp(struct channel *chan, double *freq, int nfreqs,
          ...
          int stop_stage, int useTotalSensitivityFlag) {
    ...
    for (i = 0; i < nfreqs; i++) {
        ...
        for (j = 0; j < chan->nstages; j++) {
            ...
            if (start_stage >= 0 && stop_stage &&
                (stage_ptr->sequence_no < start_stage ||
                 stage_ptr->sequence_no > stop_stage)) {
                stage_ptr = stage_ptr->next_stage;
                continue;
            } else if (start_stage >= 0 && !stop_stage &&
                       stage_ptr->sequence_no != start_stage) {
                stage_ptr = stage_ptr->next_stage;
                continue;
            }
            ...
            while (blkt_ptr) {
                ...
                switch (blkt_ptr->type) {
                    ...
                    case LAPLACE_PZ:
                        ...
                        break;
                    case IIR_PZ:
                        if (blkt_ptr->blkt_info.pole_zero.nzeros ||
                            blkt_ptr->blkt_info.pole_zero.npoles) {
 ...
 *=================================================================*/
void
convert_to_units(int inp, char *out_units, struct complex *data, double w) {
    int out, l;
 ...
 *================================================================*/
void
iir_trans(struct blkt *blkt_ptr, double wint, struct complex *out) {
 ...
 *===============================================================*/
void
calc_list(struct blkt *blkt_ptr, int i, struct complex *out) {
    double amp, phase;
    double halfcirc = 180;
 ...
 *=================================================================*/
void
analog_trans(struct blkt *blkt_ptr, double freq, struct complex *out) {
    int nz, np, i;
 ...
 *=================================================================*/
void
fir_sym_trans(struct blkt *blkt_ptr, double w, struct complex *out) {
    double *a, h0, wsint;
    struct blkt *next_ptr;
    int na;
    int k, fact;
 ...
 *=================================================================*/
void
fir_asym_trans(struct blkt *blkt_ptr, double w, struct complex *out) {
    double *a, h0, sint;
    struct blkt *next_ptr;
    int na;
    int k;
 ...
 *=================================================================*/
void
iir_pz_trans(struct blkt *blkt_ptr, double w, struct complex *out) {
 ...
 *=================================================================*/
void
calc_time_shift(double delta, double w, struct complex *out) {
 ...
 *=================================================================*/
void
zmul(struct complex *val1, struct complex *val2) {
    double r, i;
...
*=================================================================*/
void
norm_resp(struct channel *chan, int start_stage, int stop_stage) {
    struct stage *stage_ptr;
                ...
                case GAIN:
                    if (curr_seq_no) {
                        if ((fil->blkt_info.gain.gain_freq != chan->sensfreq) ||
                            ((main_type == ANALOG_PZ || main_type == LAPLACE_PZ
                              || main_type == IIR_PZ) &&
                             (main_filt->blkt_info.pole_zero.a0_freq !=
                              chan->sensfreq))) {

                            reset_gain = 1;
                            if (main_type == ANALOG_PZ ||
                                main_type == LAPLACE_PZ) {
                                main_filt->blkt_info.pole_zero.a0 = 1.0;
                                analog_trans(main_filt,
                                             fil->blkt_info.gain.gain_freq,
                                             &df);
                                ...
                                iir_trans(main_filt,
                                          twoPi * fil->blkt_info.gain.gain_freq,
                                          &df);
                            ...
                            if (reset_gain) {
                                fil->blkt_info.gain.gain /= sqrt(df.real *
                                                                 df.real +
                                                                 df.imag *
                                                                 df.imag);
                                fil->blkt_info.gain.gain *=
                                    sqrt(of.real * of.real + of.imag * of.imag);
                                fil->blkt_info.gain.gain_freq = f;
                                if (main_type == ANALOG_PZ ||
                                    main_type == LAPLACE_PZ ||
                                    main_type == IIR_PZ) {
                                    main_filt->blkt_info.pole_zero.a0 =
                                        1.0 / sqrt(of.real * of.real +
                                                   of.imag * of.imag);
                                    main_filt->blkt_info.pole_zero.a0_freq = f;
                                } else if (main_type == FIR_SYM_1 ||
                                           main_type == FIR_SYM_2 ||
                                           main_type == FIR_ASYM) {
                                    main_filt->blkt_info.fir.h0 =
                                        1.0 / sqrt(of.real * of.real +
                                                   of.imag * of.imag);
                                } else if (main_type == IIR_COEFFS) {
                                    main_filt->blkt_info.coeff.h0 =
                                        1.0 / sqrt(of.real * of.real +
                                                   of.imag * of.imag);
                                }
                            }
