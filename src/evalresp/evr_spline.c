...
*/
char *
evr_spline(int num_points, double *t, double *y, double tension, double k,
    ...
    if (num_points < 2 || num_xvals <= 0) {     /* less than 2 "source" datapoints or no "new" abscissa points */
    ...
    *p_num_retvals = 0;         /* initialize count */
    ...
    for (i = 0; i < num_xvals; ++i) {   /* for each value in "new" abscissa array */
        ...
        if ((x - t[0]) * (x - t[used]) <= 0) {  /* calculate value, enter into array, inc index */
            (*p_retvals_arr)[(*p_num_retvals)++] =
                interpolate(used, t, y, z, x, tension, false);
        } else
            range_count++;
    }
    free(z);                    /* free array for 2nd derivatives */
    ...
    if (range_count > 0) {
        return
            "One or more requested points could not be computed (out of data range)";
    }
...
fit(int n, double *t, double *y, double *z, double k, double tension,
    ...
    if (!periodic) {
        ...
        z[n - 1] =
            (v[n - 1] + vv[n - 1]) / (u[n - 1] + uu[n - 1] + 2 * s[n - 1]);
        for (i = n - 2; i >= 1; --i)
...
double
interpolate(int n, double *t, double *y, double *z, double x, double tension,
            bool periodic) {
    ...
    if (tension == 0.0)
        ...
        value = y[i]
            + diff * ((y[i + 1] - y[i]) / h -
                      h * (z[i + 1] + z[i] * 2.0) / 6.0 + diff * (0.5 * z[i] +
                                                                  diff *
                                                                  (z[i + 1] -
                                                                   z[i]) /
                                                                  (6.0 * h)));
        ...
        /* `positive' (really real) tension, use sinh's */
    {
        if (fabs(tension * h) < TRIG_ARG_MIN)
            ...
            value =
                (y[i] * relupdiff + y[i + 1] * reldiff + ((z[i] * h * h / 6.0)
                                                          *
                                                          quotient_sinh_func
                                                          (relupdiff,
                                                           tension * h))
...
quotient_sinh_func(double x, double y) {
    return ((x * x * x - x) +
            (x * x * x * x * x / 20.0 - x * x * x / 6.0 +
             7.0 * x / 60.0) * (y * y)
            + (x * x * x * x * x * x * x / 840.0 - x * x * x * x * x / 120.0 +
               7.0 * x * x * x / 360.0 - 31.0 * x / 2520.0) * (y * y * y * y));
}
