...
void
merge_lists(struct blkt *first_blkt, struct blkt **second_blkt) {
...
void
merge_coeffs(struct blkt *first_blkt, struct blkt **second_blkt) {
    int new_ncoeffs, ncoeffs1, ncoeffs2, i, j;
...
void
check_channel(struct channel *chan) {
            ...
            switch (blkt_ptr->type) {
                ...
                case FIR_COEFFS:
                    error_return(UNSUPPORT_FILTYPE,
                                 "check_channel; unsupported filter type");
                case LIST:
...
/* First we merge blocketes if the list spans for more than a single blockette */
                    ...
                    while (next_blkt != (struct blkt *) NULL &&
                           next_blkt->type == blkt_ptr->type)
                        merge_lists(blkt_ptr, &next_blkt);
                    if (stage_ptr->next_stage != NULL || prev_stage != NULL) {
                        if (!stage_ptr->next_stage &&
                            0 != chan->first_stage->next_stage->sequence_no) {
                            error_return(UNSUPPORT_FILTYPE,
...
/*        error_return(UNSUPPORT_FILTYPE, "check_channel; unsupported filter type"); */
                    ...
                    fprintf(stdout,
                            "%s WARNING: Generic blockette is detected in stage %d; content is ignored\n",
                            myLabel, i + 1);
                    fflush(stdout);
                    filt_blkt = blkt_ptr;
                    break;
                    ...
                    if (stage_type) {
                        if (stage_type == FIR_TYPE && nc > 0)
                            chan->calc_delay +=
                                ((nc -
                                  1) / 2.0) *
                                blkt_ptr->blkt_info.decimation.sample_int;
                        chan->estim_delay +=
                            (double) blkt_ptr->blkt_info.decimation.estim_delay;
                        chan->applied_corr +=
                            (double) blkt_ptr->blkt_info.decimation.
                            applied_corr;
                        chan->sint = blkt_ptr->blkt_info.decimation.sample_int * (double) blkt_ptr->blkt_info.decimation.deci_fact;     /* channel's sint is the last stage's */
                    } else
                        error_return(ILLEGAL_RESP_FORMAT,
                ...
                default:
                    error_return(UNSUPPORT_FILTYPE,
                                 "check_channel; unrecognized blkt type (type=%d)",
                                 blkt_ptr->type);
            }
            blkt_ptr = next_blkt;
        }
        ...
        if (stage_type == PZ_TYPE || stage_type == FIR_TYPE ||
            ...
            stage_type == LIST_TYPE) {
            if (prev_stage != (struct stage *) NULL &&
                prev_stage->output_units != stage_ptr->input_units)
                error_return(ILLEGAL_RESP_FORMAT,
        ...
        if ((stage_type == IIR_TYPE || stage_type == FIR_TYPE ||
             stage_type == IIR_COEFFS_TYPE) && !deci_flag)
            error_return(ILLEGAL_RESP_FORMAT,
...
void
check_sym(struct blkt *f, struct channel *chan) {
    int nc, n0, k;
    double sum = 0.0;
    ...
    if (nc && (sum < (1.0 - FIR_NORM_TOL) || sum > (1.0 + FIR_NORM_TOL))) {
        fprintf(stderr, "%s WARNING: FIR normalized: sum[coef]=%E; ", myLabel,
                sum);
    ...
    if ((nc % 2) == 0) {
        n0 = nc / 2;
        for (k = 0; k < n0; k++) {
            if (f->blkt_info.fir.coeffs[n0 + k] !=
                f->blkt_info.fir.coeffs[n0 - k - 1])
                return;
        }
        f->type = FIR_SYM_2;
 ...
 **************************************************************************/
void
interpolate_list_blockette(double **frequency_ptr, double **amplitude_ptr,
                           double **phase_ptr, int *p_number_points,
                           double *req_freq_arr, int req_num_freqs,
                           double tension) {
    int i, num;
    ...
    if (first_freq > last_freq) {       /* first is larger than last; swap values */
        val = first_freq;
        first_freq = last_freq;
        last_freq = val;
    }
    i = 0;                      /* check if any requested frequency values are out of range */
    while (i < req_num_freqs && (req_freq_arr[i] < first_freq || req_freq_arr[i] > last_freq)) {        /* for each out-of-range value at beginning of req freqs */
        ++i;
    }
    ...
    if (i > 0 && fabs(first_freq - req_freq_arr[i - 1]) < first_freq * 1e-6) {
        ...
        fix_first_flag = 0;
    if (i > 0) {                /* some requested frequency values were clipped */
        if (i >= req_num_freqs) {       /* all requested frequency values were clipped */
            error_exit(IMPROP_DATA_TYPE,
                       "Error interpolating amp/phase values:  %s",
    ...
    while (used_req_num_freqs > 0 && (req_freq_arr[used_req_num_freqs - 1] > last_freq || req_freq_arr[used_req_num_freqs - 1] < first_freq)) { /* for each out-of-range value at end of req freqs */
        --used_req_num_freqs;
    }
    ...
    if (used_req_num_freqs < req_num_freqs &&
        ...
        fix_last_flag = 0;
    if ((num = req_num_freqs - used_req_num_freqs) > 0) {       /* some requested frequency values were clipped; show message */
    ...
    if (used_req_freq_arr) {
        memcpy(used_req_freq_arr, &req_freq_arr[i],
               req_num_freqs * sizeof(double));
    }
    ...
    if ((retstr =
         evr_spline(*p_number_points, *frequency_ptr, *amplitude_ptr, tension,
                    1.0, req_freq_arr, req_num_freqs, &retvals_arr,
                    &num_retvals)) != NULL) {
        error_exit(IMPROP_DATA_TYPE, "Error interpolating amplitudes:  %s",
                   retstr);
        return;
    }
    if (num_retvals != req_num_freqs) { /* # of generated values != # requested (shouldn't happen) */
        error_exit(IMPROP_DATA_TYPE, "Error interpolating amplitudes:  %s",
                   "Bad # of values");
        return;
    }
    ...
    for (i = 1; i < *p_number_points; ++i) {    /* for each remaining "source" amplitude value */
        if ((val = (*amplitude_ptr)[i]) < min_ampval)
            min_ampval = val;   /* if new mininum then save value */
    }
    if (min_ampval > 0.0) {     /* all "source" amplitude values are positive */
        min_ampval /= 10.0;     /* bring minimum a bit closer to zero */
        /* substitude minimum for any non-positive values */
        for (i = 0; i < num_retvals; ++i) {     /* for each interpolated amplitude value */
    ...
    unwrapped_flag = 0;
    for (i = 0; i < (*p_number_points); i++) {  /* for each phase value; unwrap if necessary */
        old_pha = (*phase_ptr)[i];
        ...
        else {                  /* phase value was unwrapped */
    ...
    retstr =
        evr_spline(*p_number_points, *frequency_ptr, local_pha_arr, tension,
                   1.0, req_freq_arr, req_num_freqs, &retvals_arr,
                   &num_retvals);
    free(local_pha_arr);
    if (retstr != NULL) {
    ...
    if (unwrapped_flag) {       /* phase values were previously unwrapped; wrap interpolated values */
        added_value = 0.0;
        new_pha = retvals_arr[0];       /* check first phase value */
        if (new_pha > 180.0) {  /* first phase value is too high */
            do                  /* set offset to put values in range */
                added_value -= 360.0;
            while (new_pha + added_value > 180.0);
        } else if (new_pha < -180.0) {  /* first phase value is too low */
            do                  /* set offset to put values in range */
                added_value += 360.0;
            while (new_pha + added_value < -180.0);
        }
        for (i = 0; i < num_retvals; i++) {     /* for each phase value; wrap if out of range */
