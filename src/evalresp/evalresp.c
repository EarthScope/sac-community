     ...
     char *argv[];
{
    ...
    if (fswidx < 5) {
        error_exit(USAGE_ERROR,
                   "Not all of the required inputs are \n\t"
    ...
    if (!is_int(numfstr))
        error_exit(USAGE_ERROR, "nfreqs must be an integer, found '%s'",
                   numfstr);
    nfreqs = atoi(numfstr);
...
/* then get the response for each of the requested station-channel pairs at
    ...
    first =
        evresp_itp(sta_list, cha_list, net_code, locid, datime, units, file,
