...
print_chan(struct channel *chan, int start_stage, int stop_stage,
    ...
    while (this_stage) {
        if (start_stage >= 0 && stop_stage &&
            (this_stage->sequence_no < start_stage ||
    ...
    while (this_stage) {
        ...
        while (this_blkt) {
            ...
            switch (this_blkt->type) {
                case LAPLACE_PZ:
                    sprintf(tmp_str,
                            " LAPLACE     A0=%E NZeros= %2d NPoles= %2d",
                ...
                case ANALOG_PZ:
                    sprintf(tmp_str,
                            " ANALOG      A0=%E NZeros= %2d NPoles= %2d",
                            this_blkt->blkt_info.pole_zero.a0,
                            this_blkt->blkt_info.pole_zero.nzeros,
                            this_blkt->blkt_info.pole_zero.npoles);
                    break;
                ...
                case IIR_PZ:
                    sprintf(tmp_str,
                            " IIR_PZ      A0=%E NZeros= %2d NPoles= %2d",
                            this_blkt->blkt_info.pole_zero.a0,
                            this_blkt->blkt_info.pole_zero.nzeros,
                            this_blkt->blkt_info.pole_zero.npoles);
                    break;
                case IIR_COEFFS:
                    sprintf(tmp_str,
                            "IIR_COEFFS   H0=%E NNumers=%2d NDenums= %2d",
...
void
print_resp_itp(double *freqs, int nfreqs, struct response *first, char *rtype,
    ...
    while (resp != (struct response *) NULL) {
        ...
        if ((0 == strcasecmp(rtype, "AP")) || (0 == strcasecmp(rtype, "FAP"))) {
            ...
            for (i = 0; i < num_points; i++) {
                amp_arr[i] =
                    sqrt(output[i].real * output[i].real +
                         output[i].imag * output[i].imag);
                pha_arr[i] =
                    atan2(output[i].imag,
                          output[i].real + 1.e-200) * 180.0 / Pi;
            }
            ...
            if (!stdio_flag) {
                if (0 == strcasecmp(rtype, "AP")) {
                    ...
                    if ((fptr1 = fopen(filename, "w")) == (FILE *) NULL) {
                        error_exit(OPEN_FILE_ERROR,
                                   "print_resp; failed to open file %s",
                                   filename);
                    }
                    ...
                    if (1 == unwrap_flag) {
                        ...
                        for (i = 0; i < num_points; i++) {
                            pha = pha_arr[i] + phas1;
                            pha =
                                unwrap_phase(pha, prev_phase, 360.0,
                                             &added_value);
                            pha_arr[i] = pha;
                            prev_phase = pha;
                        }
                         ...
                         * this is requested by AFTAC
                         */
                        (void) evresp_adjust_phase(pha_arr, num_points, -360.0,
                                                   360.0);
#else
                        /* Do not unwrap */ ;
#endif
                ...
                if (0 == strcasecmp(rtype, "FAP")) {
                    ...
                    for (i = 0; i < num_points; i++) {
                        pha = pha_arr[i] + phas1;
                        pha =
                            unwrap_phase(pha, prev_phase, 360.0, &added_value);
                        pha_arr[i] = pha;
                        prev_phase = pha;
                    }
                ...
                fprintf(stdout, "%s AMP/PHS.%s.%s.%s.%s\n", myLabel,
                        resp->network, resp->station, resp->locid,
                        resp->channel);
                fprintf(stdout,
                        "%s --------------------------------------------------\n",
                        myLabel);
...
int
arrays_equal(double *arr1, double *arr2, int arr_size) {
    int i;
