...
plotcontdata(float zdata[], int nxsize, int nysize, int *nerr)
{
    ...
    float heightsave, unused, widthsave, zincrement, zlevel, zmaximum, zminimum;
    ...
    *nerr = 0;
    ...
    if (strcmp(kmcontouring.klevelmode, "LIST    ") == 0) {
        ...
        if (strcmp(kmcontouring.klevelmode, "RANGE   ") == 0) {
            ...
            extrma(zdata, 1, nxsize * nysize, &zminimum, &zmaximum, &unused);
        }
        if (cmcontouring.zlevelincrement > 0.0) {
            ...
            zmaximum = zincrement * (float) ((int) (zmaximum / zincrement) + 1);
            cmcontouring.nzlevels =
                min((int) ((zmaximum - zminimum) / zincrement) + 1, MZLEVELS);
        } else {
            cmcontouring.nzlevels = cmcontouring.nznumlevels;
            zincrement =
                (zmaximum - zminimum) / (float) (cmcontouring.nzlevels - 1);
        }
        zlevel = zminimum;
    ...
    if (strcmp(kmcontouring.klabelmode, "ON      ") == 0) {
        ...
        for (jlevel = 1; jlevel <= cmcontouring.nzlevels; jlevel++) {
            ...
            if (strcmp(kmcontouring.klabellist[ilabel - 1], "ON              ")
                ...
                if (strcmp
                    (kmcontouring.klabellist[ilabel - 1],
                     "OFF             ") == 0) {
                ...
                /* --- If "INT", use floating point conversion and delete the period. */
                              ...
                              , "INT", 3) == 0) {
                sprintf(kmcontouring.klabel[jlevel_], "%14.0f",
                        Zlevels[jlevel]);
                nc = indexa((char *) kmcontouring.klabel[jlevel_], 17, '.',
                            FALSE, TRUE);
                if (nc > 0)
                    subscpy(kmcontouring.klabel[jlevel_], nc - 1, -1, 16, " ");
                goto L_4000;
                ...
                if (kmcontouring.klabellist[ilabel - 1][5] != ' ') {
                    kmcontouring.kdecimal =
                        kmcontouring.klabellist[ilabel - 1][5];
                } else {
                    ilabel = ilabel + 1;
                    kmcontouring.kdecimal =
                        kmcontouring.klabellist[ilabel - 1][0];
                }
                ...
                if (kmcontouring.klabellist[ilabel - 1][3] != ' ') {
                    kmcontouring.kdecimal =
                        kmcontouring.klabellist[ilabel - 1][3];
                } else {
                    ilabel = ilabel + 1;
                    kmcontouring.kdecimal =
                        kmcontouring.klabellist[ilabel - 1][0];
                ...
                strcpy(kmcontouring.klabel[jlevel_],
                       kmcontouring.klabellist[ilabel - 1]
                    );
            }
