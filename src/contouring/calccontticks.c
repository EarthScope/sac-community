...
void
calccontticks() {
    int itick, jaction[MPOINTS], jcur, jlevel, jlink[MPOINTS], jpoint[MPOINTS],
        jprev, jsegment, jstart, jstop, jtemp, jtick;
        ...
        if (jtick != 0) {
          ...
          L_2000:
            if (Jlink[jprev] > 0) {
                ...
                accumlength =
                    accumlength +
                    sqrt(powi(point[jprev - 1][0] - point[jcur - 1][0], 2) +
                         powi(point[jprev - 1][1] - point[jcur - 1][1], 2));
                if (accumlength >= reqspacing) {
                    Jaction[jcur] = jtick;
                    putcontpoint(Jpoint[jcur], &point[jcur - 1][0], Jlink[jcur],
                                 Jaction[jcur]);
                    accumlength = 0.;
