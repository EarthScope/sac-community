...
calccontlabel1() {
    ...
    int jaction[MPOINTS], jcur, jlabel, jlabel1, jlast, jlevel, jlink[MPOINTS],
        jnext, jpoint[MPOINTS], jprev, jsegment, jstart, jstatus, jstop, jtemp,
        jtype, numlocs;
  ...
  L_1000:
    if (nextcontseg(&jsegment, &jlevel, &jstart, &jstop)) {
        ...
        if (strcmp(kmcontouring.klabel[jlevel - 1], "OFF             ") != 0) {
            jprev = 1;
            jcur = 2;
            jnext = 3;
          ...
          L_2000:
            if (Jpoint[jprev] != jlast && jlast > 0) {
                accumlength =
                    accumlength +
                    sqrt(powi(point[jcur - 1][0] - point[jprev - 1][0], 2) +
                         powi(point[jcur - 1][1] - point[jprev - 1][1], 2));
                ...
                if (replacemode) {
                    if (accumlength >= skiplength) {
                        ...
                        putcontlabel(jlabel, Jpoint[jcur], jtype, anglecur,
                                     jlevel);
                        anglesave = anglecur;
                    }
                    /* ---- If in search mode, look for an acceptable location. */
                } else
                    if ((fabs(anglecur) <= cmcontouring.desiredangle &&
                         accumlength >= skiplength) || (numlocs == 0 &&
                                                        accumlength >=
                                                        cmcontouring.
                                                        maxlabelspacing)) {
                    numlocs = numlocs + 1;
                    ...
                    if (numlocs == 1) {
                        jlabel1 = jlabel;
                        if (closedseg &&
                            (accumlength < cmcontouring.minlabelspacing)) {
                            skiplengthend =
                                cmcontouring.minlabelspacing - accumlength;
