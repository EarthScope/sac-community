     ...
     int *nerr;
{
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        if (lklogi("NUMBER$", 8, &cmspe.lsnumw, &cmspe.numwin)) {       /* do nothing */
        }
        ...
        else if (lklist("TYPE$", 6, (char *) kmspe.kwintp, 9, MWINTP, &cmspe.iwncor)) { /* do nothing */
        }

        /* -- "PREWHITEN ON/OFF/n":  turn prewhitening on or off. */
        else if (lklogi("PREWHITEN$", 11, &cmspe.lprewh, &cmspe.nprerq)) {      /* do nothing */
        }
    ...
    cmspe.nwinln = min((int) (cmspe.samfrq * cmspe.winlen + 0.5), nlnuse);
    ...
    autcor(spe->y + cmspe.nprewh, samint, nlnuse, cmspe.numwin, cmspe.nwinln,
           (char *) kmspe.kwintp[cmspe.iwncor - 1]
           , kmspe.kscale, specor, &cmspe.nlnfft, &cmspe.nlncor, kmspe.kermsg,
           131, speaux, ridge_fac);
