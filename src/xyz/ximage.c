     ...
     int *nerr;
{
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        else if (lklog("CBAR$", 8, &lcbar)) {   /* do nothing */
        }
    ...
    for (jfile = 1; jfile <= nfiles; jfile++) {
        ...
        if (lxlimits) {
            ...
            ixstart =
                max(1, (int) ((xstart - xminimum) / cmgem.xgen.delta) + 1);
            ixstop =
                min(nxsize, (int) ((xstop - xminimum) / cmgem.xgen.delta) + 2);
        ...
        if (lylimits) {
            ...
            iystart =
                max(1, (int) ((ystart - yminimum) / cmgem.ygen.delta) + 1);
            iystop =
                min(nysize, (int) ((ystop - yminimum) / cmgem.ygen.delta) + 2);
        ...
        setvport(cmgem.uplot.xmin, cmgem.uplot.xmax, cmgem.uplot.ymin,
                 cmgem.uplot.ymax);
        ...
        jxstart =
            (cmcontouring.ixdatastart <= 0) ? 1 : cmcontouring.ixdatastart;
        jxstop =
            (cmcontouring.ixdatastop <= 0) ? nxsize : cmcontouring.ixdatastop;
        jystart =
            (cmcontouring.iydatastart <= 0) ? 1 : cmcontouring.iydatastart;
        jystop =
            (cmcontouring.iydatastop <= 0) ? nysize : cmcontouring.iydatastop;
        ...
        /* Window Data */
        if (jxstart != 1 || jystart != 1 || jystop != nxsize ||
            jystop != nysize) {
            ...
            wdata_alloc = TRUE;
            window_data(s->y, nxsize, nysize, wdata, jxstart, jxstop, jystart,
                        jystop);
        } else {
            wdata = s->y;
            wdata_alloc = FALSE;
        ...
        show_image(wdata, nxdata, nydata, xminimum, xmaximum, yminimum, ymaximum, cmgem.plot.xmin, cmgem.plot.ymax, cmgem.plot.xmax - cmgem.plot.xmin,  /* width */
                   cmgem.plot.ymax - cmgem.plot.ymin,   /* height */
                   cmgdm.npscimage, cmgdm.nctsize, 7, lbinary, nerr);
        ...
        if (lcbar) {
            ...
            show_colorbar(cmgem.plot.xmin - 0.05,
                          cmgem.plot.ymax - (height - height * 0.75) / 2, 0.025,
                          height * 0.75, zmin, zmax, lbinary, nerr);
        }
