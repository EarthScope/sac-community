...
void
scallop(float *sdata, int specwidth, int speclength, float freqmax, float fmin,
        float fmax, int lmean, float *scdata, int *nerr) {
    int i, j;
    ...
    *nerr = 0;
    ...
    if ((fmax > freqmax) || (fmin > freqmax)) {
        printf
            ("Error: smoothing window frequency range > data maximum frequency\n");
        *nerr = 1;
        return;
    }
    ...
    if (nhalf0 < 3) {           /* need at least three points in the smoothing window */
        printf
            ("Error: Fmin smoothing window less than three points, resetting to 0, no smoothing\n");
        nfull0 = 0;
        nhalf0 = 0;
        fmin = 0.0;
    }
...
/* loop over the slices of the spectrogram */
    ...
    for (j = 0; j < speclength; j++) {
        ...
        if (nfull0 != 0) {
            smooth(trans_sdata, specwidth, lmean, index, nhalf0, nfull0,
                   factor0, smooth0, nerr);
            if (*nerr != 0)
                break;

        } else {
            for (i = 0; i < specwidth; i++)
                smooth0[i] = trans_sdata[i];
        }

        if (nfull1 != 0) {
            smooth(trans_sdata, specwidth, lmean, index, nhalf1, nfull1,
                   factor1, smooth1, nerr);
            if (*nerr != 0)
                break;

        } else {
            for (i = 0; i < specwidth; i++)
                smooth1[i] = trans_sdata[i];
        }
