...
xgrayscale(int *nerr) {
    ...
    int lvideo;
    int graylength, graywidth, imagelength, imagewidth, jfile, nfiles, nxsize,
        nysize, nzsize, xmax, xmin, ymax, ymin;
    FILE *un;
    double tmp;
  ...
  L_1000:
    ...
    for (jfile = 1; jfile <= nfiles; jfile++) {
        ...
        if (zoom > 0) {
            if ((zoom * graywidth > maximagesize) ||
                (zoom * graylength > maximagesize)) {
                *nerr = 2902;
                setmsg("ERROR", *nerr);
                apimsg(maximagesize);
                goto L_8888;
        ...
        sprintf(arg1, "%10d%10d%10.2f %c%10d%10d%10d%10d%10d%10d%10d%10d%s\n",
                ...
                ycrop, graywidth, graylength, xmin, ymin, xmax, ymax,
                " >>output.Utahtmp");
