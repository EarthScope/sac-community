...
void
spcgrm(float signal[], int lfft, int iorfft, char *type, int *order,
       double delta, int wlen, int sfft, int lncorwin, int ncorwin,
       float lencorwin, char *wintype, char *scale, float ridge_fac, int *err) {
    int i;
    int nfft = 0;
    int nlags = 0;
    ...
    *err = 0;
    ...
    if (!lncorwin) {            /* if number of correlation windows was not specified by user */
        ncorwin = wlen / cwinln;
        if ((cwinln * ncorwin) < wlen)
            ncorwin++;
    }

    autcor(signal, delta, wlen, ncorwin, cwinln, wintype, scale, ac, &nfft,
           &nlags, errmsg, 131, aux, ridge_fac);
    ...
    if ((memcmp(type, "MLM", 3) == 0) || (memcmp(type, "mlm", 3) == 0)) {
        ...
        for (i = 1; i <= (*order - 1); i++) {
            Signal[i] =
                Signal[i] * (1. - (float) (i - 1) / (float) (*order - 1));
        }
