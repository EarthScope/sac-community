...
int
spectrogram(double window, double sliceint, char *type, int *order, int nfiles,
            int filelength[], double delta, float **spec, int *specwidth,
    ...
    /*          Calculate FFT size */
    if (calcfftsize
        (delta, sliceint, window, &iorfft, &lfft, &nptswndw, &buffersize,
         &windowovrl) != 0) {
        ...
        if (err != 0) {
            fprintf(stdout,
                    "Error releasing previously stored image(spectrogram).\n");
            ...
            if (1) {
                ...
                while (1) {
                    ...
                    /* Get windowed data */
                        ...
                        getdata(nfiles, delta, nptswndw, windowovrl, windowfunc,
                                buffersize, filelength, buffer, signals);
                    //printf("getdata done: %d\n", done);
                    if (done != 0) {
                        if (done == 1) {
                            fprintf(stdout,
                                    "Error getting data(spectrogram).\n");
                            FREE(spec);
                        }
                        break;
                        ...
                        for (idx = 1; idx <= (IMGFFT / 2); idx++) {
                            ...
                            if (Signals[idx] <= 0.0) {
                                ...
                                if (ridge_regress > .100001) {
                                    printf
                                        ("Error:  Could not stabilize high res. spectrogram\n");
                                    goto L_8888;
                                }
