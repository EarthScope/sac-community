...
show_colorbar(float x, float y, float width, float height, float zmin,
    ...
    show_image(cbar, w, h, 0, (float) w, 0, (float) h, x, y, width, height,
...
void
specplot(float *specdata, int nx, int ny, float xmin, float xmax, float ymin,
         float ymax, float ywmin, float ywmax, char *imagetype, int lbinary,
         int lcbar, int lprint, int *nerr) {
    char kptext[MCMSG + 1];
    int lany, lbotaxsave, lbottcsave, lframesave, ltitlsave, ltoptcsave, lwait,
    ...
    *nerr = 0;
    ...
    cmgem.tsdef =
        fmin(cmgem.tsdef, (cmgem.view.ymax - cmgem.view.ymin) / (8.0 * 5.0));
    ...
    /* Show the iamge */
    show_image(wdata, nx, nypoints - nystart + 1, xmin, xmax, ymin, ymax,
               cmgem.plot.xmin, cmgem.plot.ymax, width, height, cmgdm.npscimage,
               cmgdm.nctsize, 7, lbinary, nerr);
    ...
    if (lcbar) {
        ...
        show_colorbar(xpmnsave, cmgem.plot.ymax - (h - h * 0.75) / 2, 0.025,
                      h * 0.75, zmin, zmax, lbinary, nerr);
    }
