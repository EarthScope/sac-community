    ...
    *nerr = 0;
    ...
    if (cmgem.lylim) {
        ...
        if (cmgem.ygen.on) {
            ...
            cmgem.data.ymax =
                cmgem.ygen.first + (number - 1) * cmgem.ygen.delta;
        } else {
            if (cmgem.lxlim) {
                if (cmgem.xgen.on) {
                    num1 =
                        (int) (ceil
                               ((cmgem.data.xmin -
                                 cmgem.xgen.first) / cmgem.xgen.delta)) + 1;
                    if (num1 < 1)
                        num1 = 1;
                    num2 =
                        (int) (floor
                    ...
                    for (jdx = 1; jdx <= number; jdx++) {
                        if (Xarray[jx] >= cmgem.data.xmin &&
                            Xarray[jx] <= cmgem.data.xmax) {
    ...
    setvport(cmgem.uplot.xmin, cmgem.uplot.xmax, cmgem.uplot.ymin,
             cmgem.uplot.ymax);
    setworld(cmgem.zdata.xmin, cmgem.zdata.xmax, cmgem.zdata.ymin,
             cmgem.zdata.ymax);
