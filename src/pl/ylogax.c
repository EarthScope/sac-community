...
ylogax() {
    ...
    decsiz =
        fabs(cmgem.uplot.ymax - cmgem.uplot.ymin) / (cmgem.zdata.ymax -
                                                     cmgem.zdata.ymin);
    isecin = 1;
    ...
    if (cmgem.axis[LEFT].annotate || cmgem.axis[LEFT].ticks) {
        ...
        if (cmgem.iskwidth > LINE_WIDTH_THIN) {
            line(cmgem.uplot.xmin, cmgem.uplot.ymin - cmgem.iskwidth * skfudge,
                 cmgem.uplot.xmin, cmgem.uplot.ymax + cmgem.iskwidth * skfudge);
        } else {
            line(cmgem.uplot.xmin, cmgem.uplot.ymin, cmgem.uplot.xmin,
                 cmgem.uplot.ymax);
        }
        ...
        for (jfac = 2; jfac <= 9; jfac++) {
            ...
            if (yrefs >= ypmnf && yrefs <= ypmxf) {
                if ((lsecax && cmgem.axis[LEFT].annotate) &&
                    (jfac % isecin) == 0) {
                ...
                if (lsectc) {
                    line(cmgem.uplot.xmin, yrefs,
                         cmgem.uplot.xmin + 0.5 * cmgem.chwid, yrefs);
                }
            }
        }
        ...
        for (jdec = idecmn; jdec <= idecmx; jdec += idecin) {
            ...
            for (jfac = 2; jfac <= 9; jfac++) {
                ...
                if (yrefs <= ypmxf) {
                    ...
                    if (lsectc) {
                        line(cmgem.uplot.xmin, yrefs,
                             cmgem.uplot.xmin + .5 * cmgem.chwid, yrefs);
                    }
                }
            }
        }
    ...
    if (cmgem.axis[RIGHT].annotate || cmgem.axis[RIGHT].ticks) {
        ...
        /* -- Right Axes line. */
        if (cmgem.iskwidth > LINE_WIDTH_THIN) {
            line(cmgem.uplot.xmax, cmgem.uplot.ymin - cmgem.iskwidth * skfudge,
                 cmgem.uplot.xmax, cmgem.uplot.ymax + cmgem.iskwidth * skfudge);
        } else {
            line(cmgem.uplot.xmax, cmgem.uplot.ymin, cmgem.uplot.xmax,
                 cmgem.uplot.ymax);
        }
        ...
        for (jfac = 2; jfac <= 9; jfac++) {
            yrefs = yref + cmgem.fac[jfac - 1] * cmgem.ympip1;
            if (yrefs >= ypmnf && yrefs <= ypmxf) {
                if ((lsecax && cmgem.axis[RIGHT].annotate) &&
                    (jfac % isecin) == 0) {
                ...
                if (lsectc) {
                    line(cmgem.uplot.xmax, yrefs,
                         cmgem.uplot.xmax - 0.5 * cmgem.chwid, yrefs);
                }
            }
        }
        ...
        for (jdec = idecmn; jdec <= idecmx; jdec += idecin) {
            ...
            for (jfac = 2; jfac <= 9; jfac++) {
                ...
                if (yrefs <= ypmxf) {
                    ...
                    if (lsectc) {
                        line(cmgem.uplot.xmax, yrefs,
                             cmgem.uplot.xmax - .5 * cmgem.chwid, yrefs);
                    }
                    if (cmgem.lygrd) {
