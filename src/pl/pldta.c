...
pldta(float xarray[], float yarray[], int number, int incx, int incy, int *nerr) {
  ...
  /*=====================================================================
    ...
    *nerr = 0;
    ...
    lclip = (((cmgem.lrqclp || cmgem.lylim) || (cmgem.lxlim && !cmgem.xgen.on))
             || cmgem.ixint == AXIS_LOG || cmgem.iyint == AXIS_LOG);
    ...
    for (jcopy = 1; jcopy <= ncopy; jcopy++) {
        ...
        if (cmgem.ixint == AXIS_LOG) {
            if (cmgem.floor > 0)
                ...
                logdta(xblock, nblock, cmgem.lfloor, cmgem.data.xmin, xblock,
                       nerr);
        ...
        if (cmgem.iyint == AXIS_LOG) {
            if (cmgem.floor > 0)
                ...
                logdta(yblock, nblock, cmgem.lfloor, cmgem.data.ymin, yblock,
                       nerr);
