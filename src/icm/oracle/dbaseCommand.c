...
void
dbaseResponse(int nfreq, double delfrq, double *xre, double *xim,
    ...
    if (isSet(LOCID, getTransferDirection())) {
        ...
        strcpy(check, khole);
        deblank(check);
        if (strlen(check) == 2 && isalnum(check[0]) && isalnum(check[1]))
            strcpy(locid, check);
        else
            strcpy(locid, "*");
    }
    ...
    file =
        GetRESPfileNameFromDB(station, component, locid, EpochTime, resptype);
    if (!file) {
        char staComp[81];
    ...
    else if (!strcmp(resptype, "fap") || !strcmp(resptype, "paz") ||
             !strcmp(resptype, "fir") || !strcmp(resptype, "pazfir")) {
