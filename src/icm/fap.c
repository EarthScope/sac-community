...
fap_grow(double *p, int n) {
    ...
    if (!tmp) {
        fprintf(stderr,
                "Error allocating memory for Frequency, Amplitude, and Phase file\n");
        free(p);
        p = NULL;
        return NULL;
    }
...
void
frequency_amplitude_phase(int nf, double df, double *xre, double *xim,
                          char *file, int file_s, int *nerr) {
    FILE *fp;
    char line[BUFLEN];
    char *p;
    ...
    while ((p = fgetsp(line, BUFLEN, fp)) != NULL) {
        ...
        if (p[0] == FAP_COMMENT || p[0] == FAP_NEWLINE ||
            p[0] == FAP_TERMINATOR) {
            continue;
        }
        if (n >= nalloc) {
            ...
            if ((freq = fap_grow(freq, nalloc)) == NULL) {
                *nerr = 2;
                goto ERROR;
            }
            if ((amp = fap_grow(amp, nalloc)) == NULL) {
                *nerr = 2;
                goto ERROR;
            }
            if ((phase = fap_grow(phase, nalloc)) == NULL) {
                *nerr = 2;
                goto ERROR;
            }
        }
    ...
    for (i = 0; i < nf; i++) {
        ...
        if (f <= freq[istart]) {
            ...
            if (i == 1)
                printf(" Extrapolating below lowest FAPFILE frequency\n");
        } else if (f >= freq[n]) {
            amp_phase_to_complex(amp[n], phase[n], &xre[i], &xim[i]);
            if (i == nf - 1)
                printf(" Extrapolating above highest FAPFILE frequency\n");
        } else {
            for (j = istart; j < n; j++) {
