...
void
hqr(double **matrix, int order, struct roots theseRoots[],      /*int *cnt , */
    int *nerr) {
        ...
        while (1) {             /* look for single small sub-diagonal element */
            ...
            if (its == 10 || its == 20) {
                ...
                for (idx = 0; idx < order; idx++)
                    matrix[idx][idx] -= x;
                ss = fabs(matrix[order_][na_]) + fabs(matrix[na_][order_ - 2]);
                x = y = 0.75 * ss;
                w = -0.4375 * ss * ss;
            }
            ...
            for (mdx = order_ - 2; mdx >= ldx; mdx--) {
                ...
                r = x - z;
                ss = y - z;
                p = (r * ss - w) / matrix[mdx + 1][mdx] + matrix[mdx][mdx + 1];
