 ...
 */
void
map_hdr_out(float *memarray, float *buffer, int lswap) {
    ...
    if (lswap == TRUE) {
        for (i = 0; i < SAC_HEADER_NUMBERS;
             i++, ptr2 += SAC_HEADER_SIZEOF_NUMBER) {
    ...
    map_chdr_out(memarray + SAC_HEADER_NUMBERS, buffer + SAC_HEADER_NUMBERS);
