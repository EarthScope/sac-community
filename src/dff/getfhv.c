    ...
    if (flag == -1) {
        if ((env_string = getenv("SACIO_MESSAGES"))) {
            ...
            for (i = 0; i < (int) (sizeof(env_on) / sizeof(char *)); i++) {
                if (strncasecmp
                    (env_string, env_on[i],
                     min(n, (int) strlen(env_on[i]))) == 0) {
                    flag = TRUE;
                }
            }
            for (i = 0; i < (int) (sizeof(env_off) / sizeof(char *)); i++) {
                if (strncasecmp
                    (env_string, env_off[i],
                     min(n, (int) strlen(env_off[i]))) == 0) {
                    flag = FALSE;
                }
            }
        } else {
...
void
getfhv_(char *kname, float *fvalue, int *nerr, int kname_s) {
    getfhv(kname, fvalue, nerr, kname_s);
}

void
getfhv__(char *kname, float *fvalue, int *nerr, int kname_s) {
    getfhv(kname, fvalue, nerr, kname_s);
}
