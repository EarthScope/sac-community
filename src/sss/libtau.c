...
#include "ttlim.h"
struct t_brkc {
    double zs, pk[JSEG], pu[2][JTSM0], pux[2][JXSM], tauu[2][JTSM], xu[2][JXSM],
        px[2][JBRN], xt[2][JBRN], taut[JOUT], coef[JOUT][5], tauc[JTSM],
        xc[JXSM], tcoef[2][JBRNA][5], tp[2][JBRNU];
    float odep, fcs[3][JSEG];
    int nin, nph0, int0[2], ki, msrc[2], isrc[2], nseg, nbrn, ku[2], km[2],
        nafl[3][JSEG], indx[2][JSEG], kndx[2][JSEG], iidx[JSEG], jidx[JBRN],
        kk[JSEG];
} brkc;
struct t_pcdc {
    char phcd[JBRN][9];
...
#define PCNTL(I_,J_)	(pcntl+(I_)*(pcntl_s)+(J_))
    ...
    static int all, fnd;
    static int i, i_, j, j1, j2, j_, k, k_, kseg, l, no, nsgpt[JBRN];
    static char cmdcd[NCMD][9] =
        { "P       ", "P+      ", "basic   ", "S+      " };
     ...
     int nph;
{
    ...
    /* int _i, _r; */
    static int i, i1, i2, i_, iph, is, j, j_, k, k1, k2, k_, kph, ks, l, l_, lp,
        m, ms, mu, n1;
    static float ztol;
    static double du, fac, sgn, tauus1[2], tauus2[2], ttau, tx, u0, u1, umin,
        xus1[2], xus2[2], z0, z1;
    static float tol = .01;
  ...
  L_6:
    ...
    if (Msrc[nph] == ks)
        goto L_8;
    bkin(brkc.nin, umdc.ndex[nph - 1][ks - 1], Ku[nph] + Km[nph], tup);
    goto L_11;
  ...
  L_9:
     ...
     *   depth. */
    if (fabs(umdc.zm[nph - 1][ks - 1] - brkc.zs) <=
        fabs(umdc.zm[nph - 1][ks] - brkc.zs))
        goto L_10;
    ks = ks + 1;
    sgn = -1e0;
  L_10:
...
/* L_15: */
  ...
  L_17:
    ...
    for (k = 1; k <= k1; k++) {
        ...
        if (fabs(brkc.pu[nph - 1][k_] - brkc.pux[nph - 1][mu - 1]) > dtol)
            goto L_18;
  ...
  L_39:
    ...
    Xus2[nph] = 0e0;
    mu = mu - 1;
    if (fabs(umin - Us[nph]) > dtol &&
        fabs(umin - brkc.pux[nph - 1][mu - 1]) <= dtol)
        mu = mu - 1;
    ...
    is = Isrc[nph];
    Tauus2[nph] = 0e0;
    if (fabs(brkc.pux[nph - 1][mu - 1] - umin) > dtol ||
        fabs(Us[nph] - umin) > dtol)
        goto L_48;
  ...
  L_47:
    ...
    /*   Unless the turning point is right on a sample slowness, one more
     *   partial integral is needed. */
    tauint(umin, umdc.pm[iph - 1][j - 1], umin, umdc.zm[iph - 1][j - 1], z1,
           &ttau, &tx);
    Tauus1[iph] = Tauus1[iph] + ttau;
    Xus1[iph] = Xus1[iph] + tx;
  ...
  L_53:
    ...
    for (i = 1; i <= brkc.nseg; i++) {
        ...
        if (!Segmsk[i])
            goto L_57;
        if ((brkc.nafl[0][i_] == nph && brkc.nafl[1][i_] == 0) && Iidx[i] <= 0)
            goto L_58;
      L_57:
        ;
    }
    ...
    for (l = pdec.ka; l >= 1; l--) {
        ...
        for (i = 2; i <= is; i++) {
            ...
            tauint(pdec.ua[nph - 1][k - 1], umdc.pm[nph - 1][j - 1],
                   umdc.pm[nph - 1][i_], umdc.zm[nph - 1][j - 1],
                   umdc.zm[nph - 1][i_], &ttau, &tx);
      ...
      L_54:
        ...
        /*   Unless the source is right on a sample slowness, one more partial
         *   integral is needed. */
        tauint(pdec.ua[nph - 1][k - 1], umdc.pm[nph - 1][is - 1], Us[nph],
               umdc.zm[nph - 1][is - 1], brkc.zs, &ttau, &tx);
        pdec.taua[nph - 1][k - 1] = pdec.taua[nph - 1][k - 1] + ttau;
      L_56:
        ;
    }
  ...
  L_43:
    ...
    for (i = 1; i <= brkc.nseg; i++) {
        ...
        if (!Segmsk[i])
            goto L_20;
        if ((Iidx[i] > 0 || labs(brkc.nafl[0][i_]) != nph) ||
            (Msrc[nph] <= 0 && brkc.nafl[0][i_] > 0))
            goto L_20;
      ...
      L_22:
        ...
        fac = brkc.fcs[0][i_];
        tabc.tau[k - 1][0] =
            fac * (Tauus1[iph] + Tauus2[iph] + Tauus1[kph] + Tauus2[kph]) +
            sgn * Tauus1[nph];
      L_24:
        m = 1;
      L_26:
      ...
      L_37:
        for (l = 1; l <= 2; l++) {
            l_ = l - 1;
          L_28:
            if (fabs(brkc.pux[nph - 1][m - 1] - brkc.px[l_][j - 1]) <= dtol)
                goto L_27;
            if (m >= mu)
                goto L_29;
            m = m + 1;
          ...
          L_29:
            tabc.xbrn[l_][j - 1] =
                fac * (Xus1[iph] + Xus2[iph] + Xus1[kph] + Xus2[kph]) +
                sgn * Xus1[nph];
          L_30:
            ;
        }
...
#define PHNM(I_,J_)	(phnm+(I_)*(phnm_s)+(J_))
    /* static char msg[68]; */
    static int i, ie, ij, in, is, j, jj, le, ln, nph;
...
*/
    ...
    for (ij = tabc.idel[0][jb - 1]; ij <= tabc.idel[1][jb - 1]; ij++) {
        ...
        for (i = is; i <= ie; i++) {
            ...
            *n = *n + 1;
            ps = Pt[ie] - dp;
            stop(*n <= MAX_PHASES, "Tt[%d > %d] (%d)", *n, MAX_PHASES,
                 __LINE__);
            stop(nph <= 2, "Us[%d > %d]", nph, 2);
            Tt[*n] =
                tabc.tn * (tabc.tau[j - 1][0] +
                           dp * (tabc.tau[j - 1][1] +
                                 dps * tabc.tau[j - 1][3]) + ps * x);
            Dtdd[*n] = dsgn * ps;
            Dtdh[*n] = hsgn * sqrt(fabs((float) (Us[nph] * Us[nph] - ps * ps)));
            Dddp[*n] = dpn * .75e0 * tabc.tau[j - 1][3] / fmax(fabs(dps), deps);
...
/*			in = istrstr( PHNM(*n - 1,0), "ab" ); */
          ...
          L_2:
            for (jj = 1; jj <= 2; jj++) {
              ...
              L_5:
                stop(j - 1 < JOUT, "tabc.tau[%d >= %d (%d)", j - 1, JOUT,
                     __LINE__);
                arg =
                    9e0 * tabc.tau[j - 1][3] * tabc.tau[j - 1][3] +
              ...
              L_6:
                stop(j - 1 < JOUT, "tabc.tau[%d >= %d] (%d)", j - 1, JOUT,
                     __LINE__);
                dps =
                    (tabc.tau[j - 1][1] - x) / (2e0 * tabc.tau[j - 1][2] * dps);
                dp = sign(dps * dps, dps);
              L_7:
                if (dp < p1 - delp || dp > p0 + delp)
                    goto L_4;
                ...
                *n = *n + 1;
                ...
                stop(*n <= MAX_PHASES, "Tt[%d > %d] (%d)", *n, MAX_PHASES,
                     __LINE__);
                stop(nph <= 2, "Us[%d > %d]", nph, 2);
                stop(jb - 1 < JBRN, "pcdc.phcd[%d >= %d] (%d)", jb - 1, JBRN,
                     __LINE__);
                Tt[*n] =
                    tabc.tn * (tabc.tau[j - 1][0] +
                               dp * (tabc.tau[j - 1][1] +
                ...
                Dtdd[*n] = dsgn * ps;
                Dtdh[*n] =
                    hsgn * sqrt(fabs((float) (Us[nph] * Us[nph] - ps * ps)));
                Dddp[*n] =
                    dpn * (2e0 * tabc.tau[j - 1][2] +
                           .75e0 * tabc.tau[j - 1][3] / fmax(fabs(dps), deps));
                fstrncpy(PHNM(*n - 1, 0), phnm_s - 1, pcdc.phcd[jb - 1],
                         strlen(pcdc.phcd[jb - 1]));
...
/*				in = istrstr( PHNM(*n - 1,0), "ab" ); */
                if (in <= 0)
                    goto L_4;
                stop(jb - 1 < JBRN, "tabc.xbrn[%d >= %d] (%d)", jb - 1, JBRN,
                     __LINE__);
                if (ps <= tabc.xbrn[2][jb - 1]) {
                    subscpy(PHNM(*n - 1, 0), in - 1, -1, phnm_s - 1, "bc");
                }
          ...
          L_9:
            if (*n > le)
                goto L_8;
            fprintf(stdout,
                    "Failed to find phase: %s%8.1f%7.4f%7.4f%7.4f%7.4f\n",
      ...
      L_12:
        ...
        *n = *n + 1;
        Tt[*n] =
            tabc.tn * (tabc.tau[j - 1][0] +
                       dp * (tabc.tau[j - 1][1] + dp * tabc.tau[j - 1][2] +
                             dps * tabc.tau[j - 1][3]) + Pt[j] * x);
        Dtdd[*n] = dsgn * (float) (Pt[j]);
        Dtdh[*n] =
            hsgn * sqrt(fabs((float) (Us[nph] * Us[nph] - Pt[j] * Pt[j])));
        Dddp[*n] =
            dpn * (2e0 * tabc.tau[j - 1][2] +
                   .75e0 * tabc.tau[j - 1][3] / fmax(dps, deps));
/*		ln = istrstr( pcdc.phcd[jb - 1], " " ) - 1; */
        ...
        fstrncpy(PHNM(*n - 1, 0) + min(ln, 8), phnm_s - 1 - min(ln, 8), "diff",
                 4);
      L_10:
        ;
    }
...
/* L_2: */
    ...
    for (i = is; i <= ie; i++) {
        ...
        j = j + 1;
        tau[i_][1] =
            coef[i_][2] * B[j - 1] + coef[i_][3] * B[j] + coef[i_][4] * B[j +
                                                                          1];
    }
    tau[i2 - 1][1] = xn;
    return;
     ...
     float *xcor, *tcor;
{
    ...
    for (jp = tabc.mbr1; jp <= tabc.mbr2; jp++) {
        ...
        if ((((strcmp(pcdc.phcd[jp_], "Pg      ") == 0 || strcmp(pcdc.phcd[jp_]
                                                                 ,
                                                                 "Pb      ") ==
               0) || strcmp(pcdc.phcd[jp_], "Pn      ") == 0) ||
             strcmp(pcdc.phcd[jp_], "P       ") == 0) &&
            tabc.xbrn[0][jp_] <= 0e0)
            goto L_3;
    }
    jp = 0;
  ...
  L_3:
    for (js = tabc.mbr1; js <= tabc.mbr2; js++) {
        ...
        if ((((strcmp(pcdc.phcd[js_], "Sg      ") == 0 || strcmp(pcdc.phcd[js_]
                                                                 ,
                                                                 "Sb      ") ==
               0) || strcmp(pcdc.phcd[js_], "Sn      ") == 0) ||
             strcmp(pcdc.phcd[js_], "S       ") == 0) &&
            tabc.xbrn[0][js_] <= 0e0)
            goto L_1;
    }
    js = 0;
  ...
  L_8:
    ...
    dp = Pt[ie] - ps;
    dps = sqrt(fabs(dp));
    x = tabc.tau[j - 1][1] + 2e0 * dp * tabc.tau[j - 1][2] +
        1.5e0 * dps * tabc.tau[j - 1][3];
    *tcor =
        tabc.tn * (tabc.tau[j - 1][0] +
                   dp * (tabc.tau[j - 1][1] + dp * tabc.tau[j - 1][2] +
                         dps * tabc.tau[j - 1][3]) + ps * x);
     ...
     int jb, int_;
{
    ...
    static int i, i1, i2, i_, is, j, k, mncnt, mxcnt, nn;
    static double dmn, dmx, dpe, dtau, hm, p0, p1, pe, pe0, pe1, pmn, scpe0,
        scpe1, shm, spe0, spe1, tau0, tau1, thm, x0, x1;
...
*/
  ...
  L_14:
    ...
    pdecu(i1, i2, tabc.xbrn[0][jb - 1], tabc.xbrn[1][jb - 1], xmin, int_, &i2);
    tabc.jndx[1][jb - 1] = i2;
  L_12:
    nn = i2 - i1 + 1;
    if (makgrd)
  ...
  L_11:
    ...
    for (i = is; i <= i2; i++) {
        ...
        scpe1 = pe1 * spe1;
        tabc.tau[j - 1][3] =
            (2e0 * dtau - dpe * (x1 + x0)) / (.5e0 * (scpe1 - scpe0) -
                                              1.5e0 * spe1 * spe0 * (spe1 -
                                                                     spe0));
        tabc.tau[j - 1][2] =
            (dtau - dpe * x0 -
             (scpe1 + .5e0 * scpe0 - 1.5e0 * pe1 * spe0) * tabc.tau[j -
                                                                    1][3]) /
            (dpe * dpe);
        tabc.tau[j - 1][1] =
            (dtau - (pe1 * pe1 - pe0 * pe0) * tabc.tau[j - 1][2] -
             (scpe1 - scpe0) * tabc.tau[j - 1][3]) / dpe;
        tabc.tau[j - 1][0] =
            tau0 - scpe0 * tabc.tau[j - 1][3] -
            pe0 * (pe0 * tabc.tau[j - 1][2] + tabc.tau[j - 1][1]);
...
/* L_7: */
        thm =
            tabc.tau[j - 1][1] + shm * (2e0 * shm * tabc.tau[j - 1][2] +
     ...
     int modnam_s;
{
    ...
    for (i = 1; i <= brkc.nbrn; i++) {
      ...
      L_9:
        if (brkc.nafl[0][k - 1] > 0 &&
            (pcdc.phcd[i_][0] == 'P' || pcdc.phcd[i_][0] == 'S'))
            goto L_3;
        for (j = 1; j <= 6; j++) {
            j_ = j - 1;
...
*/
  ...
  L_1:
    ...
    if (ptk > 1e-9)
        goto L_2;
    *tau =
        -(pti - ptj + b * log(pti / ptj) -
          b * log(fmax((ptj - b) * pti / ((pti - b) * ptj), 1e-30)));
    *x = 0e0;
    goto L_4;
  L_2:
    if (ptk == pti)
  ...
  L_5:
    ...
    xx = log(fmax
             ((ptj - b) * (sqb * sqi + b * pti -
                           sqk) / ((pti - b) * (sqb * sqj + b * ptj - sqk)),
              1e-30));
    *x = ptk * xx / sqb;
    goto L_6;
  L_7:
    xx = asin(fmax(fmin((b * pti - sqk) / (ptk * fabs(pti - b)), 1e0), -1e0)) -
        asin(fmax(fmin((b * ptj - sqk) / (ptk * fabs(ptj - b)), 1e0), -1e0));
    *x = -ptk * xx / sqb;
  L_6:
  ...
  L_3:
    ...
    *x = pti * xx / sqb;
    goto L_9;
  L_8:
    xx = sign(1.5707963267948966e0,
              b - pti) -
        asin(fmax(fmin((b * ptj - sqk) / (pti * fabs(ptj - b)), 1e0), -1e0));
    *x = -pti * xx / sqb;
  L_9:
  ...
  L_11:
    ...
    xx = log(fmax
             ((ptj - b) * (sqb * sqi + b * pti -
                           sqk) / ((pti - b) * (b * ptj - sqk)), 1e-30));
    *x = ptj * xx / sqb;
    goto L_14;
  L_12:
    xx = asin(fmax(fmin((b * pti - sqk) / (ptj * fabs(pti - b)), 1e0), -1e0)) -
        sign(1.5707963267948966e0, b - ptj);
    *x = -ptj * xx / sqb;
  L_14:
     ...
     double pt[], coef[][5];
{
    ...
    for (i = 1; i <= n2; i++) {
        ...
        ali =
            1e0 / (.125e0 * D3h[1] -
                   (.75e0 * D1h[1] + .375e0 * Dih[1] * Del[3]) * Del[3]);
        alr =
            ali * (.125e0 * Del[2] * Sdel[2] -
                   (.75e0 * Sdel[2] + .375e0 * Del[3] * Deli[2] -
                    Sdel[3]) * Del[3]);
        b3h = D3h[2] + alr * D3h[1];
        b1h = D1h[2] + alr * D1h[1];
        bih = Dih[2] + alr * Dih[1];
        ...
        D[4] =
            ali * ((Dih[1] * b3h - D3h[1] * bih) * th2p -
                   th2m * th0p) / ((Dih[4] * b3h - D3h[4] * bih) * th2p -
                                   th2m * th3p);
        ...
        coef[l - 1][0] =
            (.125e0 * Del[5] * Sdel[5] -
             (.75e0 * Sdel[5] + .375e0 * Deli[5] * Del[4] -
              Sdel[4]) * Del[4]) * D[4];
        if (i >= 3)
            coef[l - 3][1] =
                (.125e0 * Del[1] * Sdel[1] -
                 (.75e0 * Sdel[1] + .375e0 * Deli[1] * Del[2] -
                  Sdel[2]) * Del[2]) * D[1];
        /*   Construct the contributions -dG;i-1(p)/dp | p;i-2, p;i-1, and p;i. */
        coef[l - 1][2] =
            -.75e0 * (Sdel[5] + Deli[5] * Del[4] - 2e0 * Sdel[4]) * D[4];
        if (i >= 2)
            coef[l - 2][3] =
                -.75e0 * ((Sdel[2] + Deli[2] * Del[3] - 2e0 * Sdel[3]) * D[2] -
                          (D1h[1] + Dih[1] * Del[3]) * D[1]);
        if (i >= 3)
            coef[l - 3][4] =
                -.75e0 * (Sdel[1] + Deli[1] * Del[2] - 2e0 * Sdel[2]) * D[1];
    }
  ...
  L_10:
    for (j = 1; j <= 4; j++) {
      ...
      L_6:
        alr =
            (Sdel[2] + Deli[2] * Del[3] - 2e0 * Sdel[3]) / (D1h[1] +
                                                            Dih[1] * Del[3]);
        D[2] =
            1e0 / (.125e0 * Del[2] * Sdel[2] -
                   (.75e0 * Sdel[2] + .375e0 * Deli[2] * Del[3] -
                    Sdel[3]) * Del[3] - (.125e0 * D3h[1] - (.75e0 * D1h[1] +
                                                            .375e0 * Dih[1] *
                                                            Del[3]) * Del[3]) *
                   alr);
        D[1] = alr * D[2];
      ...
      L_8:
        D[3] =
            -((D3h[2] - D1h[2] * Del[4]) * D[2] +
              (D3h[1] - D1h[1] * Del[4]) * D[1]) / (D3h[3] - D1h[3] * Del[4]);
        D[4] =
            (D3h[3] * D[3] + D3h[2] * D[2] +
             D3h[1] * D[1]) / (Del[4] * Sdel[4]);
        /*   Construct the contributions G;i-1(p;i-2) and G;i-1(p;i). */
      L_9:
        if (j <= 2)
            coef[l - 1][0] =
                (.125e0 * Del[3] * Sdel[3] -
                 (.75e0 * Sdel[3] + .375e0 * Deli[3] * Del[4] -
                  Sdel[4]) * Del[4]) * D[3] - (.125e0 * D3h[2] -
                                               (.75e0 * D1h[2] +
                                                .375e0 * Dih[2] * Del[4]) *
                                               Del[4]) * D[2] -
                (.125e0 * D3h[1] -
                 (.75e0 * D1h[1] + .375e0 * Dih[1] * Del[4]) * Del[4]) * D[1];
        if (l - i1 > 1)
            coef[l - 3][1] =
                (.125e0 * Del[1] * Sdel[1] -
                 (.75e0 * Sdel[1] + .375e0 * Deli[1] * Del[2] -
        ...
        if (j <= 2)
            coef[l - 1][2] =
                -.75e0 * ((Sdel[3] + Deli[3] * Del[4] - 2e0 * Sdel[4]) * D[3] -
                          (D1h[2] + Dih[2] * Del[4]) * D[2] - (D1h[1] +
                                                               Dih[1] *
                                                               Del[4]) * D[1]);
        if (j <= 3 && l - i1 > 0)
            coef[l - 2][3] = 0e0;
...
#define PHNM(I_,J_)	(phnm+(I_)*(phnm_s)+(J_))
  ...
  L_6:
    ...
    for (i = 1; i <= *n; i++) {
        ...
        if (k <= 0)
            goto L_8;
        if (memcmp(PHNM(k - 1, 0), ctmp[j - 1], strlen(ctmp[j - 1])) == 0 &&
            fabs(Tt[k] - tmp[0][j - 1]) <= atol)
            goto L_2;
      L_8:
        k = k + 1;
        Tt[k] = tmp[0][j - 1];
        ...
        Dtdh[k] = tmp[2][j - 1];
        Dddp[k] = tmp[3][j - 1];
        fstrncpy(PHNM(k - 1, 0), phnm_s - 1, ctmp[j - 1], strlen(ctmp[j - 1]));
      L_2:
        ;
    }
    *n = k;
...
*/
    ...
    exit(-1);
  L_2:
    if (fabs(zs - umdc.zm[nph - 1][i - 1]) <= dtol &&
        fabs(umdc.zm[nph - 1][i - 1] - umdc.zm[nph - 1][i]) <= dtol)
        goto L_3;
    j = i - 1;
    Isrc[nph] = j;
    umod_v =
        umdc.pm[nph - 1][j - 1] + (umdc.pm[nph - 1][i - 1] -
                                   umdc.pm[nph - 1][j - 1]) * (exp(zs -
                                                                   umdc.zm[nph -
                                                                           1][j
                                                                              -
                                                                              1])
                                                               -
                                                               1e0) /
        (exp(umdc.zm[nph - 1][i - 1] - umdc.zm[nph - 1][j - 1]) - 1e0);
    return (umod_v);
  L_3:
    Isrc[nph] = i;
  ...
  _zmod_:                      /*ENTRY Point */
    ...
    zmod_v =
        ...
        log(fmax
            ((uend -
              umdc.pm[nph - 1][js - 1]) * (exp(umdc.zm[nph - 1][i - 1] -
                                               umdc.zm[nph - 1][js - 1]) -
                                           1e0) / (umdc.pm[nph - 1][i - 1] -
                                                   umdc.pm[nph - 1][js - 1]) +
             1e0, 1e-30));
    return (umod_v);
}                               /* end of function */
