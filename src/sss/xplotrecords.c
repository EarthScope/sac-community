     ...
     int *nerr;
{
    char kpllab[MCPFN + 7],     /* Number of elements increased. Add room for jdfl. maf 970129 */
      kvr[9];
    int lany, lbotaxsave, lbottcsave, lflip, lframesave, llefaxsave, lleftcsave,
    ...
    float angle, atime, delay, delsiz, dgscale, dstchn, dstmn, dstmx, dvr,
        ...
        xaxlen, xbp, xep, xfac, xpdel, xpllen, xpmid, xpmnsv, xpmxsv, xttint,
        xwloc, yaxlen, ybp, yep, ypllen, yttint, ywloc;
...
/*	const	int maxCropLevels = 5 ;*/
    ...
    *nerr = 0;
    ...
    while (lcmore(nerr)) {
        ...
        if (lklogc("LABELS$", 8, &cmsss.lrslab, kmsss.knmlab, 9)) {     /* do nothing */
        }

        /* -- "WEIGHTS ON|OFF":  turn weighting on or off. */
        else if (lklog("WEIGHTS$", 9, &cmsss.lrswt)) {  /* do nothing */
        }

        /* -- "CURSOR ON|OFF":  plot with or without cursor. */
        else if (lklog("CURSOR$", 8, &cmsss.lrscur)) {  /* do nothing */
        }

        /* -- "POLARITY ON|OFF":  plot with or without polarities. */
        else if (lklog("POLARITY$", 10, &cmsss.lrspol)) {       /* do nothing */
        }

        /* -- "REFERENCELINE ON|OFF":  plot with or without reference line. */
        else if (lklog("REFERENCELINE$", 15, &cmsss.lrslin)) {  /* do nothing */
        }
        ...
        else if (lklog("ASPECT$", 8, &cmsss.laspect)) { /* do nothing */
        }
        ...
        else if (lklog2("ORIGIN$", 8, "DEFAULT$", 9, "REVERSED$", 10, &cmsss.lOriginDefault)) { /* do nothing */
        }

        /* -- "ORIENT PORTRAIT|LANDSCAPE": Plot in portrait or landscape mode */
        else if (lklog2("ORIENT$", 8, "PORTRAIT$", 10, "LANDSCAPE$", 11, &cmsss.lorient)) {     /* do nothing */
        }

        /* -- "TTIME ON|OFF|DEFAULT|text": Turn travel time curves on/off */
        else if (lckey("TTIME$", 7)) {
            if (lclog(&cmtt.lttm)) {    /* do nothing */
        ...
        /* -- "XLABEL ON|OFF|DEFAULT|text": Set x axis label properties. */
            ...
            if (lclog(&cmsss.lxlabreq)) {       /* do nothing */
            } else if (lckey("DEFAULT$", 9)) {
                cmsss.lxlabreq = TRUE;
                cmsss.lxlabdef = TRUE;
        ...
        /* -- "YLABEL ON|OFF|DEFAULT|text": Set y axis label properties. */
            ...
            if (lclog(&cmsss.lylabreq)) {       /* do nothing */
            } else if (lckey("DEFAULT$", 9)) {
                cmsss.lylabreq = TRUE;
                cmsss.lylabdef = TRUE;
        ...
        /* -- "REDUCED ON | OFF | PHASE phase | VELOCITY velocity" */
            ...
            if (lclog(&cmtt.lrdtt)) {   /* do nothing */
    ...
    while (cropLevel >= 0) {    /* when cropLevel turns negative, end loop */
        ...
        for (jdfl = 1; jdfl <= saclen(); jdfl++) {
            ...
            if (lflip) {
                if (cmsss.lorient) {
                    ...
                    cmgem.plot.ymin =
                        ypmid -
                        s->h->scale * ypdel * cmsss.xpsize * vmx / vbigr;
                    cmgem.plot.ymax =
                        ypmid -
                        s->h->scale * ypdel * cmsss.xpsize * vmn / vbigr;
                } else {
                    cmgem.ximn = vmx;
                    cmgem.ximx = vmn;
                    cmgem.plot.xmin =
                        xpmid -
                        s->h->scale * xpdel * cmsss.xpsize * vmx / vbigr;
                    cmgem.plot.xmax =
                        xpmid -
                        s->h->scale * xpdel * cmsss.xpsize * vmn / vbigr;
                }
            } else {
                if (cmsss.lorient) {
                    cmgem.yimn = vmx;
            ...
            if (cmsss.lorient) {
                if (cmgem.plot.ymin < ypmnsv) {
                    cmgem.yimn =
                        cmgem.yimx + (cmgem.yimn - cmgem.yimx) * (ypmnsv -
                                                                  cmgem.plot.
                                                                  ymax) /
                        (cmgem.plot.ymin - cmgem.plot.ymax);
                    cmgem.plot.ymin = ypmnsv;
                }
                if (cmgem.plot.ymax > ypmxsv) {
                    cmgem.yimx =
                        cmgem.yimn + (cmgem.yimx - cmgem.yimn) * (ypmxsv -
                                                                  cmgem.plot.
                                                                  ymin) /
                ...
                if (cmgem.plot.xmin < xpmnsv) {
                    cmgem.ximn =
                        cmgem.ximx + (cmgem.ximn - cmgem.ximx) * (xpmnsv -
                                                                  cmgem.plot.
                                                                  xmax) /
                        (cmgem.plot.xmin - cmgem.plot.xmax);
                    cmgem.plot.xmin = xpmnsv;
                }
                if (cmgem.plot.xmax > xpmxsv) {
                    cmgem.ximx =
                        cmgem.ximn + (cmgem.ximx - cmgem.ximn) * (xpmxsv -
                                                                  cmgem.plot.
                                                                  xmin) /
            ...
            if (cmsss.lrslab) {
                ...
                if (cmsss.lorient) {
                    line(cmgem.view.xmax * cmgem.plot.xmin - cmgem.chwid,
                         ypmidu, cmgem.view.xmax * cmgem.plot.xmin, ypmidu);
                    line(cmgem.view.xmax * cmgem.plot.xmax + cmgem.chwid,
                         ypmidu, cmgem.view.xmax * cmgem.plot.xmax, ypmidu);
                } else {
            ...
            for (jvr = 1; jvr <= nvr; jvr++) {
                if (cmsss.lorient) {    /* Added to allow portrait mode, maf 960829 */
                    ...
                    if (Lvm[1])
                        angle =
                            atan((1. / Vapp[1] -
                                  1. / vr) * (cmgem.ympip1 / cmgem.xmpip1));
                    else
                        angle = atan(cmgem.ympip1 / (cmgem.xmpip1 * vr));
            ...
            for (kdx = 1; kdx <= cmtt.nttm; kdx++) {
                ...
                if (cmtt.lrdtt && (cmtt.nttrd > 0)) {
                    ...
                    if (cmsss.lorient /*== cmtt.lpreviousModel*/ ) {
                        pl2d(y1, ttx[kdx], Nttpt[kdx], 1, 1, nerr);
                    } /* end if ( cmsss.lorient == cmtt.lpreviousModel ) */
                    else {
                        pl2d(ttx[kdx], y1, Nttpt[kdx], 1, 1, nerr);
                    }           /* end else associated with if ( cmsss.lorient ) */
                    cmsss.lPlottingTT = FALSE;
                    ttint(ttx[kdx], y1, Nttpt[kdx], &xttint, &yttint, nerr);
                    FREE(y1);
                }
        ...
        if (cmsss.lrscur) {
            ...
            nextLevel =
                (cropLevel + 1 > maxCropLevels ? maxCropLevels : cropLevel + 1);
